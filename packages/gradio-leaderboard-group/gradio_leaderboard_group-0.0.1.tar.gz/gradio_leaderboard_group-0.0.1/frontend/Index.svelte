<script lang="ts">
	...
	export let fixed_precision: int = 3;
	...
	function cleanValue(data) {
		return data.map((v) => {
			...
			console.log(x.toFixed(fixed_precision));
			return x.toFixed(fixed_precision);
		});
	}

	let _headers: Headers;
	let original_headers = value.headers.map((s) => s);
	let original_data = value.data.map((s) => cleanValue(s));
	let display_value: string[][] | null;
	let styling: string[][] | null;
	let values: (string | number)[][];
	let filter_values: [string, number][] = [];
	let search_value: string | null = null;
	let default_selection = select_columns_config.default_selection.length
		? select_columns_config.default_selection
		: original_headers;

	let header_groups: Record<string, string[]> = {};
	...
	async function handle_change(data?: {
		...
		const display_headers = _headers.filter((h) =>
			default_selection.includes(h),
		);
		const display_indices = display_headers.map((name) =>
			_headers.indexOf(name),
		);
		...
		display_value = _data?.metadata?.display_value
			...
			: null;
		styling = _data?.metadata?.styling
	...
	function select_columns(values, on_load_columns) {
		const display_headers = original_headers.filter((h) =>
			on_load_columns.includes(h),
		);
		const display_indices = display_headers.map((name) =>
			original_headers.indexOf(name),
		);
		_headers = display_headers;
		values = values.map((row) => display_indices.map((i) => row[i]));
		return values;
	...
	function filter_column(column: ColumnFilter, value: any[] | any) {
		...
		if (column.type === "slider") {
			...
			filter = (row) => {
				return compare(
					row[original_headers.indexOf(column.column)],
					min,
					max,
				);
			};
			...
			filter = (row) => {
				return value.some(
					(v) => row[original_headers.indexOf(column.column)] === v,
				);
			};
		}
	...
	function search_column_values(
		...
		const query_values = search_value
			.split(";")
			.map((s) => s.trim())
			.filter((s) => s.length);
		...
		for (let i = 0; i < values.length; i++) {
			...
			for (let j = 0; j < query_values.length; j++) {
				...
				let column_index = headers.indexOf(
					search_columns.primary_column,
				);
				...
				if (colon_index !== -1) {
					...
					if (
						!search_columns.secondary_columns.length ||
						!search_columns.secondary_columns.includes(col_name)
					) {
						if (!triggered_warning)
							gradio.dispatch(
								"warning",
								`Column ${col_name} not found in secondary columns of search_columns`,
							);
				...
				const push_to =
					colon_index !== -1
						? secondary_column_matches
						: primary_column_matches;
				const val =
					values[i][column_index] === null
						? ""
						: values[i][column_index].toString();
			...
			if (
				primary_column_matches.length &&
				!secondary_column_matches.length
			) {
				...
				mask[i] =
					primary_column_matches.some((s) => s) ||
					secondary_column_matches.every((s) => Boolean(s));
			}
		}
		return mask;
	}
	...
	function filter_column_values(
		values,
		headers,
		filter_values: [string, any][],
		search_columns,
		search_value,
	) {
		console.log("filter_values", filter_values);
		const search_value_mask = search_column_values(
			original_data,
			headers,
			search_columns,
			search_value,
		);
		const masks = filter_values
			.map((tup) => filter_column(get_column_filter(tup[0]), tup[1]))
			.concat([search_value_mask]);
	...
	$: {
		if (JSON.stringify(value) !== old_value) {
			...
			original_data = value.data.map((s) => cleanValue(s));
			handle_change();
		}
	}
	...
	const non_checkbox_filter_columns = filter_columns.filter(
		(col) => col.type !== "checkbox",
	);
	const checkbox_filter_columns = filter_columns.filter(
		(col) => col.type === "checkbox",
	);
	checkbox_filter_columns.forEach((col, i) => {
		if (col.default) {
			filter_values[non_checkbox_filter_columns.length + i] = [
				col.column,
				col.default,
			];
		}
	...
	const h2g = Object.keys(select_columns_config.groups).reduce((acc, key) => {
		select_columns_config.groups[key].forEach((k) => (acc[k] = key));
		return acc;
	}, {});
	headers.forEach((h) => {
		const k = h2g[h] || "default";
		const g = header_groups[k] || [];

		if (
			!(
				hide_columns.includes(h) ||
				select_columns_config.cant_deselect.includes(h)
			)
		) {
			g.push(h);
		}
	...
	function select_all(): void {
		headers.forEach((internal_value, i) => {
			if (!default_selection.includes(internal_value)) {
				default_selection = [...default_selection, internal_value];
			}
	...
	function select_none(): void {
		headers.filter(h => !(
				hide_columns.includes(h) ||
				select_columns_config.cant_deselect.includes(h)
			)).forEach((internal_value, i) => {
			default_selection = default_selection.filter(
				(v) => v != internal_value,
			);
			gradio.dispatch("select", {
				index: i,
	...
	function toggleColumn(h) {
		console.log(h);
		if (default_selection.includes(h)) {
			default_selection = default_selection.filter((v) => v !== h);
		} else {
			default_selection = [...default_selection, h];
		}
	}
</script>
...
<Row>
	<Column>
		{#if search_columns.primary_column}
			<Row>
				<Simpletextbox
					...
					show_label={true}
					placeholder={search_columns.placeholder ??
		...
		{#if select_columns_config.allow}
			<Row
				><span>{select_columns_config.label}</span>
			...
			<Row>
				<div class="quicktoggle-container">
					<label for="quicktoggle">Quick Toggle:</label>
					<AutoComplete
						id="quicktoggle"
						items={headers.filter(
							(h) =>
								!(
									hide_columns.includes(h) ||
									select_columns_config.cant_deselect.includes(
										h,
									)
								),
						)}
						...
						dropdownClassName="quicktoggle-dropdown"
						inputClassName="quicktoggle-input"
					/>
				</div>
			</Row>
			{#each Object.entries(header_groups) as [group_name, group_headers]}
				{#if group_headers.length > 0}
					<Row>
						<Checkboxgroup
							...
							choices={group_headers.map((s) => [s, s])}
							{loading_status}
							is_collapsed={true}
						/>
					</Row>
			...
			<div>
				<Form>
					{#each non_checkbox_filter_columns as col, i}
						{#if col.type === "checkboxgroup"}
							<Checkboxgroup
								...
								on:input={(e) =>
									(filter_values[i] = [col.column, e.detail])}
							/>
					...
					{#if checkbox_filter_columns.length}
						<Checkboxgroup
							label={bool_checkboxgroup_label ||
								"Show Rows with the Following Values"}
							show_label={true}
							{gradio}
							{loading_status}
							choices={checkbox_filter_columns.map((col) => [
								col.label ?? col.column,
								col.column,
							])}
							value={checkbox_filter_columns
								.map((col) => (col.default ? col.column : null))
								.filter((s) => s)}
							on:input={(e) => {
								checkbox_filter_columns.forEach((col, i) => {
									filter_values[
										non_checkbox_filter_columns.length + i
									] = [
										col.column,
										e.detail.includes(col.column),
									];
								});
							}}
						/>
...
<style global>
	.quicktoggle-container {
		display: grid;
		grid-template-columns: 7em auto;
	}
	.autocomplete-list.quicktoggle-dropdown {
	...
	.autocomplete-input.quicktoggle-input {
