...
from spark_dataproc_local_tools.utils import BASE_DIR, BASE_PROJECT
...
def get_install_virtualenv(env_path, pip_version):
    ...
    try:
        ...
        env_path_dir = os.path.join(BASE_PROJECT, env_path)
...
def get_library_install(env_path):
    print("=====================================")
    print("===========INSTALL LIBRARY===========")
    print("=====================================")
    pip_path = Path(env_path) / ("Scripts" if sys.platform == "win32" else "bin") / "pip"
    ...
    if paquetes:
        for paquete in paquetes:
            try:
                ...
                print(f"Paquete '{paquete}' instalado en el entorno '{env_path}'.")
            except subprocess.CalledProcessError as e:
                print(f"Error al instalar el paquete '{paquete}': {e}")


def get_wheel_install(env_path):
    print("=====================================")
    print("===========INSTALL WHEEL=============")
    print("=====================================")
    pip_path = Path(env_path) / ("Scripts" if sys.platform == "win32" else "bin") / "pip"
    ...
    if wheels:
        for wheel in wheels:
            try:
                ...
                print(f"Archivo .whl '{wheel}' instalado en el entorno '{env_path}'.")
            except subprocess.CalledProcessError as e:
...
def run(env_path, pip_version="24.0"):
    ...
    get_install_virtualenv(env_path, pip_version)
    get_library_install(env_path)
    get_wheel_install(env_path)
    return True
