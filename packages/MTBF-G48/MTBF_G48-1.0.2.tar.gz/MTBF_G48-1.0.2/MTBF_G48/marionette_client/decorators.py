...
from errors import KaiOSMarionetteException
...
def do_crash_check(func, always=False):
    ...
    def _(*args, **kwargs):
        ...
        except (KaiOSMarionetteException, socket.error, IOError) as e:
            exc, val, tb = sys.exc_info()
            if not isinstance(e, KaiOSMarionetteException) or type(e) is KaiOSMarionetteException:
                if not always:
                    check()
            raise exc, val, tb
        finally:
