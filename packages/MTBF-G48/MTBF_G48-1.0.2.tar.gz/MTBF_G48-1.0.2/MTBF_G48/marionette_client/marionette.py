...
class Marionette(object):
    """Represents a Marionette connection to a device."""
    ...
    # https://git.kaiostech.com/KaiOS/gecko48/blob/master/netwerk/base/nsIPermissionManager.idl#L50
    UNKNOWN_ACTION = 0
    ALLOW_ACTION = 1
    DENY_ACTION = 2
    PROMPT_ACTION = 3

    def __init__(self, host='localhost', port=2828, baseurl=None, device_serial=None,
                 adb_host=None, adb_port=None, timeout=None, socket_timeout=360):
        self.host = host
    ...
    def push_permission(self, perm, allow):
        # Workaround to reduce memory usage of push_permission.
        permission = self.get_permission(perm)
        if permission == self.ALLOW_ACTION and allow:
            return
        elif permission == self.DENY_ACTION and not allow:
            return
    ...
    def execute_script(self, script, script_args=None, new_sandbox=False,
    ...
    def execute_async_script(self, script, script_args=None, new_sandbox=False,
                             sandbox="default", script_timeout=None,
                             debug_script=False):
