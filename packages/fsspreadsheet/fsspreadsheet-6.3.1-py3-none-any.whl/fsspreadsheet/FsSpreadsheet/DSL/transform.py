...
from ...fable_modules.fable_library.list import (is_empty, head, cons, tail as tail_25, singleton, reverse, empty, of_array_with_tail, FSharpList, iterate, iterate_indexed, map, transpose, choose)
from ...fable_modules.fable_library.seq import max
from ...fable_modules.fable_library.set import (of_seq, FSharpSet__Contains, add)
...
def FsSpreadsheet_DSL_Workbook__Workbook_parseRow_Static(cell_collection: FsCellsCollection, row: FsRow, els: FSharpList[RowElement]) -> None:
    ...
    cell_index_set: Any = of_seq(choose(chooser, els), ObjectExpr194())
    def action(el_1: RowElement, cell_collection: Any=cell_collection, row: Any=row, els: Any=els) -> None:
        if el_1.tag == 1:
            def _arrow195(__unit: None=None, el_1: Any=el_1) -> int:
            ...
            cell_1: FsCell = row.Item(_arrow195())
...
def FsSpreadsheet_DSL_Workbook__Workbook_parseSheet_Static(sheet: FsWorksheet, els: FSharpList[SheetElement]) -> None:
    ...
    row_index_set: Any = add(0, of_seq(choose(chooser, els), ObjectExpr196()))
    def action_3(_arg: tuple[str, FSharpList[SheetElement]], sheet: Any=sheet, els: Any=els) -> None:
        ...
        if pattern_matching_result == 0:
            ...
            base_row_index: int = (1 + max(row_index_set, ObjectExpr197())) or 0
            ...
            column_index_set: Any = of_seq(choose(chooser_1, columns), ObjectExpr198())
            def action_1(col_1: SheetElement, _arg: Any=_arg) -> None:
                ...
                if col_1.tag == 3:
                    ...
                    def _arrow199(__unit: None=None, col_1: Any=col_1) -> int:
                    ...
                    pattern_input = (_arrow199(), col_1.fields[0])
                ...
                cell_index_set: Any = of_seq(choose(chooser_2, elements_2), ObjectExpr200())
                def action(el_2: ColumnElement, col_1: Any=col_1) -> None:
                    nonlocal row_index_set
                    if el_2.tag == 1:
                        def _arrow201(__unit: None=None, el_2: Any=el_2) -> int:
                            nonlocal cell_index_set
                            i_6: int = 1
                        ...
                        row_1: FsRow = sheet.Row((_arrow201() + base_row_index) - 1)
            ...
            def action_2(_arg_1: SheetElement, _arg: Any=_arg) -> None:
                if _arg_1.tag == 1:
                    ...
                    def _arrow202(__unit: None=None, _arg_1: Any=_arg_1) -> int:
                        nonlocal row_index_set
                        i_1: int = 1
                    ...
                    row_4: FsRow = sheet.Row(_arrow202())
...
def FsSpreadsheet_DSL_Workbook__Workbook_Parse(self_1: Workbook) -> FsWorkbook:
    ...
    def action(i: int, wb_el: WorkbookElement, self_1: Any=self_1) -> None:
        if wb_el.tag == 1:
            ...
            def _arrow203(__unit: None=None, i: Any=i, wb_el: Any=wb_el) -> str:
                arg: int = (i + 1) or 0
                return to_text(printf("Sheet%i"))(arg)

            worksheet: FsWorksheet = FsWorksheet(_arrow203())
