    ...
    def create_at(index: int, cells: FsCellsCollection) -> FsRow:
        def get_index_by(f: Callable[[Callable[[FsCell], int], IEnumerable_1[FsCell]], FsCell]) -> int:
            if length(FsCellsCollection__GetCellsInRow_Z524259A4(cells, index)) == 0:
                ...
                def _arrow119(c: FsCell, f: Any=f) -> int:
                    return FsAddress__get_ColumnNumber(c.Address)

                return FsAddress__get_ColumnNumber(f(_arrow119)(FsCellsCollection__GetCellsInRow_Z524259A4(cells, index)).Address)


        def _arrow122(projection: Callable[[FsCell], int]) -> Callable[[IEnumerable_1[FsCell]], FsCell]:
            def _arrow121(source_1: IEnumerable_1[FsCell]) -> FsCell:
                class ObjectExpr120:
                    @property
                    def Compare(self) -> Callable[[int, int], int]:
                        return compare_primitives

                return min_by(projection, source_1, ObjectExpr120())

            return _arrow121

        min_col_index: int = get_index_by(_arrow122) or 0
        def _arrow125(projection_1: Callable[[FsCell], int]) -> Callable[[IEnumerable_1[FsCell]], FsCell]:
            def _arrow124(source_2: IEnumerable_1[FsCell]) -> FsCell:
                class ObjectExpr123:
                    @property
                    def Compare(self) -> Callable[[int, int], int]:
                        return compare_primitives

                return max_by(projection_1, source_2, ObjectExpr123())

            return _arrow124

        max_col_index: int = get_index_by(_arrow125) or 0
