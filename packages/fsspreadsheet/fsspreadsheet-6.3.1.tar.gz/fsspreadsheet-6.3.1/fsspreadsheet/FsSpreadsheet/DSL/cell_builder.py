...
from ...fable_modules.fable_library.list import (reduce, map, FSharpList, empty, append, is_empty, tail, head)
from ...fable_modules.fable_library.reflection import (TypeInfo, char_type, union_type, class_type)
...
ReduceOperation_reflection = _expr204
...
CellBuilder_reflection = _expr205
...
def CellBuilder__SignMessages_3F89FFFC(this: CellBuilder, messages: FSharpList[Message]) -> FSharpList[Message]:
    def mapping(m: Message, this: Any=this, messages: Any=messages) -> Message:
        def _arrow206(__unit: None=None, m: Any=m) -> Callable[[str], str]:
            clo: Callable[[str], str] = to_text(printf("In Cell: %s"))
            return clo

        return Message__MapText_11D407F6(m, _arrow206())
