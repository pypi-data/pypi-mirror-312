...
class Test_DLD:
    ...
    __test__ = False
    lst_test_var_core = ['Np', 'Nw', 'd_c', 'gap_w', 'pitch_w', 'gap_a',\
        'pitch_a', 'height', 'boundary_treatment']
    lst_test_var_block = ['num_unit', 'opt_mirror', 'array_counts',\
    ...
    def compare(val, ground_truth, rel=1e-6, abs=1e-12):
        ...
        elif (type(ground_truth) in [list, tuple] and
            type(val) in [list, tuple]):
            lst_check=[]
            for sn_item, item in enumerate(val):
                lst_check.append(Test_DLD.compare(val[sn_item],\
                    ground_truth[sn_item]))
            return np.all(lst_check)
        else:
            raise ValueError(f"""Invalid type of ``ground truth``
