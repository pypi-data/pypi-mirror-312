...
def _capture_call_site(spark_session: "SparkSession", depth: int) -> str:
    ...
    if ipython:
        call_sites = [
            # BEGIN-EDGE
            # Cell execution count is not supported from DBR Notebook for now.
            # Should enable the commented line below when cell execution count is supported.
            # f"line {frame.lineno} in cell [{ipython.execution_count}]"
            f"line {frame.lineno}"
            for frame in selected_frames
            # END-EDGE
        ]
