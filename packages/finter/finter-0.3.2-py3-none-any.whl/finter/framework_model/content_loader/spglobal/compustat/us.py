...
class CompustatLoader(Loader):
    ...
    def get_df(
        ...
        if adj:
            adj_cm_name = f"content.spglobal.compustat.cax.us-{self.__INST}-adjust_factor.1d"

            BaseAlpha.get_cm(adj_cm_name)
            adj_factor = self._load_cache(adj_cm_name, **base_args).reindex(columns=raw_col).fillna(1)

            if adj_div:
                BaseAlpha.get_cm(f"content.spglobal.compustat.cax.us-{self.__INST}-total_return_factor.1d")
                total_return_factor = self._load_cache(
                    f"content.spglobal.db.cax.us-{self.__INST}-total_return_factor.1d",
                    start,
                    end,
                    universe=f"us-us-{self.__INST}",
                    freq=self.__FREQ,
...
#
# BaseAlpha.get_cm(f"spglobal.db.cax.us-{self.__INST}-total_return_factor.1d")
# total_return_factor = self._load_cache(
#     f"spglobal.db.cax.us-{self.__INST}-total_return_factor.1d",
#     start,
#     end,
#     universe=f"us-us-{self.__INST}",
#     freq=self.__FREQ,
#     cache_t="hdf",
#     fill_nan=fill_nan
# ).reindex(columns=raw.columns).fillna(1)
# adj_factor /= total_return_factor
