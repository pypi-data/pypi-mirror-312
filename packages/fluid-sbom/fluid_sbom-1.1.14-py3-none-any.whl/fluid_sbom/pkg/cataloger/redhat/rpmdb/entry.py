...
def ei2h(package_entry: EntryInfo) -> EntryInfo:
    return EntryInfo(
        type=htonlu(
            package_entry.type
        ),  # Convert unsigned 32-bit integer to network byte order
        count=htonlu(
            package_entry.count
        ),  # Convert unsigned 32-bit integer to network byte order
        offset=htonl(
...
def region_swab(
    ...
    for index, pkg_entry in enumerate(pe_list):
        index_entry = IndexEntry(
            info=ei2h(pkg_entry), length=0, rd_len=0, data=b""
        )
        ...
        if (
            index < len(pe_list) - 1
            and TYPE_SIZES[index_entry.info.type] == -1
        ):
            index_entry.length = (
                htonl(pe_list[index + 1].offset) - index_entry.info.offset
            )
        else:
        ...
        d_length += index_entry.length + align_diff(
            index_entry.info.type, d_length
        )
...
def hdrblob_import(blob: HdrBlob, data: bytes) -> list[IndexEntry]:
    ...
    if entry.tag >= RPMTAG_HEADERI18NTABLE:
        ...
        index_entries, rdlen = region_swab(
            data, blob.pe_list, 0, blob.data_start, blob.data_end
        )
    else:
        ...
        index_entries, rdlen = region_swab(
            data, blob.pe_list[1:ril], 0, blob.data_start, blob.data_end
        )
        if rdlen < 0:
        ...
        if blob.ril < len(blob.pe_list) - 1:
            dribble_index_entries, rdlen = region_swab(
                data, blob.pe_list[ril:], rdlen, blob.data_start, blob.data_end
            )
            if rdlen < 0:
...
def _parse_and_validate_trailer(
    blob: HdrBlob, data: bytes, einfo: EntryInfo, region_tag: int
) -> None:
    ...
    try:
        ...
        EntryInfo(
            tag=result[0], type=result[1], offset=result[2], count=result[3]
        )
    ...
    if (
        ...
        and einfo.tag == RPMTAG_HEADERIMAGE
    ):
...
def _process_trailer_and_update_blob(blob: HdrBlob, data: bytes) -> None:
    result = struct.unpack(
        ...
        data[
            blob.data_start
            + blob.rdl
            - REGION_TAG_COUNT : blob.data_start
            + blob.rdl
        ],
    )
    trailer = EntryInfo(
...
def _read_pe_list(blob: HdrBlob, data: bytes) -> None:
    ...
    if blob.pv_len >= HEADER_MAX_BYTES:
        raise ValueError(
            f"Blob size({blob.pv_len}) BAD, 8 + 16 * il({blob.i_length}) +"
            f" dl({blob.d_length})"
        )
