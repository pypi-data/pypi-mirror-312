...
from clickzetta.zettapark.mock._select_statement import MockSelectStatement
            ...
            for c, t in numerical_string_col_type_dict.items():
                ...
                if isinstance(t, StringType):
                    if name in ["mean", "stddev"]:
                        agg_cols.append(to_char(func(lit(0))).as_(c))
                    else:
...
{output_queries}"""
    ...
    def _resolve(self, col_name: str) -> Union[Expression, NamedExpression]:
        ...
        cols = list(
            filter(
                lambda attr: attr.name.upper() == normalized_col_name.upper(),
                self._output,
            )
        )
        if len(cols) == 1:
