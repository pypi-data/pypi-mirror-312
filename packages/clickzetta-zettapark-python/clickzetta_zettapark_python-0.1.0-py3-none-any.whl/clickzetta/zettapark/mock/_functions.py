...
def _register_func_implementation(
    zettapark_func: Union[str, Callable], func_implementation: Callable
):
    try:
        _MOCK_FUNCTION_IMPLEMENTATION_MAP[zettapark_func.__name__] = func_implementation
    except AttributeError:
        _MOCK_FUNCTION_IMPLEMENTATION_MAP[zettapark_func] = func_implementation


def _unregister_func_implementation(zettapark_func: Union[str, Callable]):
    try:
        try:
            del _MOCK_FUNCTION_IMPLEMENTATION_MAP[zettapark_func.__name__]
        except AttributeError:
            del _MOCK_FUNCTION_IMPLEMENTATION_MAP[zettapark_func]
    except KeyError:
        pass
    ...
    for data in e:
        ...
        try:
            ...
            if len_integer_part > precision:
                raise ZettaparkSQLException(f"Numeric value '{data}' is out of range")
    ...
    if isinstance(column.sf_type, StringType):
        ...
        def convert_str_to_bool(x: Optional[str]):
            if x is None:
                ...
                return False
            raise ZettaparkSQLException(f"Boolean value {x} is not recognized")
...
def mock_to_binary(
    ...
    if isinstance(column.sf_type.datatype, (StringType, NullType)):
        ...
        if fmt == "HEX":
            ...
            raise ZettaparkSQLException(f"Invalid binary format {fmt}")
...
def mock_parse_json(expr: ColumnEmulator):
    from clickzetta.zettapark.mock import CUSTOM_JSON_DECODER
