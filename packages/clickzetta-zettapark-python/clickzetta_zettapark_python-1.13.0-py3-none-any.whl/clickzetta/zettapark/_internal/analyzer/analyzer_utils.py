...
def in_expression(column: str, values: List[str], in_subquery: bool = False) -> str:
    if in_subquery and len(values) == 1:
        return column + IN + COMMA.join(values)
...
def range_statement(start: int, end: int, step: int, column_name: str) -> str:
    query = (
        ...
        + FROM
        + LEFT_PARENTHESIS
        + (SELECT + "EXPLODE" + LEFT_PARENTHESIS
           + "SEQUENCE" + LEFT_PARENTHESIS
           + f"{start}, {end}, {step}"
...
def math_unary_expression(child: str, sql_operator: str, operator_first: bool) -> str:
...
def varchar(length: Optional[int] = None) -> str:
    if length:
