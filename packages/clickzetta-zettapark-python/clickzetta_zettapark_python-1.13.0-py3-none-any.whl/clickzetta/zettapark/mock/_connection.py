...
import clickzetta.zettapark.mock._constants
...
from clickzetta.zettapark._internal.analyzer.snowflake_plan_node import SaveMode
...
from clickzetta.zettapark.mock._plan import MockExecutionPlan, execute_mock_plan
from clickzetta.zettapark.mock._snowflake_data_type import TableEmulator
...
class MockServerConnection:
    class TabularEntityRegistry:
        ...
        def write_table(
            ...
            if mode == SaveMode.APPEND:
                ...
                if name in self.table_registry:
                    raise ZettaparkSQLException(f"Table {name} already exists")
                else:
                    self.table_registry[name] = table
            else:
        ...
        def get_review(self, name: Union[str, Iterable[str]]) -> MockExecutionPlan:
            ...
            raise ZettaparkSQLException(f"View {name} does not exist")
    ...
    def __init__(self, options: Optional[Dict[str, Any]] = None) -> None:
        ...
        self._conn._session_parameters = {
            ...
            "_PYTHON_ZETTAPARK_USE_SQL_SIMPLIFIER_STRING": True,
        }
        self._options = options or {}
        self._active_account = self._options.get(
            "account", clickzetta.zettapark.mock._constants.CURRENT_ACCOUNT
        )
        self._active_warehouse = self._options.get(
            "warehouse", clickzetta.zettapark.mock._constants.CURRENT_WAREHOUSE
        )
        self._active_user = self._options.get(
            "user", clickzetta.zettapark.mock._constants.CURRENT_USER
        )
        self._active_database = self._options.get(
            "database", clickzetta.zettapark.mock._constants.CURRENT_DATABASE
        )
        self._active_role = self._options.get(
            "role", clickzetta.zettapark.mock._constants.CURRENT_ROLE
        )
        self._active_schema = self._options.get(
            "schema", clickzetta.zettapark.mock._constants.CURRENT_SCHEMA
        )
    ...
    def _get_current_parameter(self, param: str, quoted: bool = True) -> Optional[str]:
        try:
            ...
            return name.upper() if name is not None else name
            # return (
            #     (
        ...
        if isinstance(res, TableEmulator):
            ...
            for col in res.columns:
                if isinstance(
                    ...
                    from clickzetta.zettapark.mock import CUSTOM_JSON_ENCODER
            ...
            # when setting output rows, zettapark python running against snowflake don't escape double quotes
