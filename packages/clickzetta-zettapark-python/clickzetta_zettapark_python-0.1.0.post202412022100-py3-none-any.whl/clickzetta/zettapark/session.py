...
from clickzetta.zettapark.mock._connection import MockServerConnection
from clickzetta.zettapark.mock._pandas_util import (
    _convert_dataframe_to_table,
    _extract_schema_and_data_from_pandas_df,
)
from clickzetta.zettapark.mock._plan_builder import MockSnowflakePlanBuilder
...
"""
    ...
    def create_dataframe(
        ...
        if installed_pandas and isinstance(data, pandas.DataFrame):
            ...
            if isinstance(self._conn, MockServerConnection):
                ...
                try:
                    sf_schema = self._conn._get_current_parameter(
                        "schema", quoted=False
                    )
                except AttributeError:
    ...
    def _table_exists(self, raw_table_name: Iterable[str]):
        ...
        fqdn = ".".join(
            strip_double_quotes_in_like_statement_in_table_name(n)
            for n in raw_table_name
        )
        tables = self._run_query(f"show tables like '{fqdn}'")
