...
MULTI_QUOTED_EXPRESS = re.compile(r"(`\s*[^`]*\s*`\s*[\+\-\*/%]\s*)+`\s*[^`]*\s*`")


def quote_name(name: str) -> str:
    if ALREADY_QUOTED.match(name):
        return validate_quoted_name(name.lower())
...
def validate_quoted_name(name: str) -> str:
    if MULTI_QUOTED_EXPRESS.match(name):
        return name
    elif DOUBLE_QUOTE in name[1:-1].replace(DOUBLE_QUOTE + DOUBLE_QUOTE, EMPTY_STRING):
        raise ZettaparkClientExceptionMessages.PLAN_ANALYZER_INVALID_IDENTIFIER(name)
    else:
        return name


def escape_quotes(unescaped: str) -> str:
    return unescaped.replace('"', "").replace(DOUBLE_QUOTE, "")
