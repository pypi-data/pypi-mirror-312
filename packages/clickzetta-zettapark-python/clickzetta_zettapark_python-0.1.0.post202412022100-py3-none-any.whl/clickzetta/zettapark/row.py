...
class Row(tuple):
    ...
    def __getitem__(self, item: Union[int, str, slice]):
        if isinstance(item, int):
            ...
            if self._named_values:
                if f"`{item}`" in self._named_values:  # FIXME remove this workaround
    ...
    def __setattr__(self, key, value):
        ...
        if value is not None:
            if not self._case_sensitive:
                value = tuple(canonicalize_field(val) for val in value)
    ...
    def _populate_named_values_from_fields(self) -> None:
        ...
        if (
            ...
            self.__dict__["_named_values"] = {
                f"`{k.lower()}`": v for k, v in zip(self._fields, self)
            }
