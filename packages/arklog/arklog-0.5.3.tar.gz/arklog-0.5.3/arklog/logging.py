...
def set_path_logging(path: pathlib.Path = pathlib.Path("data/logger.toml")) -> None:
    """Load the logging settings from a toml file."""
    with path.open() as toml_file:
        config = toml.load(toml_file)
    add_logger_methods()
    logging.config.dictConfig(dict(config))


def set_config_logging(config: dict = None) -> None:
    """Create the default logging settings."""
...
def set_logging(argument: dict | pathlib.Path) -> None:
    """Create the default logging settings."""
    if isinstance(argument, pathlib.Path):
        return set_path_logging(argument)
    elif isinstance(argument, dict):
        return set_config_logging(argument)
    raise TypeError(f"Argument must be of type pathlib.Path or dict!")
