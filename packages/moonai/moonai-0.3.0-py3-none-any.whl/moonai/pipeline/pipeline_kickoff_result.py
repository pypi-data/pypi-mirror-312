...
class PipelineKickoffResult(BaseModel):
    ...
    def json(self) -> Optional[str]:
        if self.squads_outputs[-1].missions_output[-1].output_format != "json":
            raise ValueError(
                "No JSON output found in the final mission of the final squad. Please make sure to set the output_json property in the final mission in your squad."
            )
