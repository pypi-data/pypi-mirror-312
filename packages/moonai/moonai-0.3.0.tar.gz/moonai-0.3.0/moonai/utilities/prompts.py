...
class Prompts(BaseModel):
    ...
    def mission_execution(self) -> dict[str, str]:
        """Generate a standard prompt for mission execution."""
        slices = ["role_playing"]
        if len(self.tools) > 0:
            slices.append("tools")
        else:
        ...
        if (
            ...
            return {
                "system": system,
                "user": self._build_prompt(["mission"]),
    ...
    def _build_prompt(
        ...
        if not system_template and not prompt_template:
            ...
            prompt = prompt_template.replace(
                "{{ .Prompt }}", "".join(self.i18n.slice("mission"))
            )
