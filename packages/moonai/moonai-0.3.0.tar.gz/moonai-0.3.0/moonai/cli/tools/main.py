...
from moonai.cli.config import Settings
from moonai.cli.utils import (
...
class ToolCommand(BaseCommand, PlusAPIMixin):
    """
    A class to handle tool repository related operations for moonai projects.
    """
    ...
    def publish(self, is_public: bool, force: bool = False):
        ...
        console.print(
            f"Succesfully published {published_handle} ({project_version}).\nInstall it in other projects with moonai tool install {published_handle}",
            style="bold green",
        )
    ...
    def login(self):
        ...
        if login_response.status_code != 200:
            console.print(
                "Authentication failed. Verify access to the tool repository, or try `moonai login`. ",
                style="bold red",
            )
            raise SystemExit
