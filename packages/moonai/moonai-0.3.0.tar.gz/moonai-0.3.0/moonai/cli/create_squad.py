...
from moonai.cli.constants import ENV_VARS, MODELS
from moonai.cli.provider import (
    get_provider_data,
    select_model,
    select_provider,
)
from moonai.cli.utils import copy_template, load_env_vars, write_env_file
...
def create_folder_structure(name, parent_folder=None):
    ...
    click.secho(
        f"Creating {'squad' if parent_folder else 'folder'} {folder_name}...",
        fg="green",
        bold=True,
    )
...
def copy_template_files(folder_path, name, class_name, parent_folder):
    ...
    config_template_files = ["config/agents.yaml", "config/missions.yaml"]
    src_template_files = (
        ["__init__.py", "main.py", "squad.py"] if not parent_folder else ["squad.py"]
    )
...
def create_squad(name, provider=None, skip_provider=False, parent_folder=None):
