...
def method_calls_squad(method):
    """Check if the method calls `.squad()`."""
    try:
    ...
    class SquadCallVisitor(ast.NodeVisitor):
        def __init__(self):
            self.found = False

        def visit_Call(self, node):
            if isinstance(node.func, ast.Attribute):
                if node.func.attr == "squad":
                    self.found = True
            self.generic_visit(node)

    visitor = SquadCallVisitor()
    visitor.visit(tree)
...
def add_nodes_to_network(net, flow, node_positions, node_styles):
    ...
    for method_name, (x, y) in node_positions.items():
        ...
        elif method_calls_squad(method):
            node_style = node_styles["squad"]
        else:
