...
def build_pipelines(preprocessors: List[Preprocessor], detectors: List[BaseDetector], metrics: List[ProbaMetric]) -> List[EvaluationPipeline]:
    """ The given lists are assumed to be non-empty. """
    return [
        EvaluationPipeline(preprocessor=preprocessor, detector=detector, metrics=metrics)
        for preprocessor in preprocessors
        for detector in detectors
    ]


def convert_to_proba_metrics(metrics: List[Metric], thresholds: List[Thresholding]) -> List[ProbaMetric]:
    """ The given lists are assumed to be non-empty. """
    proba_metrics = []
    for metric in metrics:
        if isinstance(metric, BinaryMetric):
            proba_metrics.extend(ThresholdMetric(thresholder=threshold, metric=metric) for threshold in thresholds)
