...
/* harmony import */ var _utils_useRouter__WEBPACK_IMPORTED_MODULE_3__ = __webpack_require__(/*! ./utils/useRouter */ "./lib/utils/useRouter.js");
...
async function activate(app, palette, restorer, statusBar, router) {
    ...
    const router1 = new _utils_useRouter__WEBPACK_IMPORTED_MODULE_3__.RouterHook(router);
    console.log('Current1:', router1);
    // 提前处理 projectData 加载
    let projectData = {};
    try {
        const projectId = localStorage.getItem('projectId');
        projectData = await (0,_api_project__WEBPACK_IMPORTED_MODULE_4__.getProjectDetail)(projectId);
    }
    catch (error) {
        _jupyterlab_apputils__WEBPACK_IMPORTED_MODULE_1__.Notification.error('获取项目信息失败');
    }
    // 插件组件的实例化
    const timeWidget = new _widgets_time__WEBPACK_IMPORTED_MODULE_5__["default"]();
...
__webpack_require__.r(__webpack_exports__);
/* harmony export */ __webpack_require__.d(__webpack_exports__, {
/* harmony export */   RouterHook: () => (/* binding */ RouterHook)
/* harmony export */ });
...
class RouterHook {
    constructor(router) {
        this.router = router;
        this._routeChanged = new _lumino_signaling__WEBPACK_IMPORTED_MODULE_1__.Signal(this);
        // 初始化当前路由状态
    ...
    parseRoute(location) {
        ...
        const parsed = _jupyterlab_coreutils__WEBPACK_IMPORTED_MODULE_0__.URLExt.parse(request);
    ...
    // 获取指定参数值
    getParam(name) {
        return (this.currentRoute.params[name] || this.currentRoute.query[name] || null);
    }
    // 获取所有查询参数
    getQuery() {
    ...
    // 监听特定参数变化
    watchParam(paramName, callback) {
        const handler = (sender, route) => {
            const newValue = route.params[paramName] || route.query[paramName] || null;
            callback(newValue);
        };
        this._routeChanged.connect(handler);
        return {
...
//# sourceMappingURL=lib_index_js.9ef9123cfd77895cd4d6.js.map