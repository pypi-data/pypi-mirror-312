...
def find_haplotypes(G, method='min', verbose=False):
    """Find haplotypes by finding all simple paths and selecting those with weights.
...
def main(bam_file, snps_file, plot, output, verbose):
    ...
    with console.status("[bold green]Processing..." if not verbose else "[bold green]Starting haplotype analysis...") as status:
        ...
        haplotypes = find_haplotypes(G, verbose=verbose)
