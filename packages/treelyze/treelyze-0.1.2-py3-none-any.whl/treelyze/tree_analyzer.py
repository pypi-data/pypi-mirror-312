...
from .file_summarizer import summarize_file, should_exclude, summarize_multiple_summaries
import json
...
def print_tree(
    ...
    # If summarization is enabled, perform the second phase with the complete tree
    if summarize:
        write_output("\n=== Project-wide Analysis ===")
        tree_structure = "\n".join(tree_output)
        meta_summary = json.loads(summarize_multiple_summaries(tree_structure, model, prompt))
        
        write_output("\nProject Summary:")
        write_output(meta_summary["project_summary"])
        
        if meta_summary.get("error"):
            write_output("\nError during analysis:")
            write_output(meta_summary["error"])
