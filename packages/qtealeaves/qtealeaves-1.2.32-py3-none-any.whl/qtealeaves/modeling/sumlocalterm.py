    ...
    **Attributes**
    ...
    def _get_coupl_func(self, dim, params):
        ...
        if self.coupling_entries is None:
            ...
            def get_coupl(*args, dim=dim, ctens=ctens):
                ...
                raise Exception("No match for dimension etc.")
    ...
    def get_interactions(self, ll, params, **kwargs):
        ...
        if "dim" not in kwargs:
            raise Exception("SumLocalLindblad term needs dim information")
        dim = kwargs["dim"]
    ...
    def quantum_jump_apply(self, state, operators, params, rand_generator, **kwargs):
        ...
        **Arguments**
        ...
        if operators[self.operator[0]].has_symmetry:
            ...
            raise Exception("Symmetric Lindblad operator not supported.")
        ...
        # remind that sparse MPO matrix have the form
        #       ( 1 0 )     ( 1 0 ) ( 1 )
        # (L 1) ( L 1 ) ... ( L 1 ) ( L )
        ...
        for idx, ii in enumerate(not_masked_sites):
            ...
            if idx == 0:
                ...
                op_left.set_subtensor_entry(
                    [0, 0, 0, 0],
                    [1, local_dim, local_dim, 1],
                    operators[self.operator[0]] * cvec[ii],
                )
                op_left.set_subtensor_entry(
                    [0, 0, 0, 1], [1, local_dim, local_dim, 2], operators[(ii, "id")]
                )
                ...
                op_right.set_subtensor_entry(
                    [1, 0, 0, 0],
                    [2, local_dim, local_dim, 1],
                    operators[self.operator[0]] * cvec[ii],
                )
                op_right.set_subtensor_entry(
                    [0, 0, 0, 0], [1, local_dim, local_dim, 1], operators[(ii, "id")]
                )
                # pylint: disable-next=protected-access
                ...
                op_bulk.set_subtensor_entry(
                    [1, 0, 0, 1], [2, local_dim, local_dim, 2], operators[(ii, "id")]
                )
                # pylint: disable-next=protected-access
