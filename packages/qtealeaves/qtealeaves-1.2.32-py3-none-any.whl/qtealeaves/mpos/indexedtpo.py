...
class ITPO(_AbstractEffectiveOperators):
    ...
    * iucTPO : iTPO with compression and smart update for time-dependent parameters.
    ...
    def setup_as_eff_ops(self, tensor_network, measurement_mode=False):
        ...
        # Can change in future, pending change to guarantee iso_center is tuple in TTN
        if hasattr(tensor_network.iso_center, "__len__"):
            if tuple(tensor_network.iso_center) != tuple(
                tensor_network.default_iso_pos
            ):
                # If iso center exist, shifting iso is the correct way to go
                tensor_network.iso_towards(tensor_network.default_iso_pos)
        elif tensor_network.iso_center != tensor_network.default_iso_pos:
            tensor_network.iso_towards(tensor_network.default_iso_pos)
