<template>
  <div class="pricing-step bg-white w-full border border-transparent rounded-md">
    ...
    >
        ...
        <div class="pricing-step-content-element-wrap flex w-4/12 py-[0.5rem] pl-[0.75rem]">
          <div class="pricing-step-content-element-el flex w-full justify-start items-center">
            <div class="input-wrap flex pr-[0.75rem]">
              ...
              <InputField
                v-else
                :model-value="removeTrailingZeros(upliftPricing!.fuel_pricing?.client?.unit_price_amount)"
                class="w-6/12 mb-0"
                :is-white="true"
                :is-half="true"
                placeholder=" "
          ...
          >
            ...
            <InputField
              v-else
              :model-value="formatNumber(upliftPricing!.fuel_pricing?.client?.amount_total)"
              class="roi-input w-full"
              :is-white="true"
              placeholder=" "
              :disabled="true"
            >
              <template #suffix>{{
                upliftPricing!.fuel_pricing?.client?.amount_currency?.code
              }}</template>
            </InputField>
          </div>
        </div>
      ...
      <div
        v-if="
          (orderPricing && orderPricing.fees?.length > 0) ||
          (multipleUplifts && upliftPricing && upliftPricing.fees?.length > 0)
        "
      ...
      <div
        v-for="(fee, key) in multipleUplifts ? upliftPricing!.fees : orderPricing.fees"
        v-if="
          (orderPricing && orderPricing.fees.length > 0) ||
          (multipleUplifts && upliftPricing && upliftPricing.fees?.length > 0)
        "
...
import type { IFuelPricingObj, IOrder, IUpliftFuelPricing } from 'shared/types';
...
const orderPricing: Ref<IFuelPricingObj | IUpliftFuelPricing | null> = computed(() =>
  props.multipleUplifts ? props.upliftPricing : orderReferenceStore.orderPricing
);
...
const onPriceChange = useDebounceFn(async (value: any) => {
  if (props.multipleUplifts) {
    await onUpdateUpliftOrderPricing(props.upliftId, value, props.upliftPricing!, true);
  } else {
