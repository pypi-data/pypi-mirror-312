...
class FovusS3Adapter:
    ...
    def live_tail_file(self):
        job_current_status = self.fovus_api_adapter.get_job_current_status(self.cli_dict[JOB_ID])
        if job_current_status != "Running":
            Util.print_error_message(
                f"Job {self.cli_dict[JOB_ID]} is not running. Please check your job ID and try again."
            )
            return
        file_path = self.cli_dict[LIVE_TAIL_FILE]
        key_content = self._download_ssh_key_content()
        list_runs = self.fovus_api_adapter.get_list_runs(
            self.fovus_api_adapter.get_list_runs_request(
                workspace_id=self.fovus_api_adapter.workspace_id, job_id=self.cli_dict[JOB_ID]
            )
        )
        ...
        for run in list_runs["runList"]:
            if run["taskName"] == task_name and len(run["computeNodeDnsList"]) != 0:
                ...
                self._live_tail_file(key_content, compute_node_dns, file_path)
    ...
    def _download_ssh_key_content(self):
        s3_info = self._get_download_s3_info()
        response = s3_info["s3_client"].get_object(
            Bucket=s3_info["s3_bucket"],
            Key=f"ssh-keys/{self.fovus_api_adapter.workspace_id}-{self.fovus_api_adapter.user_id}.pem",
        )
        key_content = response["Body"].read().decode("utf-8")
        return key_content

    def _live_tail_file(self, private_key_content: str, hostname: str, file_path: str):
        print("Establishing connection...")
        pkey = paramiko.RSAKey.from_private_key(io.StringIO(private_key_content))
        ssh = paramiko.SSHClient()
        ssh.set_missing_host_key_policy(paramiko.AutoAddPolicy())
        ssh.connect(hostname, port=22, username=self.cli_dict[JOB_ID], pkey=pkey)
