...
class Leaf:
    ...
    def leafy_icsd(self, file='../data/icsd_compositions.csv'):
        ...
        icsd.to_pickle('../data/icsd_unique_struct_leaf+.pickle')
        return icsd
    ...
    def best_icsd_similarity(self, top=1, drop_self_duplicates=True, unique_structure_type=False):
        ...
        if os.path.isfile('../data/icsd_unique_struct_leaf+.pickle'):
            icsd = pd.read_pickle('../data/icsd_unique_struct_leaf+.pickle')
        else:
...
class CreateLeaf:
    ...
    def average_features_cifs(self, list_cifs, mode='dir'):
        """ process CIFs
        ...
        if mode == 'list':
            ...
            raise ValueError('average_features_cifs: wrong value for "mode", ("dir", "list") are allowed')
        ...
        print(f'Computed LEAFs for {ndf.columns}') 
...
if __name__=="__main__":
    ...
    newleaf = CreateLeaf()
    leafs_test = newleaf.average_features_cifs(fname, mode='dir')
