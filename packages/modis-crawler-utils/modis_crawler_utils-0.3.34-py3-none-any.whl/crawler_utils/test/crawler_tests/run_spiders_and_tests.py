...
from crawler_utils.test.crawler_tests.func.utils import  get_args, get_args_by_parser_names,setup_logger, timing_decorator
...
def run_spider(crawler_process: CrawlerProcess, spider_name: str, spider_arg: List[str], tmp_dir: Path,
        ...
        new_settings = configure_crawler_settings(parser_name, tmp_dir, mock_mode_enabled)
...
def run_crawler_process(spider_args_list: List[List[str]], tmp_dir: Path, mock_mode_enabled: bool):
    ...
    for spider_args in spider_args_list:
        spider_name = (SPIDER_NAMES.get('search')
                       if 'search' in spider_args[PARSER_NAME_INDEX]
                       else SPIDER_NAMES.get('target'))

        run_spider(process, spider_name, spider_args, tmp_dir, mock_mode_enabled)
...
def run_tests(script_args, tmp_dir: Path):
    """
     Executes pytest tests  based on the script arguments.

     Args:
         script_args: Script arguments, parsed by the argument parser.
         tmp_dir (Path): Path to the temporary directory used for testing.

     """
    custom_args = []

    if script_args.parser_names:
        or_string = ' or '.join(script_args.parser_names)
        custom_args.append(f"-k {or_string}")

    if script_args.mock_mode:
...
def main():
    ...
    Args:
        args: Parsed command-line arguments  (--mock-mode, --save-tmp).
    ...
    with tempfile.TemporaryDirectory() as output_dir:
        ...
        crawling_args = (get_args_by_parser_names(args.parser_names, args.mock_mode) if args.parser_names else get_args(args.mock_mode))
        run_crawler_process(crawling_args, tmp_dir, args.mock_mode)

        if args.save_tmp:
            output_path = Path(sys.path[-1]) if args.output == './' else Path(args.output)
