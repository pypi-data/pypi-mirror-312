...
class Nomad_SDK:
    ...
    def __init__(self, config) -> None:
        if not hasattr(self, "initialized"):
            ...
            # if user sets token, refresh_token, expiration_seconds, and id, then no need to login
            if not self.token and not self.refresh_token_val and not self.expiration_seconds and not self.id:
                self.login()
