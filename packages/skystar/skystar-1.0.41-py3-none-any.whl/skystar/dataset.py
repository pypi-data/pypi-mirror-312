...
try:
    ...
    import skystar.sky_dataset.load as ssl
except ImportError:
...
class selfdata_starsky(Dataset):
    ...
    def prepare(self):
        if self.split:
            x_train, t_train, x_test, t_test = ssl.load(self.filename,split=self.split)
            if self.training:
                self.data, self.label = x_train, t_train
            else:
                self.data, self.label = x_test, t_test
        else:
            x, t = ssl.load(self.filename,split=self.split)
            return x,t
