...
model, tokenizer = Finetuning_functions.load_model_with_lora(model_name)
...
for filename in os.listdir(data_path):
    if filename.endswith(".json") and "_questions_" in filename:
        ...
        data_chocolatine = Finetuning_functions.reformat_for_chocolatine(data, system_message=f"Contexte : {theme}")
        train_dataset_on_chunk, eval_dataset_on_chunk = Finetuning_functions.create_datasets(data_chocolatine)
...
tokenized_train_dataset = train_dataset.map(Finetuning_functions.preprocess_instruction, batched=True, fn_kwargs={"tokenizer": tokenizer})
tokenized_eval_dataset = eval_dataset.map(Finetuning_functions.preprocess_instruction, batched=True, fn_kwargs={"tokenizer": tokenizer})
...
training_args = TrainingArguments(
    ...
    num_train_epochs=2,  # Adjust epochs here
    learning_rate=5e-5,
    fp16=True,
    logging_dir="./logs",
    save_total_limit=1,
