...
class iac3:
    ...
    def sfr_test(self, far_input_data, near_input_data, width, height, raw_path, raw_type, result_log, ny = 4):
        ...
        # 输入标准值
        self.sfr_dll.inputData(near_input_data[0], near_input_data[1], near_input_data[2], near_input_data[3], far_input_data[0],
                      far_input_data[1], far_input_data[2], far_input_data[3])

        # logger.info(f"输入标准值：({self._config['size'][0]}, {self._config['size'][1]})")

        logger.info(
            f"输入标准值：({self._config['size'][0]}, {self._config['size'][1]}, {self._config['size'][3]})")  ##['size'][3]中“4”代表在刀口4分之一出找

        # 输入裁剪的分辨率
        self.sfr_dll.inputResolution(width, height, ny)
        logger.info(f'生成raw_path， log_path， rtype')
        rraw_path = ctypes.create_string_buffer(raw_path.encode('gbk'))
        log_path = ctypes.create_string_buffer(result_log.encode('gbk'))
        rtype = ctypes.create_string_buffer(raw_type.encode('gbk'))
        logger.info('获取sfr结果')
        ...
        result =  self.sfr_dll.getIAC3RawPathC(rraw_path, byref(dataList), byref(count), log_path, rtype)
