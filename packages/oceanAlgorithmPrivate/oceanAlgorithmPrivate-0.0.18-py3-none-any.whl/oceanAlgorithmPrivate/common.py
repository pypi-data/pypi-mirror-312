...
class Private(LoggerHelper):
    ...
    def __new__(cls, *args, **kwargs):
        if cls._instance is None:
            ...
            print("New Private instance created")
        return cls._instance
    
    def __init__(self, log_level='DEBUG'):
        print("Entering Private.__init__")
        if not hasattr(self, 'initialized') or not self.initialized:
            super().__init__(log_level=log_level)
            print("Private class initialized.")
            ...
            self.logger.debug("Private attributes initialized.")
        else:
            print("Private already initialized.")
    ...
    def read_data(self, file_path, json_data, schema_json, variable):
    ...
    def save_results(self, variable, eval_report, roc_fig):
        target_name = variable['dep_var']
        if roc_fig: 
            self.upload_file([eval_report, roc_fig], var_names=['report', 'fig'], target_name=target_name)
            print(f"{self.base_filename}_{target_name}_fig.png")
        else:
            self.upload_file([eval_report], var_names=['report'], target_name=target_name)
