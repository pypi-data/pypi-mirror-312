...
from orangecontrib.darfix.utils.refactoring import (
    OWDarfixWidgetOneThread,
    fromOWSendDatasetToDataset,
)
...
class BinningWidgetOW(OWDarfixWidgetOneThread, ewokstaskclass=Binning):
    """
    ...
    def setDataset(
        ...
        if not isinstance(dataset, dtypes.OWDataset):
            raise TypeError(
                f"dataset is expected to be an instance of OWDataset. Got {type(dataset)}."
            )
    ...
    def task_output_changed(self):
        ...
        dataset = fromOWSendDatasetToDataset(self.get_task_output_value("dataset"))
