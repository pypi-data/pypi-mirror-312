...
def test_EDFDataSelectionWidgetOW(
    ...
    # note: warning: the output type is OWSendDataset and not a dtypes.Dataset because
    # of the backward compatibility
    assert isinstance(dataset, dtypes.OWSendDataset), f"type is {type(dataset)}"
    data = dataset[0].dataset.data
    bg_dataset = dataset[0].bg_dataset
    ...
    if processed_data_dir:
        processing_dir = dataset[0].dataset.dir
        assert processing_dir == os.path.join(processed_data_dir, "treated")

    title = dataset[0].dataset.title
