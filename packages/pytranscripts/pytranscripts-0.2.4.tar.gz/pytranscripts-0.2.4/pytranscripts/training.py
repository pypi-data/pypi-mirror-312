...
class TranscriptTrainer:
    def __init__(
        ...
        top_levels: Optional[Dict[str, List[str]]] = None
    ):
        """
        ...
        Args:
            ...
            top_levels (Dict[str, List[str]], optional): Dictionary mapping top-level categories to their subcategories
        """
        # Default Labels
        ...
        self.TOP_LEVELS = top_levels or {
            "Multi-level organizations Characteristics": [
                "Provider Characteristics", 
                "Health System Characteristics"
            ],
            "Multi-level organizations Perspectives/Values": [
                "Imaging modalities in general",
                'Value equation',
                "Clinical utility & efficiency-Provider perspective",
                "Patient/Physican interaction in LUS",
        ...
        self.CATEGORIES = {}
        for top_level, subcategories in self.TOP_LEVELS.items():
        ...
        logger.success("DATASET LABELS AND IDS PREPARED SUCCESSFULLY")
    ...
    def _load_and_prepare_data(self):
        ...
        self.train_dataset = Dataset.from_dict({
            'text': self.train_data.full_quote.values.tolist(),
            'label': self.train_data.labels.tolist()
        })
        self.eval_dataset = Dataset.from_dict({
            'text': self.eval_data.full_quote.values.tolist(),
    ...
    def _compute_metrics(self, eval_pred):
        ...
        logger.info("COMPUTE METRIC LOADED")

    def train_distilbert(self):
        """Train DistilBERT model"""
        os.makedirs(os.path.join(self.destination_path, 'interview_classifier'), exist_ok=True)
        ...
        logger.info("BERT METRICS LOADED SUCCESSFULLY")

        trainer.train()
        logger.info("BERT TRAINING FINISHED SUCCESSFULLY")
        return trainer

    def train_electra(self):
        """Train Electra model"""
        os.makedirs(os.path.join(self.destination_path, 'interview_electra'), exist_ok=True)
    ...
    def classify_using_model(self, model_path, input_dataframe):
        """
        ...
        for index, row in result_sheet.iterrows():
            if row[self.LABELS].values.sum() > 0:
                prediction = self._classify_text(row['full_quote'], model_pipeline)
                feature = prediction[0]['label']

                result_sheet.at[index, feature] = 1

                # Add top-level category columns
                for top_level, categories in self.TOP_LEVELS.items():
    ...
    def train_and_classify(self):
        """
        Train both models and run classification
        
        Returns:
            tuple: Trained DistilBERT and Electra models
        """
        # Train models
        ...
        bert_sheet = self.classify_using_model(bert_checkpoint, self.full_data)
        electra_sheet = self.classify_using_model(electra_checkpoint, self.full_data)

        # Save results
        bert_sheet.to_excel(
            os.path.join(self.destination_path, 'SplitBert.xlsx'), 
            index=False
        )
        electra_sheet.to_excel(
            os.path.join(self.destination_path, 'SplitElectra.xlsx'), 
            index=False
        )
...
if __name__ == "__main__":
    ...
    # # Example with custom labels and top levels
    # custom_labels = [
    #     'Label1', 'Label2', 'Label3', 'Label4'
    # ]

    # custom_top_levels = {
    #     "Category A": ['Label1', 'Label2'],
    #     "Category B": ['Label3', 'Label4']
    # }

    # custom_trainer = TranscriptTrainer(
    #     input_file='CompletedMerged.xlsx',
    #     destination_path='results/',
    #     labels=custom_labels,
    #     top_levels=custom_top_levels,
    #     max_length=512,
    #     num_train_epochs=20
    # )

    # # Train models and classify with custom configuration
    # custom_bert_model, custom_electra_model = custom_trainer.train_and_classify()