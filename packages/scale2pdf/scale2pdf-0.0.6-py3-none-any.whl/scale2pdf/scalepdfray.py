...
@ray.remote
def scalablepdf(pdf_path, extract_images=True, output_folder=None):
    pdf_name = os.path.splitext(os.path.basename(pdf_path))[0]
    sanitized_name = sanitize_filename(pdf_name)
    output_folder = output_folder or os.path.join(os.getcwd(), sanitized_name)
...
def process_pdfs(input_path, extract_images=True, output_folder=None):
    if os.path.isfile(input_path):
        scalablepdf(input_path, extract_images, output_folder)
    elif os.path.isdir(input_path):
        pdf_files = [os.path.join(input_path, f) for f in os.listdir(input_path) if f.endswith('.pdf')]
        futures = [scalablepdf.remote(pdf_path, extract_images, output_folder) for pdf_path in pdf_files]
        ray.get(futures)
    else:
        print(f"Invalid input path: {input_path}")

def scalable_ray(input_path, extract_images=True, num_cpus=4, output_folder=None):
    ray.init(num_cpus=num_cpus, ignore_reinit_error=True)
    process_pdfs(input_path, extract_images, output_folder)
