...
def get_path_parameters_from_ref(ref: str) -> dict:
    """Retrieve path parameters from a reference."""
    ref_parts = ref.split('/')
    param_name = ref_parts[-1]
    param = swagger_settings.get_component(ref)
    return param
    ...
    async def wrapper(*args, **kwargs):
        ...
        for path in paths_info:
            for param in path['parameters']:
                ...
                # Handle parameters that are references to other components
                elif '$ref' in param:
                    ref_param = get_path_parameters_from_ref(param['$ref'])
                    if ref_param and ref_param.get('in') == 'path':
                        if ref_param.get('schema') is not None:  # Check if schema exists
                            path_params[ref_param['name']] = ref_param['schema'].get('enum', [])
