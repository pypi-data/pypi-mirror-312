...
def load(
    ...
    if spdf:
        ...
        # At this point, the zip files are downloaded but they are in
        # the wrong directory. We need to move them to a directory containing the month
        files_zip = []
        for f in files_zip_spdf:
            stem = Path(f).stem
            if len(stem) >= 10:
                # Stem is similar to 'SECS20120301'
                month = stem[8:10]
        ...
        # SPDF does not contain and .gz files
        files_gz = []
    else:
        files_zip = download(
