...
for n = 1 : length(bgFiles)
  ...
  load([output_path, fgFiles{n}, '_dwellTime.mat']);
  longFrame = ceil(dwellTimeData(1) / (ExposureTime / 1000)) + 1;
  soxStack = tiffread([input_path, fgFiles{n}, '.tif']);
  stack = tiffread([input_path, bgFiles{n}, '.tif']);
  for m = 1 : length(trackedPar)
    coord = trackedPar(m).xy ./ impars.PixelSize;
    coord = round(coord);
    dapiI = zeros(size(coord, 1), 1); soxI = zeros(size(coord, 1), 1);
    for p = 1 : size(coord, 1)
      dapiRaw = stack(trackedPar(m).Frame(p)).data;
      soxRaw = soxStack(trackedPar(m).Frame(p)).data;
      dapiI(p) = dapiRaw(coord(p, 2), coord(p, 1));
      soxI(p) = soxRaw(coord(p, 2), coord(p, 1));
    end
    trackedPar(m).dapiIntensity = dapiI;
    trackedPar(m).soxIntensity = soxI;
  end
  figure;
  [X, Y] = meshgrid(1:512);
  mesh(X, Y, zeros(512, 512), dapiRaw); view(0, 90);
  hold on;
  for m = 1 : length(trackedPar)
    if size(trackedPar(m).xy, 1) > 2 %longFrame
      plot(trackedPar(m).xy(1, 1)/ impars.PixelSize, trackedPar(m).xy(1, 2)/impars.PixelSize, 'rx');
    else
      plot(trackedPar(m).xy(1, 1)/ impars.PixelSize, trackedPar(m).xy(1, 2)/impars.PixelSize , 'bx');
    end
  end
