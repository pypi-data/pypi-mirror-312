function [tlist, rlist, JDH] = calculateJDH(tracks, nJumps,bin_edges,timeStep, clip_factor, analysis_type, NormalizeFlag, PlotFlag)
    ...
    if  TrackLength(i) > j
      ...
      if analysis_type == "percentage"
        ...
        if length(idx) < clip_factor
          JumpsToConsider = length(idx);
        else
          JumpsToConsider = clip_factor;
        end
      end
      ...
      jd_temp = sqrt((jd_temp_x(j+1:FrameToStop)- jd_temp_x(1:FrameToStop-j)).^2 + ...
        (jd_temp_y(j+1:FrameToStop)- jd_temp_y(1:FrameToStop-j)).^2);
      jd =  cat(1,jd,jd_temp);
