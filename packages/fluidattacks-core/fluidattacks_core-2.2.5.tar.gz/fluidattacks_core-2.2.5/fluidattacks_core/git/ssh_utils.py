...
async def _execute_git_command(
    ssh_file_name: str, raw_root_url: str, branch: str
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        try:
            stdout, stderr, return_code = await _execute_git_command(
                ssh_file_name, raw_root_url, branch
            )
            if return_code == 0:
                last_commit = stdout.decode().split("\t")[0]
            else:
                LOGGER.error(
...
async def call_ssh_ls_remote(
    repo_url: str, credential_key: str, branch: str
) -> str | None:
