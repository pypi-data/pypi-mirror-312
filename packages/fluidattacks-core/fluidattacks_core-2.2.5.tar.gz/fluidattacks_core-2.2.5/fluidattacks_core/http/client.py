...
def get_secure_connector(
    ...
    class SecureTCPConnector(aiohttp.TCPConnector):
        async def _resolve_host(
            ...
            resolved_ips = [
                ipaddress.ip_address(host["host"]) for host in hosts
            ]
            validate_local_request(
                resolved_ips,
...
async def request(
    ...
    *,
    ...
    connector = (
        get_secure_connector(allow_local_network, allow_localhost)
        if dns_rebind_protection
        else aiohttp.TCPConnector
    )
    connection = connector(
        ssl=ssl.create_default_context(cafile=certifi.where()),  # NOSONAR
    )
