...
class ImageClassModelWrapper(ClassModelWrapper):
	...
	Examples
	--------
	>>> model_wrapper = ImageClassModelWrapper(model, classes=classes)
	...
	def train(self, data: Union[str, Path], imgsz: Union[int, tuple, list] = None,
		...
		self.classes = self.classes or train_set.classes
	...
	def evaluate(self, data: Union[str, Path, ImageFolder] = None,
...
class SplitImageClassModelWrapper(ImageClassModelWrapper):
	"""
	根文件下是分类文件夹
	
	Examples
	--------
	>>> model_wrapper = SplitImageClassModelWrapper(model, classes=classes)
	>>> model_wrapper.train(train_texts, y_train val_data, collate_fn)
	>>> model_wrapper.predict(test_texts)
	>>> model_wrapper.evaluate(test_texts, y_test)
	0.9876
	...
	def train(self, data: Union[str, Path, ImageFolder], val_size=0.2, imgsz: Union[int, tuple, list] = None,
	          transform=None, train_transform=None,
	          epochs=100,
	          optimizer: Union[type, optim.Optimizer] = None, scheduler: LRScheduler = None,
	          lr=0.001, T_max: int = 0,
		...
		if val_size > 0.0:
			...
			if isinstance(data, (str, Path)):
				...
				dataset = data
				self.transform = transform or dataset.transform or get_train_transform(imgsz)
			else:
				raise TypeError(f'Expected str or Path or ImageFolder, but {type(data)}')
				
			self.classes = self.classes or dataset.classes
			train_size = 1 - val_size
			# (PIL.Image.Image, int)
			train_set, val_set = random_split(dataset, [train_size, val_size])
			# (torch.Tensor, int)
			train_set = ImageDataset(train_set, train_transform)
			val_set = ImageDataset(val_set, self.transform)
		else:
			val_set = None
			if isinstance(data, (str, Path)):
				...
				train_set = data
				train_set.transform = train_transform or train_set.transform or transform or get_train_transform(imgsz)
			self.classes = self.classes or train_set.classes
			
		log_utils.info(f"Classes: {self.classes}")
		
		# 调用的是ImageClassModelWrapper父类ClassModelWrapper的train方法
		super(ImageClassModelWrapper, self).train(train_set, val_set,
		                                     epochs=epochs, optimizer=optimizer, scheduler=scheduler, lr=lr,
		                                     T_max=T_max, batch_size=batch_size, eval_batch_size=eval_batch_size,
		                                     num_workers=num_workers, num_eval_workers=num_eval_workers,
		                                     pin_memory=pin_memory, pin_memory_device=pin_memory_device,
	...
	def evaluate(self, data: Union[str, Path, ImageFolder],
	             imgsz: Union[int, tuple, list] = None,
	             transform=None,
	             batch_size: int = 64,
	             num_workers: int = 0) -> float:
