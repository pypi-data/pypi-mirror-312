...
class Lab:
    ...
    def get_pdf(self):
        ...
        lab_tasks_path = os.path.join(mathwork_path, 'Lab Tasks')

        # Check if the Lab Tasks folder exists
        if not os.path.exists(lab_tasks_path):
            return f"The 'Lab Tasks' folder does not exist in {mathwork_path}"

        # Search for the PDF file that matches the lab number and name
        for filename in os.listdir(lab_tasks_path):
            if filename.startswith(f'CV Lab-{self.lab_number}') and filename.endswith('.pdf'):
