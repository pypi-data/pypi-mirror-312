...
class TextInputManager:
    ...
    def selection_value(self):
        if self.selection_cursor_start is None or self.selection_cursor_end is None:
            return ""
        return (self.left + self.right)[self.selection_cursor_start:self.selection_cursor_end]
    ...
    def _process_shift_left(self):
        ...
        if self.selection_cursor_start is None:
            ...
            # Si on revient sur une sélection faite vers la droite
            if self.cursor_pos > self.selection_cursor_start:
                self.cursor_pos -= 1
                self.selection_cursor_end = self.cursor_pos
                # Si le curseur repasse au même niveau que le début de sélection
                if self.cursor_pos == self.selection_cursor_start:
                    self.selection_cursor_start = None
                    self.selection_cursor_end = None
            else:
                # Extension de la sélection vers la gauche
                self.cursor_pos -= 1
                self.selection_cursor_start = self.cursor_pos
    ...
    def _process_left(self):
        # if len(self.left)>1 and self.left[-1] == "\n":
        #     self.cursor_pos -= 2
        # else:
        self.cursor_pos -= 1
...
class TextInputVisualizer:
    ...
    def _rerender(self):
        ...
        if self.manager.selection_value() != "":
            ...
            selection_start = self.manager.selection_cursor_start
...
class MultilineTextLineInput(MultilineTextInputVisualizer):
    ...
    def __init__(self, parent: [Boite, LineEdit],
        ...
        super().__init__(self.manager, self.parent.width - 6, self.parent.height - 6, self.parent.font_object, antialias, font_color,
