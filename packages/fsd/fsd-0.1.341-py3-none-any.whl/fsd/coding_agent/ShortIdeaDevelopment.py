...
class ShortIdeaDevelopment:
    ...
    def initial_setup(self, role, crawl_logs, context, file_attachments, assets_link):
        ...
        system_prompt = (
            ...
            "only return section that need for user request, do not return non relavent section, if dependency need to be installed always inlcude \"MUST INSTALL DEPEDENCY\" , if there is image in this only format need to be newly generated 'PNG, png, JPG, jpg, JPEG, jpeg, and ico' then always include \"MUST GENERATE IMAGE\", if any file, non-assets file but include svg that need to be moved or newly created, always inlcude \"MUST NEED FILE MANAGER\", everything in the end, the last sentence, start with **hiding from users: \"MUST INSTALL DEPEDENCY\" ,  \"MUST GENERATE IMAGE\", \"MUST NEED FILE MANAGER\""
        )
