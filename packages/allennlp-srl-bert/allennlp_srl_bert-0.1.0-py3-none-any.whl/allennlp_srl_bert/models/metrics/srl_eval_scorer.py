...
from allennlp_srl_bert.common.util import is_distributed
from allennlp_srl_bert.common.checks import ConfigurationError
from allennlp_srl_bert.training.metrics.metric import Metric

logger = logging.getLogger(__name__)

DEFAULT_SRL_EVAL_PATH = os.path.abspath(
    os.path.join(
        os.path.dirname(os.path.realpath(__file__)), "..", "tools", "srl-eval.pl"
    )
)
    ...
    def __call__(
        ...
        with open(predicted_path, "w", encoding="utf-8") as predicted_file, open(
            ...
            for verb_index, sentence, predicted_tag_sequence, gold_tag_sequence in zip(
                ...
                from allennlp_srl_bert.structured_prediction.models.srl import (
