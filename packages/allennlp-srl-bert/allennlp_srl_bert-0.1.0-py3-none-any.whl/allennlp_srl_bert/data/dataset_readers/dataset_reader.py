...
from allennlp_srl_bert.common import util
...
class DatasetReader(Registrable):
    ...
    def text_to_instance(self, *inputs) -> Instance:
        ...
        :class:`~srl_bert.predictors.predictor.Predictor`, which gets text input as a JSON
    ...
    def shard_iterable(self, iterable: Iterable[_T]) -> Iterator[_T]:
        ...
        """
        if (
            not self.manual_distributed_sharding
            or not self.manual_multiprocess_sharding
        ):
            raise ValueError(
    ...
    def _multi_worker_islice(
        ...
        if self._distributed_info is not None:
            if max_instances is not None:
                ...
                if self._distributed_info.global_rank < (
                    ...
                    max_instances = (
                        max_instances // self._distributed_info.world_size + 1
                    )
                else:
        ...
        if self._worker_info is not None:
            if max_instances is not None:
                ...
                if self._worker_info.id < (
                    max_instances % self._worker_info.num_workers
                ):
