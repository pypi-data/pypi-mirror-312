...
import logging
import torch
from allennlp_srl_bert.common.util import pad_sequence_to_length


from allennlp_srl_bert.data.vocabulary import Vocabulary
from allennlp_srl_bert.data.tokenizers import Token, PretrainedTransformerTokenizer
...
class PretrainedTransformerIndexer(TokenIndexer):
    ...
    def _postprocess_output(self, output: IndexedTokenList) -> IndexedTokenList:
        ...
        if self._max_length is not None:
            ...
            type_ids = type_ids[
                self._num_added_start_tokens : len(type_ids)
            ...
            assert all(
                ...
                for segment_indices, segment_type_ids in zip(
                    folded_indices, folded_type_ids
                )
            )
