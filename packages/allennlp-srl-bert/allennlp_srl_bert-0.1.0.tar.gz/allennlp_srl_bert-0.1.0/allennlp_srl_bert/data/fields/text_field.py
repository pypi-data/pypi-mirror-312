...
from allennlp_srl_bert.data.fields.sequence_field import SequenceField
from allennlp_srl_bert.data.tokenizers import Token
from allennlp_srl_bert.data.token_indexers.token_indexer import (
    TokenIndexer,
    IndexedTokenList,
)
...
class TextField(SequenceField[TextFieldTensors]):
    ...
    def empty_field(self):
        ...
        if self._token_indexers is not None:
            for indexer_name, indexer in self.token_indexers.items():
                text_field._indexed_tokens[indexer_name] = (
    ...
    def duplicate(self):
        ...
        if self._token_indexers is not None:
            new = TextField(
                deepcopy(self.tokens), {k: v for k, v in self._token_indexers.items()}
            )
        else:
