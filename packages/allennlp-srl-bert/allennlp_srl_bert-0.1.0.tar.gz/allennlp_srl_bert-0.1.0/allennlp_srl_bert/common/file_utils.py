...
CACHE_ROOT = Path(os.getenv("ALLENNLP_CACHE_ROOT", Path.home() / ".allennlp_srl_bert"))
...
def filename_to_url(
    ...
    """
    return _cached_path.filename_to_url(
        filename, cache_dir=cache_dir or CACHE_DIRECTORY
    )
...
_active_tensor_caches: MutableMapping[int, "TensorCache"] = (
...
class TensorCache(MutableMapping[str, Tensor], ABC):
    ...
    def __new__(
        cls, filename: Union[str, PathLike], *, read_only: bool = False, **kwargs
    ):
...
class LocalCacheResource:
    ...
    def __init__(
        self, resource_name: str, version: str, cache_dir: str = CACHE_DIRECTORY
    ) -> None:
        self.resource_name = resource_name
        self.version = version
        self.cache_dir = cache_dir
        self.path = os.path.join(
            self.cache_dir, _resource_to_filename(resource_name, version)
        )
...
def text_lines_from_file(
    filename: Union[str, PathLike], strip_lines: bool = True
) -> Iterator[str]:
...
def remove_cache_entries(
    patterns: List[str], cache_dir: Union[str, Path] = None
) -> int:
    """
    ...
    for resource, entry in cache_entries.items():
        for meta in entry.regular_files:
            logger.info(
                "Removing cached version of %s at %s", resource, meta.cached_path
            )
        ...
        for meta in entry.extraction_dirs:
            logger.info(
                "Removing extracted version of %s at %s", resource, meta.cached_path
            )
...
def inspect_cache(patterns: List[str] = None, cache_dir: Union[str, Path] = None):
    ...
    """
    from allennlp_srl_bert.common.util import format_timedelta, format_size
