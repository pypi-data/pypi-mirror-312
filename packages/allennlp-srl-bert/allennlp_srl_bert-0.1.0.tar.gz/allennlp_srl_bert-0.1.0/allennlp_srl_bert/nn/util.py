...
from allennlp_srl_bert.common.util import (
    int_to_device,
    is_distributed,
    is_global_primary,
)
...
def read_state_dict(
    ...
    """
    state = torch.load(
        path, map_location=device_mapping(cuda_device), weights_only=True
    )
...
def find_text_field_embedder(model: torch.nn.Module) -> torch.nn.Module:
    ...
    """
    from allennlp_srl_bert.modules.text_field_embedders.text_field_embedder import (
        TextFieldEmbedder,
    )
...
def find_embedding_layer(model: torch.nn.Module) -> torch.nn.Module:
    ...
    from allennlp_srl_bert.modules.text_field_embedders.basic_text_field_embedder import (
...
def load_state_dict_distributed(
    ...
    from allennlp_srl_bert.nn.parallel.sharded_module_mixin import ShardedModuleMixin
    ...
    if isinstance(module, ShardedModuleMixin) or not submodules:
        ...
        _unexpected_keys = [
            k
            for k in _unexpected_keys
            if "." not in k or k.split(".")[0] not in submodules.keys()
        ]
        update_key_list(missing_keys, _missing_keys)
        update_key_list(unexpected_keys, _unexpected_keys)
        ...
        for name, submodule in submodules.items():
            ...
            update_key_list(
                unexpected_keys, [f"{name}.{key}" for key in _unexpected_keys]
            )
