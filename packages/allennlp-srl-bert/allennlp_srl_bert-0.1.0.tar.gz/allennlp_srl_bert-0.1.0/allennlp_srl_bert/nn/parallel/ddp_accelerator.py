...
from allennlp_srl_bert.common.util import int_to_device
...
class TorchDdpAccelerator(DdpAccelerator):
    ...
    def wrap_model(self, model: "Model") -> Tuple["Model", DdpWrappedModel]:
        ...
        wrapped_model = torch.nn.parallel.DistributedDataParallel(
            model,
            device_ids=(
                None if self.cuda_device == torch.device("cpu") else [self.cuda_device]
            ),
            **self._ddp_kwargs,
        )
        ...
        wrapped_model._register_state_dict_hook(
            TorchDdpAccelerator._remove_torch_ddp_prefix
        )
        wrapped_model._register_load_state_dict_pre_hook(
            TorchDdpAccelerator._add_torch_ddp_prefix
        )
