...
from allennlp_srl_bert.common import FromParams, Registrable
...
class KaimingUniformInitializer(_InitializerWrapper):
    ...
    def __init__(
        self, a: float = 0.0, mode: str = "fan_in", nonlinearity: str = "leaky_relu"
    ):
        super().__init__(
...
class SparseInitializer(_InitializerWrapper):
    ...
    def __init__(self, sparsity: float, std: float = 0.01):
        super().__init__(
            init_function=torch.nn.init.sparse_, sparsity=sparsity, std=std
        )
...
class BlockOrthogonalInitializer(_InitializerWrapper):
    ...
    def __init__(self, split_sizes: List[int], gain: float = 1.0):
        super().__init__(
            init_function=block_orthogonal, split_sizes=split_sizes, gain=gain
        )
        ...
        if tarfile.is_tarfile(weights_file_path):
            with extracted_archive(weights_file_path) as extraction_path:
                self.weights = torch.load(
                    get_weights_path(extraction_path), map_location="cpu"
                )
        else:
