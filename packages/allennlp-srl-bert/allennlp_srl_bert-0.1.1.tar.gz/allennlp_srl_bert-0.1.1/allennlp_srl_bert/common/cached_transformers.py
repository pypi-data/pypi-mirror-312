    ...
    if transformer is None:
        if not load_weights:
            ...
            if all(isinstance(x, int) for x in reinit_modules):
                ...
                if any(
                    layer_idx < 0 or layer_idx > num_layers
                ...
                try:
                    for layer_idx in reinit_modules:
                        transformer.encoder.layer[layer_idx].apply(
                            transformer._init_weights
                        )
...
def get_tokenizer(model_name: str, **kwargs) -> transformers.PreTrainedTokenizer:
    from allennlp_srl_bert.common.util import hash_object
