...
from allennlp_srl_bert.common.util import lazy_groups_of, shuffle_iterable
from allennlp_srl_bert.common.tqdm import Tqdm
from allennlp_srl_bert.data.instance import Instance
from allennlp_srl_bert.data.data_loaders.data_loader import DataLoader, TensorDict
from allennlp_srl_bert.data.data_loaders.data_collator import (
    DataCollator,
    DefaultDataCollator,
)
from allennlp_srl_bert.data.dataset_readers import (
    DatasetReader,
    WorkerInfo,
    DatasetReaderInput,
)
from allennlp_srl_bert.data.fields import TextField
from allennlp_srl_bert.data.samplers import BatchSampler
from allennlp_srl_bert.data.vocabulary import Vocabulary
import allennlp_srl_bert.nn.util as nn_util
...
class MultiProcessDataLoader(DataLoader):
    ...
    def __init__(
        ...
        *,
        ...
        effective_batch_size = (
            ...
            else self.batch_sampler.get_batch_size()
        )
    ...
    def _join_workers(
        self, workers: List[BaseProcess], queue, txs: List[Connection]
    ) -> None:
    ...
    def _safe_queue_put(
        ...
        while True:
            ...
            if fds:
                logger.warning(
                    "worker %d parent process has died, exiting now", worker_id
                )
                queue.cancel_join_thread()
                return False
    ...
    def _instances_to_batches(
        ...
        instance_iterator = (
            self._index_instance(instance) for instance in instance_iterator
        )

        if move_to_device and self.cuda_device is not None:
            tensorize = lambda batch: nn_util.move_to_device(
        ...
        if self.batch_sampler is not None:
            ...
            if self.max_instances_in_memory is not None:
                instance_chunks = lazy_groups_of(
