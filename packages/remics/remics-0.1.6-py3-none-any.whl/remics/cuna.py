...
def get_network(rdscr_df, plot_flag, pvalues, community_flag=1, verbose=0):
    ...
    final_df.to_csv('final_network.csv')
    final_df['edge_pair'] = final_df[['v1','v2']].apply(lambda row:tuple(sorted(row)), axis=1)
    duplicates = final_df[final_df.duplicated(subset='edge_pair')]
    no_dup_df = final_df.drop_duplicates(subset='edge_pair')
    if verbose == 1: 
        print("There were " + str(duplicates.shape[0]) + " duplicate edges")

    #print("\n ====== STABLE INTERACTIONS ====== \n")
    #sort by count and extract stable interactions
    s = no_dup_df.groupby(by=['v1','v2']).count()
    ...
    df_ct = pd.merge(no_dup_df, df1, on=['v1','v2'], how='inner')
