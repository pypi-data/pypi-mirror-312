...
class EventModel(BaseModel):
    ...
    def dispatch(self):
        ...
        Dispatch event.
        """
        if self._uid not in _event_registry:
    ...
    def get_dependency(self, dependency_type: Type[T]) -> T:
        ...
        """
        _event_registry.add(self)  # TODO: Could also be on init?
...
def _base_event(event: str | Enum) -> _Event:
    return _Event(type=_get_event_id(event))
...
def is_event_trigger(event: str | Enum) -> bool:
    """
...
def resolve_event_components() -> list[dcc.Store]:
    ...
    """
    return [dcc.Store(id=event._component_id, storage_type="memory") for event in list(_event_registry)]
...
"""
_event_registry: set[EventModel] = set()
