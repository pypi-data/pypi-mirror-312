...
def get_image_roi(image: np.ndarray, roi_coords: np.ndarray, shape_type: str) -> tuple[np.ndarray, tuple[int, int]]:
    """
    Get ROI from image
    ...
    if shape_type != "rectangle":
        raise ValueError("ROIs must be a single rectangle")
    elif roi_coords.shape != (4,2):
        notifications.show_warning(f"Expected ROI shape to be (4,2): Got {roi_coords.shape}. Reshaping ROI to square.")
        roi_coords = np.delete(roi_coords, [0, 1], axis=1)
...
def restore_image(
    ...
    image_name = f"{image_name}_{restore_name}"
...
def count_objects(
    ...
    def compute_masks(batch_size):
        if roi is None or len(roi.data) == 0:
            ...
            image_roi, roi_offset = get_image_roi(image, roi.data[0], roi.shape_type[0])
            ...
            return resized_masks, roi_offset
