...
class TestUtils(unittest.TestCase):
    ...
    def test_points2grids(self):
        ...
        llpoiint = (10.25, 30.5)
        urpoint = (15, 35)
        x_grid = np.arange(llpoiint[0], urpoint[0], xstep)
        y_grid = np.arange(llpoiint[1], urpoint[1], ystep)
        xx, yy = np.meshgrid(lons, lats)
        values = 3*lons**2 + 2*lats**2
        points = np.array([lons, lats, values]).transpose()
        data, inds = gnss_utils.points2grids(points, llpoiint, (len(x_grid), len(y_grid)), (xstep, ystep))
