...
def points2grids(points, llpoint, shape, cellsize, method="mean"):
    """
    points: n*3
    points[0]: x
    points[1]: y
    points[2]: z
    llpoint: (llx, lly) 左下角像元中心点坐标
    shape: (ncol, nrow) 列数(x方向）, 行数(y方向）
    cellsize: (xcellsize, ycellsize), x 和 y方向格网大小
    method="mean", "max", "min", "std", "count"。平均值，最大值，最小值，标准差，计数
    """
    ncol, nrow = shape[0], shape[1]
    xstep, ystep = cellsize[0], cellsize[1]
    xmin, ymin = llpoint[0]-xstep/2., llpoint[1]-ystep/2.
