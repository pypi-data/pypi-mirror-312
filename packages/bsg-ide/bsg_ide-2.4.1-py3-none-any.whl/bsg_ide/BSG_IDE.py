    ...
    def setup_logo(self):
        ...
        try:
            ...
            if not self.has_logo:
                print("Warning: Using ASCII logo - no image found")
                self.has_logo = False
...
#---------------------------------------------------------------------------------------
    def setup_window_scaling(self):
        """Configure window scaling based on screen resolution"""
        try:
            # Get screen dimensions
            screen_width = self.winfo_screenwidth()
            screen_height = self.winfo_screenheight()

            # Calculate scaling factor based on reference resolution of 1920x1080
            width_scale = screen_width / 1920
            height_scale = screen_height / 1080
            self.scale_factor = min(width_scale, height_scale)

            # Set minimum window size (scaled)
            min_width = int(800 * self.scale_factor)
            min_height = int(600 * self.scale_factor)
            self.minsize(min_width, min_height)

            # Set initial window size (scaled)
            initial_width = int(1200 * self.scale_factor)
            initial_height = int(800 * self.scale_factor)

            # Calculate centered position
            pos_x = (screen_width - initial_width) // 2
            pos_y = (screen_height - initial_height) // 2

            # Set geometry with position and size
            self.geometry(f"{initial_width}x{initial_height}+{pos_x}+{pos_y}")

            # Configure font scaling
            self.default_font_size = int(12 * self.scale_factor)
            self.title_font_size = int(14 * self.scale_factor)
            self.small_font_size = int(10 * self.scale_factor)

            # Configure widget sizes
            self.button_height = int(32 * self.scale_factor)
            self.icon_size = int(24 * self.scale_factor)
            self.padding = int(5 * self.scale_factor)

            # Update fonts for all text widgets
            self._update_widget_fonts()

        except Exception as e:
            print(f"Warning: Error setting up window scaling: {str(e)}")
            # Use default values if scaling fails
            self.scale_factor = 1.0
            self.geometry("1200x800")
            self.minsize(800, 600)

    def _update_widget_fonts(self):
        """Update fonts for all text widgets based on scaling"""
        try:
            # Update content editor font
            if hasattr(self, 'content_editor'):
                self.content_editor.configure(
                    font=("TkFixedFont", self.default_font_size)
                )

            # Update notes editor font
            if hasattr(self, 'notes_editor'):
                self.notes_editor.configure(
                    font=("TkFixedFont", self.default_font_size)
                )

            # Update slide list font
            if hasattr(self, 'slide_list'):
                self.slide_list.configure(
                    font=("TkFixedFont", self.small_font_size)
                )
            ...
            print(f"Warning: Error updating widget fonts: {str(e)}")

    def create_widgets(self):
        """Modified create_widgets with scaling support"""
        # Call setup_window_scaling first
        self.setup_window_scaling()

        # Create menu frame with scaled padding
        self.setup_top_menu()

        # Configure grid with scaled weights
        self.grid_columnconfigure(1, weight=1)
        self.grid_rowconfigure(0, weight=1)

        # Create main components with scaled dimensions
        self.create_sidebar()
        self.create_main_editor()
        self.create_toolbar()
        self.create_context_menu()
        ...
        # Adjust grid weights for terminal
        self.grid_rowconfigure(1, weight=3)  # Main editor
        self.grid_rowconfigure(4, weight=1)  # Terminal
    ...
    def _scale_widget(self, widget, min_width=None, min_height=None):
        """Helper method to scale individual widgets"""
        try:
            # Scale widget dimensions
            if min_width:
                widget.configure(width=int(min_width * self.scale_factor))
            if min_height:
                widget.configure(height=int(min_height * self.scale_factor))

            # Scale padding
            widget.configure(padx=self.padding, pady=self.padding)

        except Exception as e:
            print(f"Warning: Error scaling widget: {str(e)}")

    def update_window_scaling(self, event=None):
        ...
        if event and event.widget == self:
            try:
                ...
                # Store new dimensions in session data
                ...
                print(f"Warning: Error updating window scaling: {str(e)}")
 #---------------------------------------------------------------------------------------
