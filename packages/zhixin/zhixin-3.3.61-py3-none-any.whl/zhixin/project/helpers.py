...
from zhixin import __version__, exception, fs
from zhixin.compat import IS_MACOS, IS_WINDOWS, hashlib_encode_data
...
def is_zhixin_project(project_dir=None):
    if not project_dir:
        project_dir = get_project_dir()
    return os.path.isfile(os.path.join(project_dir, ".zxide", "zhixin.ini"))


def find_project_dir_above(path):
    if os.path.isfile(path):
        path = os.path.dirname(path)
    if is_zhixin_project(path):
        return path
...
def get_project_watch_lib_dirs():
    """Used by zhixin-node-helpers.project.observer.fetchLibDirs"""
...
def get_project_cache_dir():
    """Deprecated, use ProjectConfig.get("zhixin", "cache_dir") instead"""
    return ProjectConfig.get_instance().get("zhixin", "cache_dir")
    ...
    try:
        ...
        if not IS_MACOS:
            try:
                ...
                pass
    return os.path.join(docs_dir, "ZhiXin", "Projects")


def compute_project_checksum(config):
    # rebuild when ZX Core version changes
    ...
    for d in (
        config.get("zhixin", "include_dir"),
        config.get("zhixin", "src_dir"),
        config.get("zhixin", "lib_dir"),
    ):
...
def _get_cached_build_metadata(env_names):
    build_dir = ProjectConfig.get_instance().get("zhixin", "build_dir")
    result = {}
