...
from zhixin.package.version import get_original_version
from zhixin.platform.exception import UnknownBoard
from zhixin.platform.factory import PlatformFactory
from zhixin.project.config import ProjectOptions
...
def _ZxPlatform():
    ...
    return PlatformFactory.from_env(env["ZXENV"], targets=COMMAND_LINE_TARGETS)
...
def LoadZxPlatform(env):
    p = env.ZxPlatform()

    # Ensure real platform name
    env["ZXPLATFORM"] = p.name
...
def PrintConfiguration(env):  # pylint: disable=too-many-statements
    ...
    def _get_configuration_data():
        return (
            ...
            if not board_config
                ...
                "CONFIGURATION:",
                "https://docs.ZhiXin-Semi.com/page/boards/%s/%s.html"
    ...
    def _get_packages_data():
        ...
        for item in platform.dump_used_packages():
            ...
            if "src_url" in item and int(ARGUMENTS.get("ZXVERBOSE", 0)):
