...
def get_project_global_lib_dir():
    return ProjectConfig.get_instance().get("zhixin", "globallib_dir")
...
def invoke_command(ctx, cmd, **kwargs):
    ...
    for input_dir in input_dirs:
        ...
        if is_zhixin_project(input_dir):
...
@click.option(
    "-g", "--global", is_flag=True, help="Manage global ZhiXin library storage"
)
@click.option(
    "-e",
    "--environment",
    ...
    help=(
        ...
        "declared in `zhixin.ini`"
    ),
)
@click.pass_context
def cli(ctx, **options):
    in_silence = ZhixinCLI.in_silence()
...
@click.option(
    ...
    is_flag=True,
    default=True,
    help="Save installed libraries into the `zhixin.ini` dependency list"
    " (enabled by default)",
)
...
def lib_install(  # pylint: disable=too-many-arguments,unused-argument
    ...
    click.secho(
        ...
        "the next releases. \nPlease use `zx pkg install` instead.\n",
        fg="yellow",
    )
...
@click.option(
    ...
    is_flag=True,
    default=True,
    help="Remove libraries from the `zhixin.ini` dependency list and save changes"
    " (enabled by default)",
)
@click.option("-s", "--silent", is_flag=True, help="Suppress progress reporting")
@click.pass_context
def lib_uninstall(ctx, libraries, save, silent):
    click.secho(
        "\nWARNING: This command is deprecated and will be removed in "
        "the next releases. \nPlease use `zx pkg uninstall` instead.\n",
        fg="yellow",
    )
    invoke_command(
        ctx,
...
def lib_update(  # pylint: disable=too-many-arguments
    ...
    if only_check and not json_output:
        raise exception.UserSideException(
            "This command is deprecated, please use `zx pkg outdated` instead"
        )
...
def lib_list(ctx, json_output):
    if not json_output:
        click.secho(
            "\nWARNING: This command is deprecated and will be removed in "
            "the next releases. \nPlease use `zx pkg list` instead.\n",
            fg="yellow",
        )
...
def lib_search(  # pylint: disable=unused-argument
    ...
    if not json_output:
        click.secho(
            "\nWARNING: This command is deprecated and will be removed in "
            "the next releases. \nPlease use `zx pkg search` instead.\n",
            fg="yellow",
        )
...
def lib_show(ctx, library, json_output):
    if not json_output:
        click.secho(
            "\nWARNING: This command is deprecated and will be removed in "
            "the next releases. \nPlease use `zx pkg show` instead.\n",
            fg="yellow",
        )
...
def lib_register(config_url):  # pylint: disable=unused-argument
    raise exception.UserSideException(
        "This command is deprecated. Please use `zx pkg publish` command."
    )
...
def lib_stats(json_output):
    if not json_output:
        click.secho(
            "\nWARNING: This command is deprecated and will be removed in "
            "the next releases. \nPlease visit "
            "https://registry.ZhiXin-Semi.com\n",
            fg="yellow",
        )
        return None
