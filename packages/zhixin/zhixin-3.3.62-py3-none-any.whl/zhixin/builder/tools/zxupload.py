...
from zhixin.device.finder import SerialPortFinder, find_mbed_disk, is_pattern_port
...
def AutodetectUploadPort(*args, **kwargs):
    ...
    if upload_protocol == "mbed" or (
        "mbed" in env.subst("$ZXFRAMEWORK") and not upload_protocol
    ):
...
def CheckUploadSize(_, target, source, env):
    ...
    print('Advanced Memory Usage is available via "ZhiXin Home > Project Inspect"')
