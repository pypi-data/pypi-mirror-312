...
def _collect_sections_info(env, elffile):
    ...
    for section in elffile.iter_sections():
        ...
        sections[section.name]["in_flash"] = env.zxSizeIsFlashSection(section_data)
        sections[section.name]["in_ram"] = env.zxSizeIsRamSection(section_data)
...
def _collect_symbols_info(env, elffile, elf_path, sections):
    ...
    for s in symbol_section.iter_symbols():
        ...
        if not env.zxSizeIsValidSymbol(s.name, symbol_type, symbol_addr):
            continue
        ...
        symbol = {
            ...
            "type": symbol_type,
            "size": symbol_size,
            "section": env.zxSizeDetermineSection(sections, symbol_addr),
        }
...
def zxSizeDetermineSection(_, sections, symbol_addr):
...
def zxSizeIsValidSymbol(_, symbol_name, symbol_type, symbol_address):
    return symbol_name and symbol_address != 0 and symbol_type != "STT_NOTYPE"


def zxSizeIsRamSection(_, section):
    return (
...
def zxSizeIsFlashSection(_, section):
    return section.get("type", "") == "SHT_PROGBITS" and "A" in section.get("flags", "")


def zxSizeCalculateFirmwareSize(_, sections):
...
def DumpSizeData(_, target, source, env):  # pylint: disable=unused-argument
    ...
    with open(elf_path, "rb") as fp:
        ...
        firmware_ram, firmware_flash = env.zxSizeCalculateFirmwareSize(sections)
        data["memory"]["total"] = {
...
def generate(env):
    env.AddMethod(zxSizeIsRamSection)
    env.AddMethod(zxSizeIsFlashSection)
    env.AddMethod(zxSizeCalculateFirmwareSize)
    env.AddMethod(zxSizeDetermineSection)
    env.AddMethod(zxSizeIsValidSymbol)
