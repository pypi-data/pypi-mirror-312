...
from zhixin.device.monitor.command import (
...
from zhixin.test.cli import cli as test_cmd
...
def cli(ctx, agent):
    ...
    contrib_dir = get_core_package_dir("contrib-zxremote")
...
@click.option(
    "-d",
    "--working-dir",
    envvar="ZHIXIN_REMOTE_AGENT_DIR",
    type=click.Path(file_okay=False, dir_okay=True, writable=True),
)
def remote_agent_start(name, share, working_dir):
    from zhixin.remote.client.agent_service import RemoteAgentService
...
def remote_agent_list():
    from zhixin.remote.client.agent_list import AgentListClient
...
def remote_update(agents, only_check, dry_run):
    from zhixin.remote.client.update_core import UpdateCoreClient
...
def remote_run(
    ...
    from zhixin.remote.client.run_or_test import RunOrTestClient
...
def device_list(agents, json_output):
    from zhixin.remote.client.device_list import DeviceListClient
...
@click.option(
    "-e",
    "--environment",
    help="Load configuration from `zhixin.ini` and specified environment",
)
@click.option(
    "--sock",
    type=click.Path(exists=True, file_okay=False, dir_okay=True, writable=True),
...
def device_monitor(ctx, agents, **kwargs):
    from zhixin.remote.client.device_monitor import DeviceMonitorClient
    ...
    def _tx_target(sock_dir):
        ...
        subprocess.call([proc.where_is_program("zhixin")] + subcmd_argv)

    sock_dir = mkdtemp(suffix="zx")
