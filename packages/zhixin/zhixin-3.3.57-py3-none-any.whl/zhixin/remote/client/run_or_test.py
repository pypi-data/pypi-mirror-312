...
from zhixin.compat import hashlib_encode_data
from zhixin.project.config import ProjectConfig
from zhixin.remote.client.async_base import AsyncClientBase
from zhixin.remote.projectsync import PROJECT_SYNC_STAGE, ProjectSync
    ...
    def add_project_items(self, psync):
        with fs.cd(self.options["project_dir"]):
            cfg = ProjectConfig.get_instance(
                os.path.join(self.options["project_dir"], ".zxide", "zhixin.ini")
            )
            psync.add_item(cfg.path, "zhixin.ini")
            psync.add_item(cfg.get("zhixin", "shared_dir"), "shared")
            psync.add_item(cfg.get("zhixin", "boards_dir"), "boards")
            ...
            if self.command == "test":
                psync.add_item(cfg.get("zhixin", "test_dir"), "test")

    def _add_project_source_items(self, cfg, psync):
        psync.add_item(cfg.get("zhixin", "lib_dir"), "lib")
        psync.add_item(
            cfg.get("zhixin", "include_dir"),
            "include",
            cb_filter=self._cb_tarfile_filter,
        )
        psync.add_item(
            cfg.get("zhixin", "src_dir"), "src", cb_filter=self._cb_tarfile_filter
        )
        if set(["buildfs", "uploadfs", "uploadfsota"]) & set(
            self.options.get("target", [])
        ):
            psync.add_item(cfg.get("zhixin", "data_dir"), "data")

    @staticmethod
    def _add_project_binary_items(cfg, psync):
        build_dir = cfg.get("zhixin", "build_dir")
        for env_name in os.listdir(build_dir):
            ...
            for fname in os.listdir(env_dir):
                ...
                if os.path.isfile(bin_file) and fname.endswith(bin_exts):
                    psync.add_item(
                        bin_file, os.path.join(".zx", "build", env_name, fname)
                    )
