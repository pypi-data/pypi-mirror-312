...
from zhixin.debug.helpers import get_default_debug_env
...
class ProjectGenerator:
    ...
    def _load_tplvars(self):
        tpl_vars = {
            ...
            "project_name": self.config.get(
                "zhixin", "name", os.path.basename(self.project_dir)
            ),
            ...
            tpl_vars.update(
                {
                    ...
                    "project_src_dir": self.config.get("zhixin", "src_dir"),
                    "project_lib_dir": self.config.get("zhixin", "lib_dir"),
                    "project_test_dir": self.config.get("zhixin", "test_dir"),
                    "project_libdeps_dir": os.path.join(
                        self.config.get("zhixin", "libdeps_dir"), self.env_name
                    ),
                }
            )
    ...
    def get_src_files(self):
        result = []
        with fs.cd(self.project_dir):
            for root, _, files in os.walk(self.config.get("zhixin", "src_dir")):
                for f in files:
                    result.append(
