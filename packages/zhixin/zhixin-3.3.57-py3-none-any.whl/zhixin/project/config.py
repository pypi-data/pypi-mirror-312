...
class ProjectConfig(ProjectConfigBase, ProjectConfigLintMixin, ProjectConfigDirsMixin):
    ...
    def save(self, path=None):
        ...
        # click.echo("_instances: %s" % self._instances)
        if path in self._instances:
            del self._instances[path]

        with open(path or self.path, mode="w+", encoding="utf8") as fp, open(path.replace('.ini', '.json') or self.path.replace('.ini', '.json'), mode="w+", encoding="utf8") as nfp:
