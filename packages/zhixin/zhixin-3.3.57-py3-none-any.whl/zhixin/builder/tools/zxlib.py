...
from zhixin import exception, fs
from zhixin.builder.tools import zxbuild
from zhixin.compat import IS_WINDOWS, hashlib_encode_data, string_types
from zhixin.http import HTTPClientError, InternetConnectionError
from zhixin.package.exception import (
    MissingPackageManifestError,
    UnknownPackageError,
)
from zhixin.package.manager.library import LibraryPackageManager
from zhixin.package.manifest.parser import (
    ManifestParserError,
    ManifestParserFactory,
)
from zhixin.package.meta import PackageCompatibility, PackageItem, PackageSpec
from zhixin.project.options import ProjectOptions


class LibBuilderFactory:
    @staticmethod
    def new(env, path, verbose=int(ARGUMENTS.get("ZXVERBOSE", 0))):
        clsname = "UnknownLibBuilder"
        if os.path.isfile(os.path.join(path, "library.json")):
            clsname = "ZhiXinLibBuilder"
        else:
            used_frameworks = LibBuilderFactory.get_used_frameworks(env, path)
            common_frameworks = set(env.get("ZXFRAMEWORK", [])) & set(used_frameworks)
        ...
        # Handle ZhiXinLibBuilder.manifest.build.builder
        # pylint: disable=protected-access
        if isinstance(obj, ZhiXinLibBuilder) and obj._manifest.get("build", {}).get(
            "builder"
        ):
        ...
        for root, _, files in os.walk(path, followlinks=True):
            ...
            for fname in files:
                if not fs.path_endswith_ext(
                    fname, zxbuild.SRC_BUILD_EXT + zxbuild.SRC_HEADER_EXT
                ):
                    continue
                with io.open(
...
class LibBuilderBase:
    ...
    def __init__(self, env, path, manifest=None, verbose=False):
        ...
        if "test" in env["BUILD_TYPE"]:
            self.env.Append(CPPDEFINES=["ZX_UNIT_TESTING"])
    ...
    @property
    def src_filter(self):
        return zxbuild.SRC_FILTER_DEFAULT + [
            "-<example%s>" % os.sep,
            "-<examples%s>" % os.sep,
            "-<test%s>" % os.sep,
            "-<tests%s>" % os.sep,
        ...
        with fs.cd(self.path):
            ...
            if self.extra_script:
                ...
                self.env.SConscript(
                    ...
                    exports={"env": self.env, "zx_lib_builder": self},
                )
    ...
    def get_search_files(self):
        return [
            ...
            for item in self.env.MatchSourceFiles(
                self.src_dir, self.src_filter, zxbuild.SRC_BUILD_EXT
            )
        ]
        ...
        while search_files:
            ...
            for item in candidates:
                ...
                if not self.PARSE_SRC_BY_H_NAME:
                    continue
                if not fs.path_endswith_ext(item_path, zxbuild.SRC_HEADER_EXT):
                    continue
                item_fname = item_path[: item_path.rindex(".")]
                for ext in zxbuild.SRC_C_EXT + zxbuild.SRC_CXX_EXT:
...
class ArduinoLibBuilder(LibBuilderBase):
    ...
    def src_filter(self):
        ...
        for ext in zxbuild.SRC_BUILD_EXT + zxbuild.SRC_HEADER_EXT:
    ...
    def lib_ldf_mode(self):
        ...
        global_value = self.env.GetProjectConfig().getraw(
            "env:" + self.env["ZXENV"], "lib_ldf_mode", missing
        )
    ...
    def _mbed_conf_append_macros(self, mbed_config_path, macros):
        ...
        with open(mbed_config_path, encoding="utf8") as fp:
            for line in fp.readlines():
                line = line.strip()
                if line == "#endif":
                    lines.append("// ZhiXin Library Dependency Finder (LDF)")
                    lines.extend(
                        [
...
class ZhiXinLibBuilder(LibBuilderBase):
    def load_manifest(self):
    ...
    def get_include_dirs(self):
        ...
        # backwards compatibility with ZhiXin 2.0
        if (
    ...
    def lib_archive(self):
        missing = object()
        global_value = self.env.GetProjectConfig().getraw(
            "env:" + self.env["ZXENV"], "lib_archive", missing
        )
        if global_value != missing:
            return self.env.GetProjectConfig().get(
                "env:" + self.env["ZXENV"], "lib_archive"
            )
        # pylint: disable=no-member
...
class ProjectAsLibBuilder(LibBuilderBase):
        ...
        if "test" in build_type:
            items.extend(
                [
                    ...
                    for item in self.env.MatchSourceFiles(
                        "$PROJECT_TEST_DIR", "$ZXTEST_SRC_FILTER"
                    )
                ]
            )
        return items
    ...
    def install_dependencies(self):
        ...
        lm = LibraryPackageManager(
            self.env.subst(os.path.join("$PROJECT_LIBDEPS_DIR", "$ZXENV"))
        )
        ...
        # reset cache
        if did_install:
            DefaultEnvironment().Replace(__ZX_LIB_BUILDERS=None)
...
def IsCompatibleLibBuilder(env, lb, verbose=int(ARGUMENTS.get("ZXVERBOSE", 0))):
    compat_mode = lb.lib_compat_mode
    if lb.name in env.GetProjectOption("lib_ignore", []):
        if verbose:
            sys.stderr.write("Ignored library %s\n" % lb.path)
        return None
    if compat_mode == "strict" and not lb.is_platforms_compatible(env["ZXPLATFORM"]):
        if verbose:
            sys.stderr.write("Platform incompatible library %s\n" % lb.path)
        return False
    if compat_mode in ("soft", "strict") and not lb.is_frameworks_compatible(
        env.get("ZXFRAMEWORK", "__noframework__")
    ):
        if verbose:
...
def GetLibBuilders(_):  # pylint: disable=too-many-branches
    ...
    if env.get("__ZX_LIB_BUILDERS", None) is not None:
        return sorted(
            env["__ZX_LIB_BUILDERS"],
            key=lambda lb: 0 if lb.is_dependent else 1,
        )

    env.Replace(__ZX_LIB_BUILDERS=[])

    verbose = int(ARGUMENTS.get("ZXVERBOSE", 0))
    ...
    for storage_dir in env.GetLibSourceDirs():
        ...
        for item in sorted(os.listdir(storage_dir)):
            ...
            if env.IsCompatibleLibBuilder(lb):
                env.Append(__ZX_LIB_BUILDERS=[lb])
            else:
                found_incompat = True
...
def ConfigureProjectLibBuilder(env):
    ...
    click.echo("LDF: Library Dependency Finder -> https://bit.ly/configure-zx-ldf")
    click.echo(
