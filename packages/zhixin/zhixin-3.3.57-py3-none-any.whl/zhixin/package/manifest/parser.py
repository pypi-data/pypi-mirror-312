...
from zhixin.compat import get_object_members, string_types
from zhixin.http import fetch_remote_content
from zhixin.package.exception import ManifestParserError, UnknownManifestError
...
class BaseManifestParser:
    ...
    def parse_examples_from_dir(package_dir):
        ...
        for root, _, files in os.walk(examples_dir):
            ...
            if is_zhixin_project(root):
                last_zx_project = root
                result[last_zx_project] = dict(
                ...
                continue
            if last_zx_project:
                if root.startswith(last_zx_project):
                    result[last_zx_project]["files"].extend(
                        [
                            os.path.relpath(os.path.join(root, f), last_zx_project)
                            for f in files
                        ]
                    )
                    continue
