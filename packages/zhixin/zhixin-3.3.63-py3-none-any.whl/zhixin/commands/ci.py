...
from zhixin.exception import CIBuildEnvsEmpty
from zhixin.project.commands.init import project_init_cmd, validate_boards
...
def cli(  # pylint: disable=too-many-arguments, too-many-branches
    ...
    if not src and os.getenv("ZHIXIN_CI_SRC"):
        src = validate_path(ctx, None, os.getenv("ZHIXIN_CI_SRC").split(":"))
    if not src:
...
def _copy_project_conf(build_dir, project_conf):
    ...
    if config.has_section("zhixin"):
        config.remove_section("zhixin")
    config.save(os.path.join(build_dir, ".zxide", "zhixin.ini"))
