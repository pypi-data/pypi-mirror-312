...
from zhixin import app, exception, fs
...
from zhixin.project.exception import ProjectError
from zhixin.project.helpers import get_project_dir, is_zhixin_project
...
class ProjectRPC(BaseRPCHandler):
    ...
    def config_call(init_kwargs, method, *args):
        ...
        if os.path.isdir(init_kwargs["path"]):
            ...
            init_kwargs["path"] = os.path.join(init_kwargs["path"], ".zxide", "zhixin.ini")
    ...
    def config_update_description(path, text):
        config = ProjectConfig(path, parse_extra=False, expand_interpolations=False)
        if not config.has_section("zhixin"):
            config.add_section("zhixin")
        if text:
            config.set("zhixin", "description", text)
        else:
            if config.has_option("zhixin", "description"):
                config.remove_option("zhixin", "description")
            if not config.options("zhixin"):
                config.remove_section("zhixin")
        return config.save()
    ...
    def get_projects():
        def _get_project_data():
            ...
            data["description"] = config.get("zhixin", "description")
            data["libExtraDirs"].extend(config.get("zhixin", "lib_extra_dirs", []))

            libdeps_dir = config.get("zhixin", "libdeps_dir")
        ...
        for project_dir in AppRPC.load_state()["storage"]["recentProjects"]:
            ...
            for board_id in data.get("boards", []):
                ...
                except exception.ZhixinException:
                    pass
    ...
    def get_project_examples():
        ...
        for pkg in pm.get_installed():
            ...
            for project_dir, _, __ in os.walk(examples_dir):
                ...
                try:
                    config = ProjectConfig(os.path.join(project_dir, ".zxide", "zhixin.ini"))
                    config.validate(silent=True)
                    project_description = config.get("zhixin", "description")
                except ProjectError:
                    continue
    ...
    async def init(self, board, framework, project_dir):
        ...
        if board:
            ...
            args = ["init", "--no-install-dependencies"]
        if framework:
    ...
    async def import_arduino(board, use_arduino_libs, arduino_project_dir):
        board = str(board)
        # don't import ZX Project
        if is_zhixin_project(arduino_project_dir):
    ...
    @staticmethod
    async def import_zx(project_dir):
        if not project_dir or not is_zhixin_project(project_dir):
            raise JSONRPC20DispatchException(
                code=4001, message="Not an ZhiXin project: %s" % project_dir
            )
    ...
    async def init_v2(self, configuration, options=None):
        ...
        envclone["ZHIXIN_FORCE_ANSI"] = "true"
        options = options or {}
    ...
    def configuration(project_dir, env):
        assert is_zhixin_project(project_dir)
        with fs.cd(project_dir):
            config = ProjectConfig(os.path.join(project_dir, ".zxide", "zhixin.ini"))
