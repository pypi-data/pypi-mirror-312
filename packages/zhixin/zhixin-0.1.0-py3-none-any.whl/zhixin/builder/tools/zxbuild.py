...
from zhixin.compat import IS_MACOS, string_types
from zhixin.package.version import pepver_to_semver
from zhixin.proc import where_is_program
...
def BuildProgram(env):
    ...
    if (
        ...
        and (env.ZxPlatform().is_embedded() or not IS_MACOS)
    ):
        env.Prepend(_LIBFLAGS="-Wl,--start-group ")
        env.Append(_LIBFLAGS=" -Wl,--end-group")

    program = env.Program(env.subst("$PROGPATH"), env["ZXBUILDFILES"])
    env.Replace(ZXMAINPROG=program)

    AlwaysBuild(
        env.Alias(
            "checkprogsize",
            program,
            env.VerboseAction(env.CheckUploadSize, "Checking size $ZXMAINPROG"),
        )
    )
...
def ProcessProgramDeps(env):
    ...
    env.BuildFrameworks(env.get("ZXFRAMEWORK"))
...
def ProcessProjectDeps(env):
    ...
    if "test" in env["BUILD_TYPE"]:
        build_files_before_nums = len(env.get("ZXBUILDFILES", []))
        plb.env.BuildSources(
            "$BUILD_TEST_DIR", "$PROJECT_TEST_DIR", "$ZXTEST_SRC_FILTER"
        )
        if len(env.get("ZXBUILDFILES", [])) - build_files_before_nums < 1:
            sys.stderr.write(
    ...
    if not env.get("ZXBUILDFILES") and not COMMAND_LINE_TARGETS:
        sys.stderr.write(
...
def CollectBuildFiles(
    ...
    middlewares = env.get("__ZX_BUILD_MIDDLEWARES")
    if not middlewares:
        return sources
...
def AddBuildMiddleware(env, callback, pattern=None):
    env.Append(__ZX_BUILD_MIDDLEWARES=[(callback, pattern)])
...
def BuildFrameworks(env, frameworks):
    ...
    if "BOARD" not in env:
        sys.stderr.write(
            "Please specify `board` in `zhixin.ini` to use "
            "with '%s' framework\n" % ", ".join(frameworks)
        )
        env.Exit(1)
