...
def get_zhixin_udev_rules_path():
    return os.path.abspath(
        os.path.join(get_assets_dir(), "system", "99-zhixin-udev.rules")
    )


def ensure_udev_rules():
    from zhixin.util import get_systype  # pylint: disable=import-outside-toplevel
    ...
    if "linux" not in get_systype():
        return None
    installed_rules = [
        "/etc/udev/rules.d/99-zhixin-udev.rules",
        "/lib/udev/rules.d/99-zhixin-udev.rules",
    ]
    if not any(os.path.isfile(p) for p in installed_rules):
        raise exception.MissedUdevRules

    origin_path = get_zhixin_udev_rules_path()
