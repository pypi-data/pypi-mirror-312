...
from zhixin import app, exception, fs, proc
from zhixin.compat import IS_WINDOWS
from zhixin.debug import helpers
from zhixin.debug.config.factory import DebugConfigFactory
from zhixin.debug.exception import DebugInvalidOptionsError
from zhixin.debug.process.gdb import GDBClientProcess
...
def cli(
    ...
    for name in ("CWD", "PWD", "ZHIXIN_PROJECT_DIR"):
        if is_zhixin_project(project_dir):
            break
        if os.getenv(name):
    ...
    with fs.cd(project_dir):
        ...
        if helpers.is_gdbmi_mode():
            os.environ["ZHIXIN_DISABLE_PROGRESSBAR"] = "true"
