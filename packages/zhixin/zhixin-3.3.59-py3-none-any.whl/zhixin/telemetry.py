...
from zhixin import __title__, __version__, app, exception, fs, util
from zhixin.cli import ZhixinCLI
from zhixin.debug.config.base import DebugConfigBase
from zhixin.http import HTTPSession
    ...
    def _commit_events(self, events):
        ...
        try:
            r = self._http_session.post(
                "https://collector.ZhiXin-Semi.com/collect",
                json=payload,
...
def log_command(ctx):
    params = {
        "path_args": ZhixinCLI.reveal_cmd_path_args(ctx),
    }
    if is_ci():
...
def log_exception(exc):
    ...
    is_fatal = any(
        [
            not isinstance(exc, exception.ZhixinException),
