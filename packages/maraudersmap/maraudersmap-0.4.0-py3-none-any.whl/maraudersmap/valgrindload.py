...
def parse_valgrind_data(
    valgrind_lines: List[str], only_sources: bool = True, skip_negligible: bool = True
) -> nx.DiGraph:
    ...
    for line in body:
        ...
        if kind == "*":
            ...
            if skip_negligible and fraction == 0:  # skip low impact files
                ...
                logger.debug(f"Skipping negligible node : {nodename}")

            if only_sources and not file.startswith(
                root_sources
            ):  # skip  files out of root_sources
                logger.debug(f"Skipping non-source node : {only_sources}")
                keep_this_node = False
