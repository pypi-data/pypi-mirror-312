...
def get_callgraph(
    ...
    if Path(path).is_file():
        ...
        tgt_code_db = run_struct(tgt_paths_dict,only_procedures=True, cpp_directives=cpp_directives)
    


    if context.strip("/") == path.strip("/"):  # often the case so it avoid recomputation
        context_code_db = tgt_code_db
    else:
        if Path(context).is_file():
            context_code_db = run_struct({os.path.basename(context):context}, cpp_directives=cpp_directives)
        else:
            tgt_paths_dict = find_package_files_and_folders(context)
            context_code_db = run_struct(tgt_paths_dict,only_procedures=True, cpp_directives=cpp_directives)
    ...
    if valgrind_graph is not None:
        ...
        callgraph = build_callgraph_from_cg(tgt_code_db,context_code_db,valgrind_graph)
    else:
