...
from libqcanvas.database.types import ModulePageType
from libqcanvas.gql_queries.canvas_course_data import Submission
...
class APIDataImporter(CanvasSyncObservable):
    ...
    async def convert_and_store_page(self, page: PageWithContent) -> None:
        ...
        db_page.unlock_at = as_utc(page.unlock_at)
        db_page.lock_at = as_utc(page.lock_at)
        db_page.creation_date = as_utc(page.created_at)
        db_page.last_modification_date = as_utc(page.updated_at)
    ...
    async def convert_and_store_term(self, term: gql.Term) -> None:
        ...
        db_term.end_date = as_utc(term.end_at)
        db_term.start_date = as_utc(term.start_at)
    ...
    async def convert_and_store_assignment(
        ...
        if not (
            ...
            or (submission is not None and db_assignment.score != submission.score)
        ):
            # Updating the record is not going to do anything
            return
        ...
        db_assignment.creation_date = as_utc(assignment.created_at)
        db_assignment.position = assignment.position
        db_assignment.group_id = group_id
        db_assignment.last_modification_date = as_utc(assignment.updated_at)

        db_assignment.can_view = (
            assignment.lock_info.can_view or not assignment.lock_info.is_locked
        )
        ...
        new_due_date = as_utc(assignment.due_at)
        ...
        if submission is not None and db_assignment.score != submission.score:
            db_assignment.score = submission.score
            actually_updated = True
            _logger.debug(
                "assignment score updated (name=%s, id=%s)",
                assignment.name,
                assignment.q_id,
            )
    ...
    async def convert_and_store_announcement(
        ...
        db_message.creation_date = as_utc(announcement.created_at)
        ...
        db_message.lock_at = as_utc(announcement.lock_at)
    ...
    async def convert_and_store_mail_item(self, mail: CourseMailItem) -> None:
        ...
        db_message.creation_date = as_utc(mail.date)
...
def _is_api_object_newer(
    ...
    if (
        ...
        return as_utc(db_object.last_modification_date) < as_utc(api_object.updated_at)
    else:
        return False
