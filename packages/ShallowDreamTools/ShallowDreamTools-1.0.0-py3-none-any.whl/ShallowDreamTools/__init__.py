...
class head:
    ...
    def set_elf(*names):
        ...
        for name in names:
            if name == "read":
                head.read_plt = head.elf.plt["read"]; head.read_got = head.elf.got["read"]
                ans.append(head.read_plt); ans.append(head.read_got)
            elif name == "puts":
                head.puts_plt = head.elf.plt["puts"]; head.puts_got = head.elf.got["puts"]
                ans.append(head.puts_plt); ans.append(head.puts_got)
            elif name == "write":
                head.write_plt = head.elf.plt["write"]; head.write_got = head.elf.got["write"]
                ans.append(head.write_plt); ans.append(head.write_got)
        return ans
