...
def test_tilted_time_lookup_batched_against_site_selection(s: int):
    S = 1 << s
...
@pytest.mark.parametrize("s", range(1, 12))
def test_tilted_time_lookup_batched_empty(s: int):
    S = 1 << s
...
def test_tilted_time_lookup_batched_fuzz(
    dtype1: typing.Type,
    dtype2: typing.Type,
    parallel: bool,
):
    ...
    validate = validate_tilted_time_lookup(algo.lookup_ingest_times_batched)
    for S in testS:
        mask1 = np.logical_or(
