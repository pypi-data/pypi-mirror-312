...
def evaluate_progress(model, val_loader, device, epoch, epochs, is_tuple_params: bool = None) -> float:
	steps = 0
	total_loss = torch.Tensor([0.0]).to(device)
	is_tuple_params = is_tuple_params if is_tuple_params is not None else isinstance(next(iter(val_loader)), (list, tuple))
	model.eval()
	with torch.no_grad():
		loop = tqdm(val_loader, desc=f"[Epoch-{epoch}/{epochs} Valid]", total=len(val_loader), colour="green")
		if hasattr(model, 'fit'):
			if is_tuple_params:
				for batch in loop:
					loss, logits = _fit(model, batch, device)
...
def evaluate_epoch(model, val_loader, device, epoch, epochs, show_progress, is_tuple_params):
	if show_progress:
		return evaluate_progress(model, val_loader, device, epoch, epochs, is_tuple_params=is_tuple_params)
	else:
...
def train_epoch_progress(model, train_loader, optimizer, device, epoch, epochs, is_tuple_params):
	steps = 0
	total_loss = torch.Tensor([0.0]).to(device)
	loop = tqdm(train_loader, desc=f"[Epoch-{epoch}/{epochs} Train]", total=len(train_loader), colour="green")
	if hasattr(model, 'fit'):
		if is_tuple_params:
			for batch in loop:
				loss = do_fit(model, batch, optimizer, device)
...
def acc_evaluate_progress(model, val_loader, device, epoch, epochs, threshold: int = 0.5, is_tuple_params: bool = None):
...
def acc_evaluate_epoch(model, val_loader, device, epoch, epochs, show_progress, is_tuple_params):
	if show_progress:
		return acc_evaluate_progress(model, val_loader, device, epoch, epochs, is_tuple_params=is_tuple_params)
	else:
	...
	if hasattr(model, 'fit'):
		if is_tuple_params:
			for batch in loop:
				...
				total_correct += correct
				steps += 1
				loop.set_postfix(Acc=f"{total_correct.item() / total:.4f}",
				                 Loss=f"{total_loss.item() / steps:.4f}",
...
def r2_loss_logits(outputs, targets):
	if isinstance(outputs, tuple):
		...
		logits = outputs
		loss = F.mse_loss(logits, targets.view(logits.size()))
	return loss, logits


def _r2_score(y_true: list[np.ndarray], y_pred: list[np.ndarray]):
...
def r2_evaluate(model, val_loader, device, is_tuple_params: bool = None):
	...
	return total_loss.item() / len(val_loader), _r2_score(labels, preds)


def r2_evaluate_progress(model, val_loader, device, epoch, epochs, is_tuple_params: bool = None):
	steps = 0
	labels, preds = [], []
	total_loss = torch.Tensor([0.0]).to(device)
	is_tuple_params = is_tuple_params if is_tuple_params is not None else isinstance(next(iter(val_loader)), (list, tuple))
		...
		if hasattr(model, 'fit'):
			if is_tuple_params:
				for batch in loop:
					...
					preds.append(logits.cpu().numpy().ravel())
					steps += 1
					loop.set_postfix(R2=f"{_r2_score(labels, preds):.4f}", Loss=f"{total_loss.item() / steps:.4f}")
			else:
				for batch in loop:
					outputs, y = _fit_y_dict(model, batch, device)
					loss, logits = r2_loss_logits(outputs, y)
	...
	return total_loss.item() / steps, _r2_score(labels, preds)


def r2_evaluate_epoch(model, val_loader, device, epoch, epochs, show_progress, is_tuple_params):
	if show_progress:
		return r2_evaluate_progress(model, val_loader, device, epoch, epochs, is_tuple_params=is_tuple_params)
	else:
	...
	if hasattr(model, 'fit'):
		...
		if is_tuple_params:
			...
			for batch in train_loader:
				...
				labels.append(label.ravel())
				preds.append(pred.ravel())
	return _r2_score(labels, preds), total_loss.item() / len(train_loader)
...
def train_epoch_progress_r2(model, train_loader, optimizer, device, epoch, epochs, is_tuple_params):
	...
	if hasattr(model, 'fit'):
		if is_tuple_params:
			for batch in loop:
				...
				labels.append(label.ravel())
				preds.append(pred.ravel())
				loop.set_postfix(R2=f"{_r2_score(labels, preds):.4f}",
				                 Loss=f"{total_loss.item() / steps:.4f}",
				                 lr=f'{optimizer.param_groups[0]["lr"]:.6f}')
		else:
			for batch in loop:
	...
	return _r2_score(labels, preds), total_loss.item() / steps
