...
from .training.utils import convert_to_tensor, convert_data, acc_predict, get_device
from .training import evaluate, Trainer, SimpleTrainer, EvalTrainer, SimpleEvalTrainer, \
...
class ModelWrapper:
	...
	def __init__(self, model_or_path: Union[nn.Module, str, Path], device: Union[str, int, torch.device] = 'auto'):
...
class ClassModelWrapper(ModelWrapper):
	...
	def predict(self, X: Union[torch.Tensor, np.ndarray, List], threshold: int = 0.5) -> np.ndarray:
		"""
		:param X:
	...
	def predict_classes(self, X: Union[torch.Tensor, np.ndarray, List], threshold: int = 0.5) -> list:
		"""
		:param X:
		:param threshold: 二分类且模型输出为一维概率时生效
		"""
	...
	def predict_proba(self,
	                  ...
	                  threshold: int = 0.5) -> Tuple[np.ndarray, np.ndarray]:
		"""
		:param X:
		:param threshold: 二分类且模型输出为一维概率时生效
		"""
	...
	@staticmethod
	def _proba(logits: torch.Tensor, threshold: int = 0.5) -> Tuple[np.ndarray, np.ndarray]:
		"""
		:param logits:
		:param threshold: 二分类且模型输出为一维概率时生效
		"""
		...
		if (shape_len == 2 and shape[1] > 1) or shape_len > 2:
			...
			if shape_len == 2:
				logits = logits.ravel()
			return np.where(logits >= threshold, 1, 0).astype(np.int64), np.where(logits >= 0.5, logits, 1 - logits)
	
	def predict_classes_proba(self,
	                          X: Union[torch.Tensor, np.ndarray, List],
	                          threshold: int = 0.5) -> Tuple[list, np.ndarray]:
		"""
		:param X:
		:param threshold: 二分类且模型输出为一维概率时生效
		"""
	...
	def evaluate(self, val_set: Dataset, batch_size=64, num_workers=0, collate_fn=None, threshold: int = 0.5) -> float:
		"""
		:param val_set:
		:param threshold: 二分类且模型输出为一维概率时生效
		:return accuracy
...
class SimpleClassModelWrapper(ClassModelWrapper):
	...
	def evaluate(self, X: Union[torch.Tensor, np.ndarray, List], y: Union[torch.LongTensor, np.ndarray, List],
	             batch_size=64, num_workers=0, collate_fn=None, threshold: int = 0.5):
		"""return accuracy"""
		X, y = convert_data(X, y)
		data_set = TensorDataset(X, y)
		return super().evaluate(data_set, batch_size, num_workers, collate_fn, threshold)
...
class SplitClassModelWrapper(SimpleClassModelWrapper):
	...
	def train(self, X: Union[torch.Tensor, np.ndarray, List], y: Union[torch.LongTensor, np.ndarray, List],
		...
		trainer = SimpleEvalClassTrainer(epochs, optimizer, scheduler, lr, T_max,
		                                 ...
		                                 checkpoint_name,
		                                 self.device)
		self.best_model, histories = trainer.train(self.model, X_train, y_train, (X_test, y_test), collate_fn, show_progress, eps)
		return histories
...
class RegressModelWrapper(ModelWrapper):
	...
	def mae(self, X: Union[torch.Tensor, np.ndarray, List[float]],
	        y: Union[torch.Tensor, np.ndarray, list[float]]) -> float:
	...
	def mse(self, X: Union[torch.Tensor, np.ndarray, List[float]],
	        y: Union[torch.Tensor, np.ndarray, list[float]]) -> float:
		if isinstance(y, list):
			y = np.array(y)
		elif torch.is_tensor(y):
	...
	def rmse(self, X: Union[torch.Tensor, np.ndarray, List[float]],
	         y: Union[torch.Tensor, np.ndarray, list[float]]) -> float:
		if isinstance(y, list):
			y = np.array(y)
		elif torch.is_tensor(y):
