...
from .utils import convert_data, convert_data_r2, get_device, is_gpu, get_workers
from ._support import evaluate_epoch, acc_evaluate_epoch, r2_evaluate_epoch, train_epoch, train_epoch_acc, train_epoch_r2
...
class Trainer:
	...
	def _get_workers(self, num_workers: int, dataset: Dataset, train: bool = True):
		return num_workers if num_workers >= 0 else get_workers(len(dataset), self.batch_size, train)
		
	def _get_pin_persistent(self, num_workers: int):
		pin_memory = self.pin_memory or (num_workers > 0 and is_gpu(self.device))
		persistent_workers = self.persistent_workers or (num_workers > 0 and is_gpu(self.device))
		return pin_memory, persistent_workers
	
	def _build_train_loader(self, train_set: Dataset, collate_fn=None):
		num_workers = self._get_workers(self.num_workers, train_set)
		pin_memory, persistent_workers = self._get_pin_persistent(num_workers)
		return DataLoader(dataset=train_set, batch_size=self.batch_size, pin_memory=pin_memory,
		                  pin_memory_device=self.pin_memory_device, persistent_workers=persistent_workers,
...
class EvalTrainer(Trainer):
	...
	def _build_val_loader(self, val_set: Dataset, collate_fn=None):
		num_eval_workers = self._get_workers(self.num_eval_workers, val_set, False)
		return DataLoader(dataset=val_set, batch_size=self.eval_batch_size,
	...
	def train(self, model, train_set: Dataset, val_set: Dataset, collate_fn=None, show_progress=False, eps=1e-5):
		train_loader = self._build_train_loader(train_set, collate_fn)
		val_loader = self._build_val_loader(val_set, collate_fn)
		self._begin_train()
		cnt = 0
		best_model = None
		min_loss = float('inf')
		...
		for epoch in range(1, self.epochs + 1):
			...
			avg_loss = train_epoch(model, train_loader, optimizer, self.device, scheduler, epoch, self.epochs,
			                       show_progress, is_tuple_params)
			val_loss = evaluate_epoch(model, val_loader, self.device, epoch, self.epochs, show_progress, is_tuple_params)
			train_losses.append(avg_loss)
	...
	def train(self, model, train_set: Dataset, collate_fn=None, show_progress=False, eps=1e-5):
		...
		for epoch in range(1, self.epochs + 1):
			acc, avg_loss = train_epoch_acc(model, train_loader, optimizer, self.device, scheduler, epoch,
			                                self.epochs, show_progress, is_tuple_params)
...
class EvalClassTrainer(EvalTrainer):
	...
	def train(self, model, train_set: Dataset, val_set: Dataset, collate_fn=None, show_progress=False, eps=1e-5):
		...
		for epoch in range(1, self.epochs + 1):
			model.train()
			acc, avg_loss = train_epoch_acc(model, train_loader, optimizer, self.device, scheduler, epoch, self.epochs,
			                                show_progress, is_tuple_params)
			val_loss, val_acc = acc_evaluate_epoch(model, val_loader, self.device, epoch, self.epochs, show_progress, is_tuple_params)
			train_accs.append(acc)
			train_losses.append(avg_loss)
...
class EvalRegressTrainer(Trainer):
	...
	def train(self, model, train_set: Dataset, val_set: Dataset, collate_fn=None, show_progress=False, eps=1e-5):
		...
		for epoch in range(1, self.epochs + 1):
			model.train()
			r2, avg_loss = train_epoch_r2(model, train_loader, optimizer, self.device, scheduler, epoch, self.epochs,
			                              show_progress, is_tuple_params)
			val_loss, val_r2 = r2_evaluate_epoch(model, val_loader, self.device, epoch, self.epochs, show_progress, is_tuple_params)
			train_r2s.append(r2)
			train_losses.append(avg_loss)
