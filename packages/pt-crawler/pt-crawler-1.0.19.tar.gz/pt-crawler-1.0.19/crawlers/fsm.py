...
class TaskItem(BaseModel):
    torrent_id: str = Field(alias='tid', coerce_numbers_to_str=True)
...
class TorrentItem(BaseModel):
    ...
    peers: Peers
    status: TorrentStatus = Field(alias='status')

    @field_validator('size', mode='before')
    def size_validator(cls, value: str) -> int: # pylint: disable=no-self-argument
    ...
    def promotion(self) -> Promotion:
        return Promotion(upload_ratio=self.status.upload_ratio, download_ratio=self.status.download_ratio) # pylint: disable=no-member
