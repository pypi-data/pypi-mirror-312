...
class SearchItemStatus(BaseModel):
    ...
    def promotion_validator(cls, value: str) -> Promotion: # pylint: disable=no-self-argument
        if value == 'FREE':
...
class SearchResponse(BaseModel):
    code: str
    message: str
    search_data: Optional[SearchData] = Field(alias='data', default=None)
...
class MTeam(Crawler):
    ...
    def get_torrents(self, pages: int = 1, mode: str = 'normal') -> List[Torrent]:
        ...
        for page in range(pages):
            ...
            search_response: SearchResponse = SearchResponse.model_validate(response.json())
    ...
    def get_user(self) -> User:
        ...
        profile_response: ProfileResponse = ProfileResponse.model_validate(response.json())
        return User(
    ...
    def get_torrent(self, torrent_id: str) -> Torrent:
        ...
        torrent_response: TorrentResponse = TorrentResponse.model_validate(response.json())
        return Torrent(
    ...
    def download_torrent(self, torrent_id: str, file_path: str) -> bool:
        ...
        di_token_response: DITokenResponse = DITokenResponse.model_validate(response.json())
    ...
    def get_tasks(self, page_size: int = 100) -> List[Task]:
        ...
        for status in [Status.SEEDING, Status.LEECHING]:
            ...
            while True:
                ...
                task_response: TaskResponse = TaskResponse.model_validate(response.json())
