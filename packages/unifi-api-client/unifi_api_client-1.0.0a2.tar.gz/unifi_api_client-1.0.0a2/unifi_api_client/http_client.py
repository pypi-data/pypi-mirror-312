...
from unifi_api_client import exceptions as api_exceptions


class HttpClient:

    def __init__(self, root_url: str, headers: any = None) -> None:
        ...
        # only retries on ConnectionError and on Transient errors when we are sure that the request has not reached UI
    ...
    def get_abs_url(self, endpoint: str) -> str:
        # remove the leading '/' because _root_url might be something like 'http://my.domain/ea/' and urljoin would then remove the '/ea'
