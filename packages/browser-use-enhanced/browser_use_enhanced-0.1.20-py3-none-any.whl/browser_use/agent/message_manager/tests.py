...
@pytest.fixture(
    ...
    ids=['gpt-4o-mini', 'gpt-4o', 'claude-3-5-sonnet', 'gemini-1.5-flash'],
)
...
def test_initial_messages(message_manager: MessageManager):
    ...
    if isinstance(message_manager.llm, ChatGoogleGenerativeAI):
        assert len(messages) == 1
        assert isinstance(messages[0], HumanMessage)
        assert 'Test actions' in messages[0].content
	...
	for i in range(200):  # Simulate 40 steps of interaction
		...
		output = AgentOutput(
			current_state=AgentBrain(
				valuation_previous_goal=f'Success in step {i}',
