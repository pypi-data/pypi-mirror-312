...
class ThreadPoolCache:
    ...
    def stop_thread_pool(task_batch_id: int):
        with ThreadPoolCache._lock:
            # 1. 发送
            thread_event = ThreadPoolCache._cache_events.pop(
                task_batch_id,
                None,
            )
...
class ExecutorManager:
    ...
    def _execute_wrapper(
        job_method: Callable,
        args: DispatchJobRequest,
    ):
        # 设置 context
        SnailContextManager.set_context(args)

        try:
            result: ExecuteResult = job_method(build_args(args))
            ...
            if args.taskType == JobTaskTypeEnum.CLUSTER:
                ThreadPoolCache.stop_thread_pool(args.taskBatchId)

        except Exception as ex:
            SnailLog.REMOTE.error(str(ex))

    @staticmethod
    def dispatch(request: DispatchJobRequest) -> ExecuteResult:
        """执行任务批次
        ...
        try:
            ...
            if executor_info is None:
                SnailLog.REMOTE.error(f"找不到执行器: {request.executorInfo}")
                return ExecuteResult.failure()
            ...
            SnailLog.REMOTE.error(f"客户端发生非预期异常. taskBatchId:[{request.taskBatchId}]")
            return ExecuteResult.failure()
