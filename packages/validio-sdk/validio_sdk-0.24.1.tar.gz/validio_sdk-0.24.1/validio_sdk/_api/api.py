...
class APIClient:
    ...
    async def execute(self, query: str, **kwargs: Any) -> Any:
        ...
        try:
            result = self._parse_result(
                await self.client.client.execute_async(graphql_query, **kwargs),
                graphql_query,
            )
    ...
    async def execute_mutation(
        ...
        return await self.execute(query, variable_values=variable_values)

    async def secrets_changed(
        self, class_name: str, variable_values: dict[str, Any]
    ) -> Any:
        """Check if secrets changed.

        Create a query and input types for the passed class name.
        :param class_name: Name of credential class, e.g. RedshiftCredential
        :param variable_values: Dictionary with secret fields
        :returns: GraphQL response containing `hasChanged`
        """
    ...
    async def infer_schema(
        ...
        return await self.execute(query, variable_values=input_variable_values)

    async def dbt_artifact_multipart_upload_create(
        self, credential_id: str, job_name: str
    ) -> str:
    ...
    async def dbt_artifact_multipart_upload_complete(self, upload_id: str) -> None:
        response = await self.execute_mutation(
    ...
    async def get_credentials(
        ...
        result = await self.execute(query, variable_values=variable_values)
    ...
    async def get_sources(
        ...
        return await get_sources(self, source_id, namespace_id)
    ...
    async def get_validators(
        ...
        if validator_id and source_id:
            raise ValidioError(
                "`source_id` is not supported when passing `validator_id`"
            )
    ...
    async def get_recommendations(self, source_id: str) -> Any:
        ...
        response = await self.execute(query, variable_values={"id": source_id})
...
async def get_sources(
    ...
    return await executor.execute(query, variable_values=variable_values)
