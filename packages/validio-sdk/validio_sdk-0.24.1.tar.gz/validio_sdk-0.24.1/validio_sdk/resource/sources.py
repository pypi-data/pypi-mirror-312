...
class Source(Resource):
    ...
    def _api_update_input(self, _namespace: str, _: "DiffContext") -> Any:
        return _api_update_input_params(
            self,
            overrides=self._api_input_overrides(),
        )
    ...
    def _decode(
        ...
        # The field is called schema both in the API and in the resource graph
        # (internally on the resource), but the constructor still calls this
        # `db_schema` so we need to rename it so we pass proper arguments.
        # TODO(VR-3817): Remove when we deprecated `db_schema`.
        _rename_dict_key(args, "schema", "db_schema")

        message_format = args.get("message_format")
        if message_format:
            mf = cast(dict[str, Any], message_format)
            _rename_dict_key(mf, "schema", "db_schema")
    ...
    def _decode_filters(
        ...
        filters_obj = cast(
            dict[str, dict[str, object]],
            children_obj[Filter.__name__] if Filter.__name__ in children_obj else {},
        )
...
class SnowflakeSource(Source):
    ...
    def __init__(
        ...
        :param db_schema: Deprecated. Replaced by `schema`.
        ...
        if db_schema is None and schema is None:
            raise ValidioError("Missing database schema")

        if db_schema is not None:
            self.add_field_deprecation("db_schema", "schema")
            schema = db_schema
...
class StreamSource(Source):
    ...
    def __init__(
        ...
        if self.message_format is not None and self.message_format._db_schema:
            self.add_deprecation(
                "Field 'db_schema' on `StreamingMessageFormat` is deprecated,"
                " please use `schema` instead"
            )
