...
def test_diff_should_detect_create_update_delete_operations_on_resources() -> None:
    ...
    manifest_w1 = FixedBatchWindow("w1", manifest_s1, "d", 10)
    manifest_w2 = FixedBatchWindow("w2", manifest_s1, "d", 20)  # To be created
    manifest_w3 = FixedBatchWindow("w3", manifest_s1, "d", 30)  # Update
    ...
    server_w1 = FixedBatchWindow("w1", server_s1, "d", 10)
    server_w3 = FixedBatchWindow("w3", server_s1, "d", 40)
    server_w4 = FixedBatchWindow("w4", server_s1, "d", 50)  # To be deleted
...
def test_diff_replace_cascade_with_conflicting_ops() -> None:
    ...
    server_w1 = TumblingWindow(
        "w1", server_s1, "d", 2, WindowTimeUnit.DAY, window_timeout_disabled=False
    )
    server_seg1 = Segmentation("seg1", server_s1, ["city"])
    server_f1 = NullFilter(name="f1", source=server_s1, field="age")
...
def test_diff_should_detect_updates_on_nested_objects(
    ...
    server_v = NumericDistributionValidator(
        ...
        reference_source_field="b",
        filter=server_f1,
        reference=Reference(server_s1, server_w1, 10, 20, filter=server_f1),
    )
