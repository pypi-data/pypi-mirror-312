...
class Reference(Diffable):
    ...
    def _filter_api_input(
        ...
        if self._filter_resource_mode():
            return {
                "filterId": ctx.filters[self.filter_name]._must_id()
    ...
    def _decode(
        ...
        obj = {
            ...
            if k not in {"source_name", "window_name", "filter_name"}
        }
...
class Validator(Resource, ABC):
    ...
    def __init__(
        ...
        if (
            ...
            and self.reference.filter is not None
    ...
    def _filter_api_input(
        ...
        if self._filter_resource_mode():
            return {
                "filterId": ctx.filters[self.filter_name]._id.value
                if self.filter_name is not None
                else None
            }
    ...
    def _api_create_input(self, _namespace: str, ctx: "DiffContext") -> Any:
        overrides = {"sourceConfig": self._source_config_create_input(ctx)}
    ...
    async def _api_update(
        self, namespace: str, client: APIClient, ctx: "DiffContext"
    ) -> None:
        # Update the validator
        await super()._api_update(namespace, client, ctx)
