...
class ProCliPlugins(LocalstackCliPlugin):
	name='pro'
	def attach(B,cli):A=cli.group;A.add_command(dns);A.add_command(aws);A.add_command(extensions);A.add_command(license);A.add_command(state);A.add_command(auth);A.add_command(pod);A.add_command(ephemeral)
