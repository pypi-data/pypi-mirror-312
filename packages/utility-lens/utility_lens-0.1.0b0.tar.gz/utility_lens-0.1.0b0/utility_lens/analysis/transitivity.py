...
class TransitivityAnalyzer:
    ...
    async def _analyze_async(self) -> Dict[str, Any]:
        ...
        '''
        pairs_to_compare = [info['pair'] for info in comparison_info]
        model_results = await self.model.batch_compare_pairs_async(pairs_to_compare, self.n_trial)
        
        # Process results (same structure as sync version)
        for info, (pair, counts, responses) in zip(comparison_info, model_results):
            if info['pair'] != pair:
                print(f"Warning: Pair mismatch detected: {info['pair']} != {pair}")
                continue
    ...
    def save_results(self, results: Dict[str, Any]):
        ...
        output_data = {
            ...
            "weak_stochastic_transitivity_satisfied": results["weak_stochastic_transitivity_satisfied"],
            "strong_stochastic_transitivity_satisfied": results["strong_stochastic_transitivity_satisfied"],
