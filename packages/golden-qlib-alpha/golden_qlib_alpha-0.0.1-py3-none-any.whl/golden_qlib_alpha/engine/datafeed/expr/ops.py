...
class Slope(Rolling):
    ...
    def _load_internal(self, instrument, start_index, end_index, *args):
        def calc_slope(x):
            x = x / x.iloc[0]  # 使用 .iloc 确保按位置访问  # 这里做了一个“归一化”
...
class KF2(Rolling):
    ...
    def __init__(self, feature, damping=1, N=1):
        super(KF2, self).__init__(feature, N, "kalman")
    ...
    def _load_internal(self, instrument, start_index, end_index, *args):
        ...
        kf=KalmanFilter(transition_matrices=[1],
                    ...
                    initial_state_mean=k1,
                    initial_state_covariance=c1,
                    )

        #先保证代码的正确性,for循环虽然慢
        result=[]
        for x in data:
            k1,c1=kf.filter_update(k1, c1, x)
            result.append(k1.data[0][0])
            #print(x,k1,c1)
        series = pd.Series(result, index=series.index)
        return series
...
def rolling_rsquare(a, window):
    ...
    for i in range(len(a)):
        if i < window - 1:
            rsquares.append(np.nan)  # not enough data points to form a window
        else:
            y = a[i - window + 1:i + 1]
            x = np.arange(window)
            slope, intercept, r_value, p_value, std_err = linregress(x, y)
            rsquares.append(r_value ** 2)
    return rsquares
