...
class DGGChat(WSBase):
    ...
    def update_flairs(self) -> dict:
        _logger.info("Updating flairs")
        self._flairs = flair_converter(self.config["flairs"])
        return self._flairs
        ...
        if event_type == EventType.ME:
            if data is not None:
                self.user = User(
                    ...
                    convert_flairs(
                        self._flairs, data.get("features"), self.update_flairs
                    ),
                )
            else:
                self.user = None
    ...
    def on_names(self, connection_count: int, users: list):
        ...
        self._users = {
            user["nick"].lower(): User(
                ...
                convert_flairs(self._flairs, user.get("features"), self.update_flairs),
            )
            for user in users
        }
