...
class mpl_editor:
    ...
    def run_server(self):
        # Niestandardowy handler, który ustawia katalog, z którego serwowane będą pliki
        class MyHandler(http.server.SimpleHTTPRequestHandler):
            def __init__(self, *args, **kwargs):
                # Ustawienie katalogu, z którego będą serwowane pliki
                kwargs['directory'] = self._package_path  # Ustawienie katalogu pakietu
                super().__init__(*args, **kwargs)

        # Pozwolenie na ponowne użycie adresu gniazda
        socketserver.TCPServer.allow_reuse_address = True
        
        # Uruchomienie serwera HTTP z niestandardowym handlerem
        httpd = socketserver.TCPServer(("", 8005), MyHandler)
        print("Serwer uruchomiony na porcie 8005...")
        httpd.serve_forever()
