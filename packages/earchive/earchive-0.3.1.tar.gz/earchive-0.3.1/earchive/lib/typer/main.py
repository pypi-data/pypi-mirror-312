...
def determine_type_convertor(type_: Any, secondary_type: Any) -> Optional[Callable[[Any], Any]]:
...
def get_click_param(
    ...
    if origin is not None:
        ...
        if is_union(origin):
            ...
            if len(types) == 1:
                ...
                main_type, secondary_type, *rest = types
                assert not len(rest) and secondary_type is bool, "Typer Currently doesn't support Union types"
    ...
    convertor = determine_type_convertor(main_type, secondary_type)
    if is_list:
