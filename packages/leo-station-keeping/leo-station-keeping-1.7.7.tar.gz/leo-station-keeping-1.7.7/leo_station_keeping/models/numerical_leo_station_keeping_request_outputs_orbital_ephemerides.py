...
class NumericalLeoStationKeepingRequestOutputsOrbitalEphemerides(BaseModel):
    ...
    """ # noqa: E501
    types: List[StrictStr] = Field(description="The types of ephemerides to generate. ")
    timestep: Union[StrictFloat, StrictInt] = Field(description="The time step (in seconds) at which the ephemerides have to be generated.  This is the time step at which the ephemerides are interpolated.  The smaller the time step, the more accurate the ephemerides but the longer the computation time. ")
    ...
    def types_validate_enum(cls, value):
        """Validates the enum"""
        for i in value:
            if i not in set(['CARTESIAN', 'KEPLERIAN', 'SYSTEM']):
                raise ValueError("each list item must be one of ('CARTESIAN', 'KEPLERIAN', 'SYSTEM')")
        return value
