...
class Battery(BaseModel):
    """
    The battery configuration. Together with the SolarArray, it is required to simulate the power system.
    """ # noqa: E501
    nominal_capacity: Union[StrictFloat, StrictInt] = Field(description="The battery nominal capacity (in Wh)", alias="nominalCapacity")
    depth_of_discharge: Union[Annotated[float, Field(le=1, strict=True, ge=0)], Annotated[int, Field(le=1, strict=True, ge=0)]] = Field(description="The battery depth of discharge [0, 1]", alias="depthOfDischarge")
    minimum_charge_for_firing: Union[Annotated[float, Field(le=1, strict=True, ge=0)], Annotated[int, Field(le=1, strict=True, ge=0)]] = Field(description="The minimum charge for firing [0, 1]", alias="minimumChargeForFiring")
    initial_charge: Optional[Union[Annotated[float, Field(le=1, strict=True, ge=0)], Annotated[int, Field(le=1, strict=True, ge=0)]]] = Field(default=1.0, description="The initial charge [0, 1]", alias="initialCharge")
    __properties: ClassVar[List[str]] = ["nominalCapacity", "depthOfDischarge", "minimumChargeForFiring", "initialCharge"]
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            ...
            "initialCharge": obj.get("initialCharge") if obj.get("initialCharge") is not None else 1.0
        })
        return _obj
