...
# ----------------------------------------------------------------------
# Past here, original GCT.  Top was copied exactly, for ease of including in my library.
...
class SetEncoder :
    ...
    def addSet ( self, ourSet ):

        outStr = ""
        for item in ourSet:
            if len(outStr) > 0:
                outStr += ITEM_DIVIDER
            outStr += ( item if item else '' )
            self.originalLength += ( len(item) if item else 0 )
        self.sets.append( outStr )
    ...
    def compressed ( self ) -> str:
        ...
        encode = ''
        for ourSet in self.sets:
            if len(encode) > 0:
                encode += SET_DIVIDER
            encode += ourSet

        encoded = self.lzString.compressToEncodedURIComponent(encode)
        encodedLength = len(encoded)
        _LOGGER.debug( f'StrCompress: encoded from original size {self.originalLength} to size {encodedLength}')
        return encoded

    def decompress ( self, fromCoded : str ) -> []:

        decoded = self.lzString.decompressFromEncodedURIComponent(fromCoded)
        sets = []
        setStrings = decoded.split( SET_DIVIDER )
        for setString in setStrings:
            ourSet = setString.split( ITEM_DIVIDER )
            asList = []
            for item in ourSet:
                if len(item) > 0:
                    asList.append( item )
