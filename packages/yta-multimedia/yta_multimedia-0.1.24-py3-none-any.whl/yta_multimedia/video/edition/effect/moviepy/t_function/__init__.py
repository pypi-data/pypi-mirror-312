...
class TFunctionSetPosition:
    ...
    """
    # TODO: This is a decorator, so move to an specific module
    def with_normal_shaking(func):
        @wraps(func)
        def wrapper(*args, **kwargs):
            # TODO: Here I have a problem. Should I get the values
            # from *args and **kwargs or should I put them as params
            # in the wrapper function (?)
            # TODO: Here I need to receive the 't', 'x', 'y'
            pos = func(*args, **kwargs)

            # The 't' value is the first one
            return shake_movement(args[0], pos[0], pos[1])
            
        return wrapper
    
    # TODO: This is actually using the 'linear' but also 
    # adding a shake effect, so I should look for another
    # way to be able to add the shake maybe as a @after
    # decorator or something similar, as an additional
    # process
    @with_normal_shaking
    def linear_with_normal_shaking(t: float, duration: float, initial_position: tuple, final_position: tuple, rate_func: type = RateFunction.linear):
        return TFunctionSetPosition.linear(t, duration, initial_position, final_position, rate_func)
        # 2nd. option is here below
    ...
    def linear_with_decreasing_shaking(t: float, duration: float, initial_position: tuple, final_position: tuple, rate_func: type = RateFunction.linear):
        ...
        # TODO: Can I make the shaking dependant on some t_function (?)
        # speed = rate_function(t % time_per_circle, time_per_circle, rate_func)
        # TODO: As you see, this could be a decorator applied @after
