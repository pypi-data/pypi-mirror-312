...
class Node(NodeParent):
    ...
    tag_prefix = {"va": "v.", "sh": "sh.", "adhyaya": "ady.", "su": "su.", "cm": "c.", "include": "inc.", "inline": "inl."}
    prop_order = ["id", "rel_id"]
    INDENT = 2

    def __init__(self, raw_data, parent=None, tag_prefix=None):
        ...
        if tag_prefix is not None:
            self.tag_prefix.update(self.__class__.tag_prefix)
            self.tag_prefix.update(tag_prefix)
...
def ikml_to_anytree(ikml_data, root_prefix=None, tag_prefix=None):
    root = Node(f"[root]", tag_prefix=tag_prefix)
    parents_stack = []
    prev_nd = root
    lvl = -1
    ...
    for line in ikml_data:
        ...
        if cur_lvl > lvl:
            ...
            try:
                ...
                break
        try:
            nd = Node(line.strip(), parent=parents_stack[-1], tag_prefix=tag_prefix)
            prev_nd = nd
        except Exception as e:
            msg = "error in ikml_to_anytree. this or prev tag: " + line
            print(msg)
...
def dict_to_anytree(ikml_dicts, root_prefix=None, tag_prefix=None):
    ...
    def _dict_to_anytree(parent, children):
        if children is not None:
            for child in children:
                ...
                node = Node(f"[{child['tag_name']}]", parent=parent, tag_prefix=tag_prefix)
                node.content = child.get("content", "")
                for p in props:
                    if p.startswith("."):
                        Node(f"[{p}] {child[p]}", parent=node, tag_prefix=tag_prefix)
                    else:
                        node[p] = child[p]
