...
def requires_authenticated() -> str:
    ...
    if access_token:
        return access_token
    return authenticate_on_browser()
...
def get_project(project_id: str, token: str | None = None):
    ...
    if not token and not (token := requires_authenticated()):
        raise NotAuthenticatedError("not authenticated")
    response = httpx.get(
        HOSTING_SERVICE + f"/v1/project/{project_id}",
        headers=authorization_header(token),
        timeout=TIMEOUT,
    )
    response.raise_for_status()
...
def get_app_status(app_id: str, token: str) -> str:
    ...
    try:
        response = httpx.get(
            HOSTING_SERVICE + f"/v1/deployments/{app_id}/status",
            headers=authorization_header(token),
        )
...
def authenticate_on_browser() -> str:
    ...
    auth_url = f"{HOSTING_UI}?request-id={request_id}"
...
def log_out_on_browser():
    ...
    with contextlib.suppress(Exception):
        delete_token_from_config()
    console.print(f"Opening {HOSTING_UI} ...")
    if not webbrowser.open(HOSTING_UI):
        console.warn(
