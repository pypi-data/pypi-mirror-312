...
class Service:
    ...
    def execute_one_task(self) -> dict | None:
        ...
        # 捕获服务错误
            ...
            self.__mqtt.send_task_event(task_info.uid, task_info.task_id, MqttEventType.Fail, e.message, e.code)
            return {"msg": str(e)}
        except Exception as e:
            traceback.print_exc()
            self.__api.task_update_task(task_info.task_id, status=TaskStatus.Fail, extra=str(e))
            self.__mqtt.send_task_event(task_info.uid, task_info.task_id, MqttEventType.Fail, str(e), -1)
            return {"msg": str(e)}
