...
# Parameterdefinition
a = 4     # Länge in x-Richtung (m)
b = 4.2   # Breite in y-Richtung (m)
t = 0.012      # Dicke (m)
E = 210000     # Elastizitätsmodul (MPa)
nu = 0.3      # Poissonzahl

# Lineare Variation der Normalspannungen
Nx0 =  245*t      # Normalkraft bei x=0 (MN/m)
Nx1 = 0 # -200  * t     # Varation nx über y
Ny0 = 0     # ny bei y=0 (MN/m)
Ny1 = 0     # Variaton ny über x
Nxy = 0       # Schubkraft pro Längeneinheit (MN/m)

# Indizes der Ansatzfunktionen
max_mode = 15
m_terms = list(range(1, max_mode + 1))
n_terms = list(range(1, max_mode + 1))

# Positionen und Flächenmomente der Quersteifen
x_s_positions = [0.25 * b, 0.5 * b, 0.75 * b]
I_s_values = [0, 0, 0]
A_s_values = [0, 0, 0]

# Positionen und Flächenmomente der Längssteifen
y_s_positions = [0.15*b, 0.30*b, 0.50*b,0.70 * b, 0.85 * b]
I_t_values = [11308 * 0.01**4, 11308 * 0.01**4, 11308 * 0.01**4, 11308 * 0.01**4, 11308 * 0.01**4]
A_t_values = [131.91 * 0.01**2, 131.91 * 0.01**2, 131.91 * 0.01**2, 131.91 * 0.01**2, 131.91 * 0.01**2]

# Instanziierung der Klasse
plate = PlateBucklingRitz(
    a,
    b,
    t,
    E,
...
eigenvalues, eigenvectors = plate.solve_eigenvalue_problem()

# Ausgabe der kleinsten kritischen Last und des zugehörigen Eigenvektors
lambda_cr, eigenvector_first = plate.get_critical_load()
print(f"Kleinste kritische Last (erste Eigenform): {lambda_cr:.4f}")

# Beulform für die erste Eigenform berechnen
X_first, Y_first, W_first = plate.get_buckling_mode(eigenvector=eigenvector_first)

# Filtern des m=1 Modus
lambda_cr_m1, eigenvector_m1 = plate.get_critical_load_m1_mode(threshold=0.9)

if lambda_cr_m1 is not None:
    print(f"Kritische Last für m=1 Modus: {lambda_cr_m1:.4f}")
    # Beulform für den m=1 Modus berechnen
    X_m1, Y_m1, W_m1 = plate.get_buckling_mode(eigenvector=eigenvector_m1)
else:
    print("Kein dominanter m=1 Modus gefunden.")
    X_m1, Y_m1, W_m1 = None, None, None
...
# Dateiname
Speichern = "Normalspannung_Konst_IT_025_1000e-4_100e-2.eps"

import os
# Basisverzeichnis
basisverzeichnis = "C:/Users/Thoma/OneDrive/KIB/Formelsammlungen_Brueckenbau/Baumechanik/Bilder/Unterkapitel_Plattenbeulen/Berechnungsbeispiele"

# Überprüfen, ob das Verzeichnis existiert, und ggf. erstellen
os.makedirs(basisverzeichnis, exist_ok=True)

# Vollständiger Dateipfad
dateipfad = os.path.join(basisverzeichnis, Speichern)

# # Speichern der Abbildung
# plt.savefig(dateipfad, format="eps")

plt.show(block=False)  # Nicht blockierend anzeigen
plt.pause(1)  # Warten Sie 1 Sekunde
plt.close(fig)  # Schließen Sie die Figur


plate.write_latex_output(
    "Dokumentation/File.tex",
    include_k_matrix=True,
    include_g_matrix=True,
    matrix_size=5,
)


# Optional: Kritische Beulspannungen berechnen
sigma_e = np.pi**2 * E / (12 * (1 - nu**2)) * (t / b) ** 2
print(f"sigma_e: {sigma_e:.4f} MPa")

print("Erste Eigenform:")
print(f"lambda_cr", lambda_cr)
print(f"k_sigma: {lambda_cr * Nx0 / t / sigma_e:.4f}")
print(f"Kritische Beulspannung [MPa] (sigma_x): {lambda_cr * Nx0 / t:.2f}")

if lambda_cr_m1 is not None:
    print("\nDominanter m=1 Modus:")
    print(f"lambda_cr, m=1", lambda_cr_m1)
    print(f"k_sigma: {lambda_cr_m1 * Nx0 / t / sigma_e:.4f}")
    print(f"Kritische Beulspannung [MPa] (sigma_x): {lambda_cr_m1 * Nx0 / t:.2f}")
