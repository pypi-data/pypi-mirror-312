...
__all__=["c_recvemail","c_recvexchange"]
...
class c_recvemail(object):  #接收邮件，当前只接收附件
    ...
    def main(self):
        ...
        print(p.getwelcome().decode())
        p.user(self.mailaddr)   #向服务器输出用户名
        p.pass_(self.mailpass)  #向服务器输出密码
        print("messages: %s. size: %s" % p.stat())  #邮件数量，总大小
        resp,mails,octets=p.list()
        js=len(mails)
        idx=js+1
        while idx>1:    #从最新的邮件开始
            idx=idx-1
            self.uid=p.uidl(idx).split()[2] #邮件的id号，用来跳过接收出错的邮件
            print("--------------------------------\n序号：%d uid=%s" %(idx,self.uid))
            try:
            ...
            self.mailsender=self.getsender(msg_content) #解析得到发件人
            self.maildate=self.readmaildate(msg_content)
            if self.handle_head():  #读入文件头之后的处理
                continue
    ...
    def decode_str(self,s):
        ...
        for value,charset in decode_header(s):
            ...
            jg.append(value)
        return jg
    def getsender(self,msg_content):#邮件解析，返回发送者
        msg=Parser().parsestr(msg_content)
        self.bt=self.jmt(msg,"Subject")[0]  #标题
        fjr,dz=self.jmt(msg,"From")     #发件人名称，发件人地址
        return dz.lower()
    def handle_af_attfile(self):    #处理附件之后
        pass
    def handle_bf_attfile(self):    #处理附件之前
        print("附件文件名:%s" %(self.attfile))
#		self.savename=self.attfile
    def handle_delmail(self):   #处理删除邮件，默认并不删除
        ...
        if (datetime.date.today()-datetime.timedelta(days=self.deloldmail)).strftime("%Y%m%d")>self.maildate:
            print("邮件时间较长，删除")
#			p.dele(idx)
    def handle_head(self):  #读入邮件头之后的处理
        print("发件人：%s" %(self.mailsender))
        print("邮件日期: %s" %(self.maildate))
        return True
