...
''').strip()
    ...
    def wrun_chrome(name='default_chrome', ports=None, tail=''):
        ...
        print('run chrome!')
    def wrun_v_chrome(name='default_chrome', ports=None, tail=''):
        if not ports:
            ports = ["18089:18089"]
        ports = ' '.join(['-p' + i for i in ports])
        wstop_docker(name)
...
def install(imagename, force=False):
    ...
    if imagename == "openresty" or imagename == 'openresty/openresty:latest': 
        ...
        if not fpath: return
        update_a_docker(workspace, 'openresty/openresty:latest', fpath, 'openresty.tar.xz', ziptype='xz', force=force)
        ...
        if argv[1] == 'v_chrome':
            if len(argv) > 2:
                ...
                if argv[2] == "remove":
                    ...
                    return
                if argv[2] == "list":
                    vvv.wrun_cmd('docker ps -a --filter "ancestor=v2_chrome:latest"')
                    return
                if argv[2] == "log":
