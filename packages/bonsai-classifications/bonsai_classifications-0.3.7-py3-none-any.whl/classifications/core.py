                        ...
                        if other_classifications:
                            for schema, codes_list in other_classifications.items():
                                if schema in attr_name:
                                    df2[f"tree_{schema}"] = codes_list

                            df2["tree_bonsai"] = new_code["code"]
                            df2["comment"] = ""
                            df2["prefixed_id"] = ""
                            df2["skos_uri"] = ""
...
class Resources:
    def __init__(self, path_to_resource):
        ...
        for file_name, df in dfs.items():
            ...
            # elif isinstance(df, pd.DataFrame) and "resources" in file_name:
            #    for index, row in df.iterrows():
            #        if "name" not in row:
            #            raise ValueError("wrong")
            #        _name = row["name"]
            #        if ("tree_" in _name or "dim_" in _name) and "level" not in _name:
