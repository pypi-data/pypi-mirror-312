...
def create_field_kwargs(field_info, field_type_map):
    ...
    if field_base is SectionField or field_base is SectionManyField:
        return field_base, field_info.get("section", {}).get("fields", [])
...
def create_dynamic_serializer(
    ...
    for field_info in fields_config:
        ...
        fields[field_info["field_name"]] = (
            create_dynamic_serializer(fields_config=field_kwargs, base_class=field_base)
            if field_base is SectionField or field_base is SectionManyField
            else field_base(**field_kwargs)
        )
