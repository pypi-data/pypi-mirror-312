...
from swifty.cache.manager import cache
...
class SwiftyDBSessionMixin(SwiftyLoggerMixin):
    ...
    def update_or_create_model_data(
        ...
        """
        valid_columns = self.valid_columns(model)
        instance = self.session.query(model).filter_by(**pk_filter)
        updated_status = instance.update(
            values=self.valid_data(valid_columns, {**data, **(extra_data_dict or {})})
        )
        if updated_status:
    ...
    def create_model_data(
        ...
        """
        valid_columns = valid_columns or self.valid_columns(model)
        if isinstance(data, dict):
            instance = model(
                **self.valid_data(valid_columns, {**data, **(extra_data_dict or {})})
            )
            ...
            instances = [
                model(
                    **self.valid_data(
                        valid_columns, {**sub_data, **(extra_data_dict or {})}
                    )
                )
