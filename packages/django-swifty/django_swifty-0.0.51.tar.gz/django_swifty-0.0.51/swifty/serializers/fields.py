...
class BasicFieldSerializer:
    ...
    def expressionator(self) -> ExpressionatorManager:
        ...
        """
        # expressionator = getattr(self.root, "expressionator", ExpressionatorManager)
        root = self.parent or self.root
        expressionator = (
            ExpressionatorManager
            if isinstance(root, BasicFieldSerializer)
            else getattr(root, "expressionator", ExpressionatorManager)
        )
        if callable(expressionator):
            return expressionator(data=getattr(root, "initial_data", {}))
...
class BaseSectionField(serializers.JSONField):
    ...
    def to_internal_value(self, data):
        ...
        return super().to_internal_value(json.dumps(data, default=json_handler))
