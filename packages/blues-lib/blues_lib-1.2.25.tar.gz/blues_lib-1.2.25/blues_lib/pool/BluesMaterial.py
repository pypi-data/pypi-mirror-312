...
class BluesMaterial(BluesFilePool):
  ...
  REQUIRED_BRIEF_FIELDS = ['material_id','material_site','material_url','material_title']
  REQUIRED_DETAIL_FIELDS = ['material_body']
  REQUIRED_TOTAL_FIELDS = ['material_id','material_site','material_thumbnail','material_url','material_title','material_body']
  ...
  def get_download_thumbnail(cls,material):
    ...
    if not material_thumbnail:
      BluesConsole.info('Has no thumbnail')
      return None
    else:
      ...
      if download_image:
        ...
        BluesConsole.info('Download thumbnail failure %s' % material_thumbnail)
        return None
  ...
  def is_legal_brief(cls,entity):
    if not entity:
      return False
    if 'material_thumbnail' not in entity:
      entity['material_thumbnail'] = ''
