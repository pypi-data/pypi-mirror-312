...
def pointing_misalignment_loss(d, sigma_s, pt):
    return pt * math.exp(-(d * d) / (2 * sigma_s**2))
...
def calculate_photocurrent(p_r, responsivity):
    """
...
def plot_SNR(p_r, SNR):
    ...
    plt.plot(p_r * 1e3, 10 * np.log10(SNR), label='SNR')
...
def plot_beam_divergence(w_0, wl, d_range, num_points):
    """
    ...
    w_d = w_0 * np.sqrt(1 + (wl * d / (np.pi * w_0**2))**2)
...
def los_channel_gain(theta_d, p_t, Adet, Ts, index, FOV, lx, ly, lz, xr, yr, zr, xt, yt, zt):
    """
    Calculate LOS channel gain and received power, and visualize as a 3D plot.

    Parameters:
    theta_d : float
        Semi-angle at half power (in degrees).
    p_t : float
        Transmitted optical power by the LED (in watts).
    Adet : float
        Detector physical area of a photodetector (in square meters).
    Ts : float
        Gain of an optical filter (default is 1 if no filter is used).
    index : float
        Refractive index of a lens at a photodetector (default is 1.5 if no lens is used).
    FOV : float
        Field of View of a receiver (in radians).
    lx, ly, lz : float
        Room dimensions (in meters).
    xr, yr, zr : float
        Coordinates of the receiver plane.
    xt, yt, zt : float
        Coordinates of the transmitter plane.

    Returns:
    None
        Displays the received power distribution as a 3D plot.
    """

    # Lambertian order of emission
    m = -np.log10(2) / np.log10(np.cos(np.deg2rad(theta_d)))

    # Distance between the transmitter and receiver
    h = d(xt, yt, zt, xr, yr, zr)

    # Gain of an optical concentrator
    G_Con = (index ** 2) / np.sin(FOV)

    # Define receiver plane grid
    Nx = int(lx * 10)  # Grid points in the X direction
    Ny = int(ly * 10)  # Grid points in the Y direction
    x = np.linspace(-lx / 2, lx / 2, Nx)
    y = np.linspace(-ly / 2, ly / 2, Ny)
    XR, YR = np.meshgrid(x, y)

    # Distance vector from the source to each point on the grid
    D1 = np.sqrt((XR - xt) ** 2 + (YR - yt) ** 2 + h ** 2)

    # Angle vector
    cosphi_A1 = h / D1

    # Ensure valid angles (avoid divide-by-zero issues)
    cosphi_A1 = np.clip(cosphi_A1, 0, 1)

    # Channel DC gain for the source
    H_A1 = (m + 1) * Adet * cosphi_A1 ** (m + 1) / (2 * np.pi * D1 ** 2)

    # Received power from the source
    P_rec = p_t * H_A1 * Ts * G_Con

    # Handle non-positive received power gracefully
    P_rec = np.clip(P_rec, 1e-10, None)  # Avoid log of zero or negative numbers
    ...
    surf = ax.plot_surface(XR, YR, P_rec_dBm, cmap='viridis', edgecolor='none')
    ax.set_xlabel('X (m)')
    ax.set_ylabel('Y (m)')
    ax.set_zlabel('Received power (dBm)')
    ax.set_title('Received Power Distribution (dBm)')
    fig.colorbar(surf, ax=ax, label='Received Power (dBm)')
    plt.show()

def optical_power_distribution(p_t, rho, lx, ly, lz, FOV, Adet, Ts, G_Con,
                               theta):
    """
    ...
    P_rec_A1 = (h1 + h2 + h3 + h4) * p_t * Ts * G_Con
...
def calculate_Drms_3D(C, theta_d, p_t, Adet, rho, Ts, index, FOV, lx, ly,
                       lz, delta_t):
        # Lambertian order of emission
        m = -np.log10(2) / np.log10(np.cos(np.radians(theta_d)))
        ...
        return mean_delay, Drms, x, y, z, theta_d, m, p_t, rho, FOV, TP1
