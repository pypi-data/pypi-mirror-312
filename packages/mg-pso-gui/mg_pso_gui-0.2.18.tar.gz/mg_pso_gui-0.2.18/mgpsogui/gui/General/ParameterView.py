...
class ParameterView(CTkScrollableFrame):
    ...
    def render(self):
        ...
        for key_value_pair in self.key_values:
            ...
            if self.edit_mode:
                type_menu = CTkOptionMenu(self.containerFrame, variable=self.key_values[index]["type"], values=["integer", "float", "date", "boolean", "string"])
                type_menu.grid(row=row, column=1, columnspan=2, padx=(0, 0), pady=(5, 5), sticky="ew")

                dest_menu = CTkOptionMenu(self.containerFrame, variable=self.key_values[index]["destination"], values=["calibration_parameters", "param"])
                dest_menu.grid(row=row, column=3, columnspan=2, padx=(0, 0), pady=(5, 5), sticky="ew")
