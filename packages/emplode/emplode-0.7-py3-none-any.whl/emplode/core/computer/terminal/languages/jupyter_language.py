    ...
    def _execute_code(self, code, message_queue):
        def iopub_message_listener():
            ...
            while True:
                ...
                # For async usage
                if (
                    hasattr(self.computer.emplode, "stop_event")
                    and self.computer.emplode.stop_event.is_set()
                ):
                    self.km.interrupt_kernel()
                ...
                try:
                    input_patience = int(
                        os.environ.get("EMPLODE_TERMINAL_INPUT_PATIENCE", 15)
                    )
                    if (
                        ...
                        params = {
                            "messages": messages,
                            "model": self.computer.emplode.llm.model,
                            "stream": True,
                            "temperature": 0,
                        }
                        if self.computer.emplode.llm.api_key:
                            params["api_key"] = self.computer.emplode.llm.api_key
...
def preprocess_python(code):
    ...
    if (
        ...
        and os.environ.get("EMPLODE_ACTIVE_LINE_DETECTION", "True").lower()
        == "true"
    ):
