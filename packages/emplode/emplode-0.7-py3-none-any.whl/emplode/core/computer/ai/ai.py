...
def fast_llm(llm, system_message, user_message):
    old_messages = llm.emplode.messages
    old_system_message = llm.emplode.system_message
    try:
        llm.emplode.system_message = system_message
        llm.emplode.messages = []
        response = llm.emplode.chat(user_message)
    finally:
        llm.emplode.messages = old_messages
        llm.emplode.system_message = old_system_message
...
class Ai:
    ...
    def chat(self, text, base64=None):
        ...
        response = ""
        for chunk in self.computer.emplode.llm.run(messages):
            if "content" in chunk:
                response += chunk.get("content")
        return response

        old_messages = self.computer.emplode.llm.emplode.messages
        old_system_message = self.computer.emplode.llm.emplode.system_message
        old_import_computer_api = self.computer.import_computer_api
        old_execution_instructions = (
            self.computer.emplode.llm.execution_instructions
        )
        try:
            self.computer.emplode.llm.emplode.system_message = (
                "You are an AI assistant."
            )
            self.computer.emplode.llm.emplode.messages = []
            self.computer.import_computer_api = False
            self.computer.emplode.llm.execution_instructions = ""

            response = self.computer.emplode.llm.emplode.chat(text)
        finally:
            self.computer.emplode.llm.emplode.messages = old_messages
            self.computer.emplode.llm.emplode.system_message = (
                old_system_message
            )
    ...
    def query(self, text, query, custom_reduce_query=None):
        ...
        chunks = split_into_chunks(
            text, chunk_size, self.computer.emplode.llm, overlap
        )

        # (Map) Query each chunk
        responses = query_map_chunks(chunks, self.computer.emplode.llm, query)

        # (Reduce) Compress the responses
        response = query_reduce_chunks(
            responses, self.computer.emplode.llm, chunk_size, custom_reduce_query
        )
