...
def server(emplode, host="0.0.0.0", port=8000):
    ...
    async def stream_endpoint(request: Request) -> Response:
        async def event_stream() -> Generator[str, None, None]:
            ...
            for response in emplode.chat(message=data["message"], stream=True):
                yield response
    ...
    print(
        "\nOpening a simple `emplode.chat(data)` POST endpoint at http://localhost:8000/chat."
    )
    print(
        "Opening an `e.protocol` compatible WebSocket endpoint at http://localhost:8000/."
    )
