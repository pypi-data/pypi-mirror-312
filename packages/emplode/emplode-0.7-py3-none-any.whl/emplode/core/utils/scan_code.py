...
def scan_code(code, language, emplode):
    """
    Scan code with semgrep
    """
    language_class = emplode.computer.terminal.get_language(language)

    temp_file = create_temporary_file(
        code, language_class.file_extension, verbose=emplode.verbose
    )
    ...
    try:
        ...
        if scan.returncode == 0:
            ...
            print(
                f"  {'Code Scanner: ' if emplode.safe_mode == 'auto' else ''}No issues were found in this {language_name} code."
            )
            print("")
    ...
    cleanup_temporary_file(temp_file, verbose=emplode.verbose)
