...
1. Running `emplode --disable_telemetry` in command line.
2. Executing `emplode.disable_telemetry = True` in Python.
...
def get_or_create_uuid():
    try:
        uuid_file_path = os.path.join(
            os.path.expanduser("~"), ".cache", "emplode", "telemetry_user_id"
        )
        os.makedirs(
...
def send_telemetry(event_name, properties=None):
    if properties is None:
        properties = {}
    properties["em_version"] = pkg_resources.get_distribution(
        "emplode"
    ).version
    try:
