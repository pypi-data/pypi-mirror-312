...
def terminal_interface(emplode, message):
    ...
    # If (len(emplode.messages) == 1), they probably used the advanced "i {command}" entry, so no message should be displayed.
    if (
        not emplode.auto_run
        and not emplode.offline
        and not (len(emplode.messages) == 1)
    ):
        emplode_intro_message = [
            "**Emplode** will require approval before running code."
        ]

        if emplode.safe_mode == "ask" or emplode.safe_mode == "auto":
            if not check_for_package("semgrep"):
                emplode_intro_message.append(
                    f"**Safe Mode**: {emplode.safe_mode}\n\n>Note: **Safe Mode** requires `semgrep` (`pip install semgrep`)"
                )
        else:
            emplode_intro_message.append("Use `emplode -y` to bypass this.")

        if (
            not emplode.plain_text_display
        ):  # A proxy/heuristic for standard in mode, which isn't tracked (but prob should be)
            emplode_intro_message.append("Press `CTRL-C` to exit.")

        emplode.display_message("\n\n".join(emplode_intro_message) + "\n")
    ...
    while True:
        if interactive:
            if (
                len(emplode.messages) == 1
                and emplode.messages[-1]["role"] == "user"
                and emplode.messages[-1]["type"] == "message"
            ):
                # They passed in a message already, probably via "i {command}"!
                message = emplode.messages[-1]["content"]
                emplode.messages = emplode.messages[:-1]
            else:
                ### This is the primary input for Emplode.
                try:
                    message = (
                        cli_input("> ").strip()
                        if emplode.multi_line
                        else input("> ").strip()
                    )
        ...
        if isinstance(message, str):
            ...
            if message.startswith("%") and interactive:
                handle_magic_command(emplode, message)
                continue

            # Many users do this
            if message.strip() == "emplode --local":
                print("Please exit this conversation, then run `emplode --local`.")
                continue
            if message.strip() == "pip install --upgrade emplode":
                print(
                    "Please exit this conversation, then run `pip install --upgrade emplode`."
                )
                continue

            if (
                emplode.llm.supports_vision
                or emplode.llm.vision_renderer != None
            ):
                ...
                if image_path:
                    # Add the text emplode's message history
                    ...
                    # Pass in the image to emplode in a moment
                    message = {
                        "role": "user",
                        "type": "image",
                        "format": "path",
        ...
        try:
            for chunk in emplode.chat(message, display=False, stream=True):
                yield chunk
                ...
                if chunk["type"] == "confirmation":
                    if not emplode.auto_run:

                        # End the active code block so you can run input() below it
                        if active_block and not emplode.plain_text_display:
                        ...
                        if not emplode.safe_mode == "off":
                            if emplode.safe_mode == "auto":
                                should_scan_code = True
                            elif emplode.safe_mode == "ask":
                                response = input(
                        ...
                        if should_scan_code:
                            scan_code(code, language, emplode)

                        if emplode.plain_text_display:
                            response = input(
                        ...
                        if response.strip().lower() == "y":
                            ...
                            active_block = CodeBlock(emplode)
                            ...
                            emplode.messages[-1]["content"] = code  # Give it code
                ...
                if chunk["type"] == "message":
                    ...
                    if "end" in chunk and emplode.os:
                        last_message = emplode.messages[-1]["content"]
                        ...
                        emplode.computer.os.notify(sanitized_message)

                        # Speak message aloud
                        if platform.system() == "Darwin" and emplode.speak_messages:
                            if voice_subprocess:
                ...
                if (
                    ...
                    if (emplode.os == True) and (emplode.verbose == False):
                    ...
                    if (
                        emplode.messages[-1].get("format") != "output"
                        or emplode.messages[-1]["role"] != "computer"
                        or emplode.messages[-1]["type"] != "console"
                    ):
                        ...
                        emplode.messages[-1]["content"] += (
                            "\n" + extra_computer_output
                        )
                        emplode.messages[-1]["content"] = emplode.messages[-1][
                            "content"
                        ].strip()
                ...
                if chunk["type"] == "console":
                    ...
                    if "format" in chunk and chunk["format"] == "active_line":
                        ...
                        if emplode.os and active_block.active_line != None:
                            action = ""
                            ...
                            if action.startswith("computer"):
                                ...
                                if description:
                                    emplode.computer.os.notify(description)
