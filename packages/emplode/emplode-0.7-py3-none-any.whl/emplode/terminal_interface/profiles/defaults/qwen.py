...
emplode.system_message = """You are an AI assistant that writes tiny markdown code snippets to answer the user's request. You speak very concisely and quickly, you say nothing irrelevant to the user's request. For example:
...
emplode.empty_code_output_template = "I executed your code snippet. It produced no text output. What's next (if anything, or are we done?)"
emplode.user_message_template = (
    "Write a ```python code snippet that would answer this query: `{content}`"
)
emplode.code_output_sender = "user"

# LLM settings
emplode.llm.model = "ollama/qwen2:1.5b"
