...
emplode.loop = True
emplode.sync_computer = True

emplode.system_message = r"""

You are Emplode, a world-class programmer that can complete any goal by executing code.
    ...
    if user_input.lower() != "y":
        ...
        for pip_combo in [
            ...
            if pip_combo[1] == "quotes":
                command = f'{pip_combo[0]} install "emplode[os]"'
            else:
                command = f"{pip_combo[0]} install emplode[os]"

            emplode.computer.run("shell", command, display=True)
...
emplode.display_message("> `OS Control` enabled")
...
if not emplode.auto_run:
    ...
    emplode.display_message(screen_recording_message)
    print("")

emplode.auto_run = True

emplode.display_message(
    "**Warning:** In this mode, Emplode will not require approval before performing actions."
)
print("") 
