...
from ..utils.em_dir import em_dir
from .historical_profiles import historical_profiles

profile_dir = os.path.join(em_dir, "profiles")
user_default_profile_path = os.path.join(profile_dir, "default.yaml")

here = os.path.abspath(os.path.dirname(__file__))
em_default_profiles_path = os.path.join(here, "defaults")
default_profiles_paths = glob.glob(os.path.join(em_default_profiles_path, "*"))
...
def profile(emplode, filename_or_url):
    ...
    return apply_profile(emplode, profile, profile_path)


def get_profile(filename_or_url, profile_path):
    shortcuts = ["e.com/", "www.e.com/", "https://e.com/", "http://e.com/"]
    for shortcut in shortcuts:
        if filename_or_url.startswith(shortcut):
            filename_or_url = filename_or_url.replace(
                shortcut, "https://emplode.com/profiles/"
            )
    ...
    response.raise_for_status()
    if extension == ".py":
        return {"start_script": response.text, "version": EM_VERSION}
    elif extension == ".json":
...
class RemoveEmplode(ast.NodeTransformer):
    """Remove `from emplode import emplode` and `emplode = Emplode()`"""

    def visit_ImportFrom(self, node):
        if node.module == "emplode":
            for alias in node.names:
                if alias.name == "emplode":
                    return None
        return node
...
def apply_profile(emplode, profile, profile_path):
        ...
        if message.lower() == "y":
            ...
            if profile_path.endswith("default.yaml"):
                with open(profile_path, "r") as file:
                    text = file.read()
                text = text.replace(
                    "version: " + str(profile["version"]), f"version: {EM_VERSION}"
                )
    ...
    if "computer" in profile and "languages" in profile["computer"]:
        ...
        emplode.computer.languages = [
            i
            for i in emplode.computer.languages
            if i.name.lower() in [l.lower() for l in profile["computer"]["languages"]]
        ]
        del profile["computer.languages"]

    apply_profile_to_object(emplode, profile)
...
def migrate_profile(old_path, new_path):
    ...
    old_system_messages = [
        """You are Emplode Agent, a world-class programmer developed by Emplode that can complete any goal by executing code.
...
You are capable of **any** task.""",
        """You are Emplode Agent, a world-class programmer developed by Emplode that can complete
...
def open_storage_dir(directory):
    dir = os.path.join(em_dir, directory)
...
def get_default_profile(specific_default_profile):
    for default_yaml_file in default_profiles_paths:
        ...
        profile_path = os.path.join(em_default_profiles_path, filename)
...
def determine_user_version():
    # Pre 0.2.0 directory
    old_dir_pre_020 = platformdirs.user_config_dir("Emplode")
    # 0.2.0 directory
    old_dir_020 = platformdirs.user_config_dir("Emplode Terminal")

    if os.path.exists(em_dir) and os.listdir(em_dir):
        # Check if the default.yaml profile exists and has a version key
        default_profile_path = os.path.join(em_dir, "profiles", "default.yaml")
...
def migrate_app_directory(old_dir, new_dir, profile_dir):
    ...
    # After all migrations have taken place, every yaml file should have a version listed. Sometimes, if the user does not have a default.yaml file from 0.2.0, it will not add the version to the file, causing the migration message to show every time  is launched. This code loops through all yaml files post migration, and ensures they have a version number, to prevent the migration message from showing.
...
def migrate_user_app_directory():
    ...
    if user_version == "pre_0.2.0":
        old_dir = platformdirs.user_config_dir("Emplode")
        migrate_app_directory(old_dir, em_dir, profile_dir)

    elif user_version == "0.2.0":
        old_dir = platformdirs.user_config_dir("Emplode Terminal")
        migrate_app_directory(old_dir, em_dir, profile_dir)
