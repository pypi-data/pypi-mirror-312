...
from emplode.terminal_interface.profiles.profiles import write_key_to_profile
from emplode.terminal_interface.utils.display_markdown_message import (
    display_markdown_message,
)

contribute_cache_path = os.path.join(
    os.path.expanduser("~"), ".cache", "emplode", "contribute.json"
)
...
def display_contribution_message():
    display_markdown_message(
        """
---
> We're fine-tuning an open-source language model.

Want to contribute? Run `emplode --model e` to use our free, hosted model. Conversations with this `e` model will be used for fine tuning.
...
def display_contributing_current_message():
    display_markdown_message(
        f"""
---
> This conversation will be used to fine tune an open-source language model.
"""
    )


def send_past_conversations(emplode):
    past_conversations = get_all_conversations(emplode)
    if len(past_conversations) > 0:
        print()
        print(
            "We are about to send all previous conversations to Emplode for fine tuning an open-source language model. Please make sure these don't contain any private information. Run `emplode --conversations` to browse them."
        )
        print()
        time.sleep(2)
        uh = input(
            "Do we have your permission to send all previous conversations to Emplode? (y/n): "
        )
        print()
        if uh == "y":
            print("Sending all previous conversations to Emplode...")
            contribute_conversations(past_conversations)
            print()


def set_send_future_conversations(emplode, should_send_future):
    write_key_to_profile("contribute_conversation", should_send_future)
    display_markdown_message(
        """
> Emplode will contribute conversations from now on. Thank you for your help!

To change this, run `emplode --profiles` and edit the `default.yaml` profile so "contribute_conversation" = False.
"""
    )
...
def contribute_conversation_launch_logic(emplode):
    contribution_cache = get_contribute_cache_contents()

    if emplode.will_contribute:
        contribute_past_and_future_logic(emplode, contribution_cache)
...
def contribute_past_and_future_logic(
    ...
    if not contribution_cache["asked_to_contribute_future"]:
        if user_wants_to_contribute_future():
            set_send_future_conversations(emplode, True)
...
def get_all_conversations(emplode) -> List[List]:
    def is_conversation_path(path: str):
        _, ext = os.path.splitext(path)
        return ext == ".json"

    history_path = emplode.conversation_history_path
...
def contribute_conversations(
    ...
    url = "https://api.emplode.com/v1/contribute/"
