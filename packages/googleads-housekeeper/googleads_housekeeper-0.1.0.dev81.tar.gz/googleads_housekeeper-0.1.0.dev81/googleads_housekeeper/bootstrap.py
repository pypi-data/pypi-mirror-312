...
# pylint: disable=C0330, g-bad-import-order, g-import-not-at-top
...
deployment_type = os.getenv('ADS_HOUSEKEEPER_DEPLOYMENT_TYPE')
logging.info('TempDebug: '
             'ADS_HOUSEKEEPER_DEPLOYMENT_TYPE: %s', deployment_type)
if True:
  try:
    pass
...
class GoogleCloudBootstrapper(Bootstrapper):
  def __init__(
    ...
    logging.info( 'TempDebug: project_id %s', project_id)
    self.start_orm = False
