...
def getOllamaServerClient(server="tool"):
    if server=="chat" and config.ollamaChatHost_localIp:
        return f"http://{get_local_ip()}:{config.ollamaChatServer_port}"
    elif server=="tool" and config.ollamaToolHost_localIp:
        return f"http://{get_local_ip()}:{config.ollamaToolServer_port}"
    return Client(host=f"{config.ollamaChatServer if server=='chat' else config.ollamaToolServer}:{config.ollamaChatServer_port if server=='chat' else config.ollamaToolServer_port}")
