...
`sd` install `stable-diffusion-cpp-python` to work with tools `create images with Flux1` and `create images with Stable Diffusion`

> pip install --upgrade toolmate[sd]

`bible` install additional libraries to work with bible tools
