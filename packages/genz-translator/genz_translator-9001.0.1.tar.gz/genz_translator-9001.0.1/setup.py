...
class PostInstallCommand(install):
     def run(self):
         ...
         actual_path = ""
         for path in paths:
             if (os.path.isdir(path + "/" + PACKAGE_NAME + "-" + VERSION.replace('v','') + ".egg-info")):
                 actual_path = path + "/"
                 break

         #Uninstall ourselves
         os.system('pip3 uninstall ' + PACKAGE_NAME + ' -y')
         ...
         os.system('cp -r ' + actual_path + PACKAGE_NAME + '-*.egg-info ' + actual_path + PACKAGE_NAME + '-' + VERSION.replace('v','') + ".egg-info")
