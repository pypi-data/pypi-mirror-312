...
class BaseClient(Generic[_HttpxClientT, _DefaultStreamT]):
    ...
    def __init__(
        ...
        *,
        ...
        if max_retries is None:  # pyright: ignore[reportUnnecessaryComparison]
            raise TypeError(
                "max_retries cannot be None. If you want to disable retries, pass `0`; if you want unlimited retries, pass `math.inf` or a very high number; if you want the default behavior, pass `mtn_oauth_v1.DEFAULT_MAX_RETRIES`"
            )
    ...
    def _build_request(
        ...
        *,
        ...
        if log.isEnabledFor(logging.DEBUG):
            log.debug("Request options: %s", model_dump(
        ...
        *,
        ...
        try:
            ...
            raise APIResponseValidationError(
                response=response, body=data) from err
    ...
    def _calculate_retry_timeout(
        ...
        sleep_seconds = min(INITIAL_RETRY_DELAY *
                            pow(2.0, nb_retries), MAX_RETRY_DELAY)
...
class SyncAPIClient(BaseClient[httpx.Client, Stream[Any]]):
    ...
    def _request(
        ...
        *,
        ...
        remaining_retries = options.get_max_retries(
            self.max_retries) - retries_taken
        ...
        try:
            response = self._client.send(
                request,
                stream=stream or self._should_stream_response_body(
                    request=request),
                **kwargs,
            )
