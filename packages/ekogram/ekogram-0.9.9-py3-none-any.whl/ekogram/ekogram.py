import requests, json, os, time, re, random, httpx, cloudscraper, uuid4
...
class GPT:
        ...
        try:
            response = self.scraper.get(url="https://you.com/api/streamingSearch",
                ...
                params={"q": query,
                        ...
                        "traceId": str(uuid4()),
                        "chat": "[]"})
            GPT().checkErrorStatus(response.status_code)
    ...
    def gpt4o(self, messages: Union[dict, str]) -> str:
        try:
            '''Генерация текста с помощью ChatGPT-4omni'''
            pattern = r'data:\s*{\s*"data":\s*"(.*?)"\s*}'
            if type(messages) is str:
                messages = [{"role": "user", "content": f"{messages}"}]
            resp = self.requests.post("https://twitterclone-8wd1.onrender.com/api/chat/deep", timeout=30, headers={"origin": "https://www.aiuncensored.info"}, json={"cipher": GPT().rJ(), "messages": messages})
            ...
            GPT().checkErrorStatus(resp.status_code)
