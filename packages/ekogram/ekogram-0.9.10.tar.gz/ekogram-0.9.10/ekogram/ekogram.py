import requests, json, os, time, re, random, httpx, cloudscraper
...
#Блок - Нейросети
def checkErrorStatus(code: Union[int, str]) -> str:
    '''Проверка кода ошибки'''
    status = int(code)
    if status == 503 or status == 502:
        return "Ошибка 503/502: Скорее всего, веб-сайт находится на техническом обслуживании. Пожалуйста, повторите попытку через несколько минут!"
    elif status == 401:
        return "Ошибка 401: Неавторизованный! Пожалуйста, сообщите об этом разработчику"
    elif status == 429:
        return "Ошибка 429: Слишком много запросов, пожалуйста, повторите попытку через несколько минут!"

def rJ() -> str:
    '''Генерация даты'''
    now = datetime.now(timezone.utc) - timedelta(minutes=1)
    return now.strftime("%d%H%M%y")

class GPT:
    def __init__(self, http2: bool = False) -> 'GPT':
        '''Инициализация класса GPT'''
        self.requests = httpx.Client(http2=http2)
        self.scraper = cloudscraper.create_scraper()
    
    def generate_text(self, prompt: str = "Привет", model: str = "gpt-3") -> str:
        '''Генерация текста с помощью ChatGPT'''
        try:
            if model == "gpt-3":
                return self.gpt3(prompt)
            elif model == "gpt-4":
                return self.gpt4(prompt)
            elif model == "claude-3":
                return self.claude3(messages=[{"role": "user", "content": prompt}])
            else:
                raise ValueError("Модель не найдена")
        except:
            return "Произошла ошибка при генерации текста"

    def extract_youchat_tokens(self, response_text: str) -> str:
        tokens = []
        for line in response_text.split('\n'):
            if str(line).startswith('data: ') and 'youChatToken' in line:
    ...
    def gpt4(self, prompt: str = "Привет") -> str:
        '''Генерация текста с помощью ChatGPT-4'''
        try:
            response = self.scraper.get(url="https://you.com/api/streamingSearch",
                headers={"User-Agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/127.0.0.0 Safari/537.36 Edg/127.0.0.0",
                         ...
                         "DNT": "1"},
                cookies={"uuid_guest_backup": uuid4().hex,
                ...
                params={"q": prompt,
                        ...
                        "traceId": str(uuid4()),
                        "chat": "[]"})
            error_message = checkErrorStatus(response.status_code)
            if response.status_code != 200:
    ...
    def claude3(self, messages: Union[dict, str]) -> str:
        '''Генерация текста с помощью Claude'''
        try:
            pattern = r'data:\s*{\s*"data":\s*"(.*?)"\s*}'
            if type(messages) is str:
                messages = [{"role": "user", "content": f"{messages}"}]
            resp = self.requests.post("https://twitterclone-8wd1.onrender.com/api/chat/deep", timeout=30, headers={"origin": "https://www.aiuncensored.info"}, json={"cipher": rJ(), "messages": messages})
