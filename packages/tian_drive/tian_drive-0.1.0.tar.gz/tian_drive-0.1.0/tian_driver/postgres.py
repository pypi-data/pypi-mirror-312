...
class Postgres(Driver):
    ...
    def __init__(
        ...
        logger.debug(f"Connected to {self.__params}")
    ...
    def query_none(self, **kwargs) -> NoReturn:
        ...
        self._validate_params({'sql'}, set(kwargs.keys()))
        cursor = self.__conn.cursor()
        logger.info(f"Executing query: {kwargs['sql']}")
        _ = self.__execute(cursor, kwargs['sql'], *kwargs.get('args', []))
        self.__conn.commit()
        cursor.close()
    ...
    def __build_connection(
        ...
        self.__conn.autocommit = commit if autocommit is not None else True


    def execute(self, sql: AnyStr, *args) -> NoReturn:
        """Execute a query.

        :param sql: AnyStr -> SQL query statement
        :param args: Optional[Iterable[Any]] -> Object with query replacement values
        """
        logger.warning(f"Executing query: {sql}", *args)
        cursor = self.__conn.cursor()
        _ = self.__execute(cursor, sql, *args)
        cursor.close()
