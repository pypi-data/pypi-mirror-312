...
def clean_summary(tissue_type, target_species, compare=True, reference_species="Human"):
    ...
    merge them and remove markdown code block symbols.
        ...
        if compare:
            try:
                with open(considerations_file, 'r') as f:
                    content += f"\n\n## 4. VERY IMPORTANT:Since were analyzing {target_species} {tissue_type}, and are using lots of {reference_species} {target_species} info here, therefore it is important to think about the differences between {target_species} and {reference_species}. \n\n"
                    content += f.read()
