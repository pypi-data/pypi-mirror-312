...
{
    MLHP_CHECK( indicator.size( ) == oldGrid.ncells( ),
    ...
    auto traverse = [&]( auto&& self, CellIndex ifull ) -> void
    {
        ...
        {
            maxdepth[ifull] = indicator[oldGrid.leafIndex( ifull )];
        }
    };
...
auto adaptiveRefine( const AbsHierarchicalGrid<D>& oldMesh,
                     ...
                     const std::vector<int>& maxdepth,
                     CellIndex ifull )
{
    auto result = findInOtherGrid( newMesh, oldMesh, ifull );
...
RefinementFunction<D> refineAdaptively( const AbsHierarchicalGrid<D>& oldMesh,
                                        const std::vector<int>& indicator )
{
    auto maxdepth = adaptiveMaxDepth( oldMesh, indicator );
    ...
    return [=, &oldMesh]( const MeshMapping<D>& mapping, RefinementLevel )
    {
        ...
        return adaptiveRefine( oldMesh, newMesh, maxdepth, newMesh.fullIndex( mapping.icell ) );
    };
}
                                              ...
                                              const std::vector<int>& indicator )
{
    ...
    auto maxdepth = mesh::adaptiveMaxDepth( oldMesh, indicator );
    ...
    while( begin != end )
    {
        ...
        for( std::int64_t ii = 0; ii < static_cast<std::int64_t>( end - begin ); ++ii )
        {
            ...
            refinementMask[index] = mesh::adaptiveRefine( oldMesh, *newMesh, maxdepth, index + begin );
        }
