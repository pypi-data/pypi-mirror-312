...
def get_heroku_credentials(heroku_config_dict=None):
    try:
        ...
        if response.status_code == 200:
            ...
            if db_url:
                return CustomResponse(data=parse_database_url(db_url), status=200)
            else:
                logger.error("Database URL not found in config vars")
                print("Database URL not found in config vars")
                return CustomResponse(data="Database URL not found in config vars", status=404)
            ...
            return CustomResponse(data=f"Failed to retrieve config vars from Heroku. Status Code: {response.status_code}", status=404)
        ...
        return CustomResponse(data=f"An unexpected error occurred: {str(e)}", status=404)
