...
from zeta.utils.supabase import user_uid_to_uuid, get_supabase_user_uid
...
def create_supabase_auth(supabase_url, supabase_key):
    client: supabase.Client = supabase.create_client(supabase_url, supabase_key)
    ...
    def verify_auth(f):
        ...
        def decorated_function(*args, **kwargs):
            ...
            try:
                token = auth_header.split(" ")[1]
                response: supabase.UserResponse = client.auth.get_user(token)
...
if SUPABASE_URL and SUPABASE_KEY:
    verify_auth = create_supabase_auth(SUPABASE_URL, SUPABASE_KEY)
else:
...
def _create_new_supabase_user_session(user_uid: str) -> str:
    supabase_uuid: str = user_uid_to_uuid(user_uid)
