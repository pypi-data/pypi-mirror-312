...
ZETA_GCP_SERVICE_ACCOUNT_JSON = os.environ.get("ZETA_GCP_SERVICE_ACCOUNT_JSON")
...
class GoogleStorageBucket(StorageBucket):
    def __init__(self, backend: StorageConfig):
        ...
        if ZETA_GCP_SERVICE_ACCOUNT_JSON is None:
            raise ValueError("Environment variable ZETA_GCP_SERVICE_ACCOUNT_JSON is not set.")

        self._client = storage.Client.from_service_account_json(ZETA_GCP_SERVICE_ACCOUNT_JSON)
