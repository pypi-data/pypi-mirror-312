...
class StoreOp(_ods_ir.OpView):
  ...
  def __init__(self, output, input, input_gmem, *, ginfo=None, support_compress=None, compress_info=None, l2m_addr=None, loc=None, ip=None):
    operands = []
    results = []
    attributes = {}
    regions = None
    ...
    if compress_info is not None: attributes["compress_info"] = (compress_info if (
        ...
        not _ods_ir.AttrBuilder.contains('Tpu_CompressAttr')) else
          _ods_ir.AttrBuilder.get('Tpu_CompressAttr')(compress_info, context=_ods_context))
    if l2m_addr is not None: attributes["l2m_addr"] = (l2m_addr if (
        issubclass(type(l2m_addr), _ods_ir.Attribute) or
        not _ods_ir.AttrBuilder.contains('I64Attr')) else
          _ods_ir.AttrBuilder.get('I64Attr')(l2m_addr, context=_ods_context))
    results.append(output)
  ...
  @builtins.property
  def l2m_addr(self):
    if "l2m_addr" not in self.operation.attributes:
      return None
    return self.operation.attributes["l2m_addr"]
  ...
  def l2m_addr(self, value):
    if value is not None:
      self.operation.attributes["l2m_addr"] = value
    elif "l2m_addr" in self.operation.attributes:
      del self.operation.attributes["l2m_addr"]
