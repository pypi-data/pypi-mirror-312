...
class ActionTracker:
        ...
        try:
            if project_type == "classification":
                for output in outputs:
                    ...
                    results.append(
                        {"category": self.index_to_category[str(predicted_class)], "confidence": confidence}
                    )
            elif project_type == "detection":
                for output in outputs:
    ...
    def _reformat_inference_targets(self, targets, project_type):
        ...
        try:
            if project_type == "classification":
                results = [
                        {"category": self.index_to_category[str(target.cpu().item())], "confidence": 1}
    ...
    def store_inference_results(
        self, images, outputs, targets, split_type, project_type, format_inputs = True, pil_iamges = False
    ):
        is_exported = "pytorch" not in self.action_details.get("runtimeFramework", "pytorch").lower()
        self.index_to_category = self.get_index_to_category(is_exported)
        if format_inputs:
        ...
        try:
            ...
            samples = [
                {
                    "_idModel": self._idModel_str,
                    "modelType": "exported" if is_exported else "trained",
            ...
            try:
                ...
                for image, presigned_url in zip(images, presigned_urls):
                    if not pil_iamges:
