...
def plot_pct_box( df_pct, sg_map, ncols, figsize = None, dpi = 100,
    ...
    for kk, item in enumerate(items):
        ...
        if len(group_sel) > 1:
            ...
            if annot_ref in lst:
                ...
                for k, l1 in enumerate(lst):
                    if l1 != l2:
                        b1 = df_tmp['Group'] == l1
                        b2 = df_tmp['Group'] == l2
                        lst1 = df_tmp.loc[b1, item].tolist()
                        lst2 = df_tmp.loc[b2, item].tolist()
                        # res_levene = levene(lst1, lst2)
                        res = ttest_ind(lst2, lst1, 
                                        # equal_var = (res_levene.pvalue > 0.05),
                                        # permutations = 0,
                                        alternative = 'two-sided')
                        # pval_corr = bonferroni(res.pvalue, 'auto')
                        print( [item, l1, l2], np.round( res.pvalue, 4 ) )
                        if res.pvalue <= pval_cutoff:
                            lst_pair.append((l2, l1))
            else:
                for k, l1 in enumerate(lst):
            ...
            ax = sns.boxplot(data = df_tmp, x = 'Group', y = item, order = group_order)
            if stripplot:
                sns.stripplot(x='Group', y=item, data=df_tmp, jitter=True, 
            ...
            if annot:
                add_stat_annotation(ax, data=df_tmp, x = 'Group', y = item, 
                                    order = group_order,
