...
from model_wrapper.training import ClassTrainer, SimpleClassTrainer, EvalClassTrainer, SimpleEvalClassTrainer
from model_wrapper import ModelWrapper, SimpleModelWrapper, ClassModelWrapper, SimpleClassModelWrapper, \
	SplitClassModelWrapper, RegressModelWrapper, SimpleRegressModelWrapper, SplitRegressModelWrapper
...
class TextModelWrapper(SimpleClassModelWrapper):
	...
	def train(self, texts: Union[Collection[str], np.ndarray, pd.Series],
	          ...
	          n_jobs=-1, show_progress=True, eps=1e-5) -> dict:
	...
	def predict(self, texts: Collection[str], max_length: int = None, n_jobs=-1, threshold: int = 0.5) -> np.ndarray:
		"""
		:param texts:
	...
	def predict_classes(self, texts: Collection[str], max_length: int = None, n_jobs=-1, threshold: int = 0.5) -> list:
		"""
		:param texts:
		:param threshold: 二分类且模型输出为一维概率时生效
		"""
	...
	def predict_proba(self, texts: Collection[str], max_length: int = None, n_jobs=-1, threshold: int = 0.5) -> Tuple[
	...
	def predict_classes_proba(self, texts: Collection[str], max_length: int = None, n_jobs=-1, threshold: int = 0.5) -> \
	...
	def evaluate(self, texts: Union[str, Collection[str], np.ndarray, pd.Series],
	             ...
	             max_length: int = None, collate_fn=None, n_jobs=-1, threshold: int = 0.5) -> float:
		""" return accuracy """
		X = self.get_vec(texts, max_length, n_jobs=n_jobs)
	...
	def train(self, texts: Union[Collection[str], np.ndarray, pd.Series],
		...
		if 0. < val_size < 1.:
			...
			trainer = SimpleEvalClassTrainer(epochs, optimizer, scheduler, lr, T_max,
			                                 ...
			                                 checkpoint_name,
			                                 self.device)
			self.best_model, histories = trainer.train(self.model, X, y, (X_test, y_test), collate_fn, show_progress,
			                                           eps)
		else:
			trainer = SimpleClassTrainer(epochs, optimizer, scheduler, lr, T_max,
			                             batch_size,
...
class PaddingTextModelWrapper(ClassModelWrapper):
	...
	def predict(self, texts: Collection[str], max_length: int = None, threshold: int = 0.5) -> np.ndarray:
		"""
		:param texts:
		:param threshold: 二分类且模型输出为一维概率时生效
		"""
	...
	def predict_classes(self, texts: Collection[str], max_length: int = None, threshold: int = 0.5) -> list:
		"""
		:param texts:
		:param threshold: 二分类且模型输出为一维概率时生效
		"""
	...
	def predict_proba(self, texts: Collection[str], max_length: int = None, threshold: int = 0.5) -> Tuple[
		np.ndarray, np.ndarray]:
		"""
		:param texts:
		:param threshold: 二分类且模型输出为一维概率时生效
	...
	def predict_classes_proba(self, texts: Collection[str], max_length: int = None, threshold: int = 0.5) -> Tuple[
		list, np.ndarray]:
		"""
		:param texts:
		:param threshold: 二分类且模型输出为一维概率时生效
	...
	def evaluate(self, texts: Union[str, Collection[str], np.ndarray, pd.Series],
	             y: Union[torch.LongTensor, np.ndarray, List], batch_size=64, num_workers=0,
	             max_length: int = None, threshold: int = 0.5) -> float:
		""" return accuracy """
		X = self.tokenizer.batch_encode(texts, padding=False)
		if isinstance(y, (np.ndarray, List)):
			y = torch.tensor(y, dtype=torch.long)
		...
		return super().evaluate(val_set, batch_size=batch_size, num_workers=num_workers,
		                        collate_fn=PaddingTokenCollator(self.tokenizer.pad, max_length), threshold=threshold)
	...
	def train(self, texts: Union[Collection[str], np.ndarray, pd.Series],
		...
		if 0. < val_size < 1.:
			...
			trainer = EvalClassTrainer(epochs, optimizer, scheduler, lr, T_max,
			                           ...
			                           checkpoint_name,
			                           self.device)
			self.best_model, histories = trainer.train(self.model, train_set, val_set,
			...
			trainer = ClassTrainer(epochs, optimizer, scheduler, lr, T_max,
			                       ...
			                       checkpoint_name,
			                       self.device)
			self.best_model, histories = trainer.train(self.model, train_set,
			                                           PaddingTokenCollator(self.tokenizer.pad, max_length),
			                                           show_progress, eps)
		return histories
