...
#     def run_contrained_inversion(self):
#         self.n_data = len(self.data)
...
#         def objective(self):
#             sigma_data, sigma_param = self.sigmas()
#             residual = (G@self.params*self.sigma_data) @ (G@self.params)
#             smooth = np.sum((S @ self.params)**2)
#             regularization = (self.params*self.sigma_param)@self.params
#             return residual+self.lam*regularization+self.gam*smooth

#         bounds = [(0, None) for _ in range(self.n_param)]
#         result = minimize(objective, self.param, bounds=bounds,
#                           method="L-BFGS-B")
#         self.params = result.x

    def objective(self, params):
        residual = ((self.G@params-self.dat)*self.sigma_data) @ \
            (self.G@params-self.dat)
        smooth = np.sum((self.S@params)**2)
        regularization = (params*self.sigma_param)@params
        return residual+self.lam*regularization+self.gam*smooth
...
#    by the user
            ...
            if self.positive:
                bounds = [(0, None) for _ in range(self.n_param)]
                bounds[-1] = (None, None)
                result = minimize(self.objective, self.params, bounds=bounds,
                                  method="L-BFGS-B")
                self.params = result.x
