...
async def generate_episodes(
    ...
    async for seed in tqdm_asyncio(seeds, desc=f"Running {mode} episodes"):
        ...
        episode_data = await generate_single_episode(agent, mode, seed, max_steps, output_dir)
        all_episodes_data.append(episode_data)
        # Get traces after each episode
        print("Len traces: ", event_store._traces.keys())
...
if __name__ == "__main__":
    import asyncio
    asyncio.run(run_craftax_episodes(
        n_seeds=2
    ))