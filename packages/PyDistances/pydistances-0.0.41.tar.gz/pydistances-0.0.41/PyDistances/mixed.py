...
def ggower_dist(xi, xr, X, p1, p2, p3, d1='euclidean', d2='sokal', d3='matching', 
                q=1, robust_method='trimmed', alpha=0.05, epsilon=0.05, n_iters=20,
                VG_sample_size=300, VG_n_samples=5, random_state=123, weights=None):
    
    S, S_robust = None, None
    ...
    if d1 in ['robust_mahalanobis', 'mahalanobis']:
            ...
            if d1 == 'robust_mahalanobis':
                S_robust = S_robust(X=X_quant, method=robust_method, alpha=alpha, 
                                    epsilon=epsilon, n_iters=n_iters, weights=weights)
            elif d1 == 'mahalanobis':
                S = np.cov(X_quant, rowvar=False)

    VG1, VG2, VG3 = vg_ggower_fast_estimation(X=X, p1=p1, p2=p2, p3=p3, d1=d1, d2=d2, d3=d3, robust_method=robust_method, 
                                                                 alpha=alpha, epsilon=epsilon, n_iters=n_iters,
                                                                 VG_sample_size=VG_sample_size, VG_n_samples=VG_n_samples, 
