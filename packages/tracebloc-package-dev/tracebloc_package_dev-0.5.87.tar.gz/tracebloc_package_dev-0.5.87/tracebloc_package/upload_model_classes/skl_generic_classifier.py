...
class SKLGenericClassifier(Model, SklearnChecks):
    ...
    def small_training_loop(
        ...
        try:
            dataloader = get_dataloader(num_feature_points=self.num_feature_points, num_classes=self.classes)
    ...
    def model_func_checks(self):
        try:
            ...
            if not os.path.exists(
                ...
                shutil.copy(os.path.join(self.tmp_dir_path, f"{TRAINED_WEIGHTS_FILENAME}.pkl"), os.path.join(self.tmp_dir_path, f"{PRETRAINED_WEIGHTS_FILENAME}.pkl"))
                self.progress_bar.update(1)
    ...
    def average_weights(self):
        ...
        if self.model_type == 'linear':
            ...
            voting_classifiers = [('tree_' + str(i), weights[i]) for i in range(len(weights))]
            ensemble_model = VotingClassifier(estimators=voting_classifiers, voting='hard')
            ...
            coefs = [np.mean([model.coefs_[i] for model in weights], axis=0) for i in
                     range(len(weights[0].coefs_))]
            intercepts = [np.mean([model.intercepts_[i] for model in weights], axis=0) for i in
                          range(len(weights[0].intercepts_))]
            averaged_model.coefs_ = coefs
            averaged_model.intercepts_ = intercepts

        elif self.model_type == 'svc':
            # For linear SVM, average coef_ and intercept_
            voting_classifiers = [('svc_' + str(i), weights[i]) for i in range(len(weights))]
            ensemble_model = VotingClassifier(estimators=voting_classifiers, voting='hard')
            averaged_model = ensemble_model

        elif self.model_type == 'knn':
            # Ensemble strategy for k-NN
            voting_classifiers = [('knn_' + str(i), self.model_type[i]) for i in range(len(self.model_type))]
            ensemble_model = VotingClassifier(estimators=voting_classifiers, voting='soft')
            averaged_model = ensemble_model
        else:
            raise ValueError(f"model type {self.model_type} is not supported for Sklearn")
