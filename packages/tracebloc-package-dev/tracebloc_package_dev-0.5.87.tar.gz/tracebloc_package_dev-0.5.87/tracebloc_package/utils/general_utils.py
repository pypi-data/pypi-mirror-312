...
def get_model_params_count(framework="tensorflow", model=None) -> int:
    ...
    if framework == TENSORFLOW_FRAMEWORK:
        ...
        if hasattr(model, 'coef_') and model.coef_ is not None:
            ...
            intercept_count = model.intercept_.size if hasattr(model, 'intercept_') else 0
            ...
            return sum(get_model_params_count(framework="sklearn", model=est) for est in model.estimators_)
        else:
            return 400
    else:
