...
class AsyncNovaJira:
    ...
    async def create(self, jira_endpoint: str,
                                      jira_api_token: str, jira_username: str,
...
class SyncNovaJira:
    ...
    def create(self, jira_endpoint: str,
                                      jira_api_token: str, jira_username: str,
                                      jira_id: str) -> CreateScenariosResponse:
        """Creates Test Scenarios using a JIRA ID
        Connects to a JIRA instance using provided credentials and domain URL, then generates test scenarios based on the specified JIRA Ticket ID.
...
class AsyncNovaRally:
    ...
    async def create(self, TICKET_ID, WORKSPACE_NAME, RALLY_URL, RALLY_API_KEY):


        url = urljoin(self.base_url,Configurations.getNovaContextPath("get_rally_workspaces"))
        ...
        async_client = AsyncHTTPClient()
        response = await async_client.get(url=url, params=params, headers=headers)
        if response:
            ...
            for workspace in workspaces:
                if workspace.get("_refObjectName") == WORKSPACE_NAME:
                    ...
                    ticket_details_url = urljoin(self.base_url,Configurations.getNovaContextPath("get_rally_ticket_details"))
                    ticket_params = {
                        "TICKET_ID": TICKET_ID,
                    ...
                    ticket_response = await async_client.get(url=ticket_details_url, params=ticket_params,headers=headers)

                    # if ticket_response:
                    description = ticket_response.get("description", "")
                    if description:
                        async_description_instance = AsyncNovaUserDescription(api_key=self.api_key)
                        async_description_response = await async_description_instance.create(description)
                        # print(f"AsyncDescription Response: {async_description_response}")
                        return async_description_response

                    return CreateScenariosResponse(ok=False,message=f"Description not found for for Workspace: {WORKSPACE_NAME} ")

            return CreateScenariosResponse (ok=False,message=f"WORKSPACE_NAME : '{WORKSPACE_NAME}' not found in workspaces.")
        ...
        if response:
            ...
            for workspace in workspaces:
                if workspace.get("_refObjectName") == WORKSPACE_NAME:
                    ...
                    ticket_response =  sync_client.get(url=ticket_details_url, params=ticket_params, headers=headers)

                   
                    description = ticket_response.get("description", "")
                    if description:
                        sync_description_instance = SyncNovaUserDescription(api_key=self.api_key)
                        sync_description_response = sync_description_instance.create(description)
                        # print(f"SyncDescription Response: {sync_description_response}")
                        return sync_description_response

                    return CreateScenariosResponse(ok=False,message=f"Ticket: '{TICKET_ID}' not found in {WORKSPACE_NAME} ")
