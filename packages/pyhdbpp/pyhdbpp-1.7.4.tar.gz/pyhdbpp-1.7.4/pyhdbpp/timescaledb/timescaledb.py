...
class TimescaleDbReader(AbstractReader):
    """
    ...
    def get_closer_attributes_values(self, attributes, columns=[], time_ref=None, n=1):
        """
        ...
        """
        data = {}
        time_ref = datetime.datetime.now() if time_ref is None else time_ref
                ...
                if len(att_id) == 0:
                    ...
                    if(n < 0):
                        self._cursor.execute(
                            sql.SQL("SELECT data_time, {fields} FROM {table} WHERE att_conf_id=%s AND data_time < %s AND quality=0 AND att_error_desc_id IS null ORDER BY data_time DESC LIMIT %s").format(
                                fields = sql.SQL(',').join([sql.Identifier(field) for field in columns]),
                                table = sql.Identifier(att_id[0][1]))
                            , (att_id[0][0], time_ref, -n))
                    else:
                        self._cursor.execute(
                            sql.SQL("SELECT data_time, {fields} FROM {table} WHERE att_conf_id=%s AND data_time > %s AND quality=0 AND att_error_desc_id IS null ORDER BY data_time DESC LIMIT %s").format(
                                fields = sql.SQL(',').join([sql.Identifier(field) for field in columns]),
        ...
        for attr in attributes:
            try:
                ...
                if (len(att_id) > 1):
                    ...
                    self._cursor.execute(
                        sql.SQL("SELECT data_time, {fields} FROM {table} WHERE att_conf_id=%s AND quality=0 AND att_error_desc_id IS null AND data_time BETWEEN %s AND %s ORDER BY data_time DESC").format(
                            fields = sql.SQL(',').join([sql.Identifier(field) for field in columns]),
                            table = sql.Identifier(att_id[0][1]))
