...
class MyAirTest(OperateABC):
    ...
    def wait_disappear_element(self, pos, timeout=180, rgb=False, translucent=False, ocrPlus=False,
        ...
        while self.exists(pos, rgb=rgb, translucent=translucent, ocrPlus=ocrPlus,
    ...
    def wait_element_appear(self, pos, timeout=180, rgb=False, translucent=False, ocrPlus=False, ocr_match_type=False):
        start_loading_time = time.time()
        while self.exists(pos, rgb=rgb, translucent=translucent, ocrPlus=ocrPlus,
    ...
    def wait_for_any(self, pos_list: list, timeout=30, rgb=False, translucent=False,
                     ocrPlus=False, ocr_match_type=False):
        """
        ...
        while True:
            for pos in pos_list:
                if self.exists(pos, rgb=rgb, translucent=translucent, ocrPlus=ocrPlus, ocr_match_type=ocr_match_type):
                    return pos
    ...
    def wait_next_element(self, last_click_pos, next_pos, timeout=180, rgb=False, translucent=False,
        ...
        while self.exists(next_pos, rgb=rgb, translucent=translucent, ocrPlus=ocrPlus,
                          ocr_match_type=ocr_match_type) is False:
            ...
            if last_click_pos_staging is None:
                last_click_pos_staging = self.click(last_click_pos, rgb=rgb, translucent=translucent,
                                                    ocrPlus=ocrPlus, ocr_match_type=ocr_match_type)
            else:
    ...
    def wait_next_element_then_click(self, next_element, timeout=180, rgb=False, translucent=False,
        ...
        start_time = time.time()
        while True:
            next_pos = self.exists(next_element, rgb=rgb, translucent=translucent, ocrPlus=ocrPlus,
                                   ocr_match_type=ocr_match_type)
    ...
    def wait_last_element_disappear(self, last_click_pos, last_pos, timeout=180, rgb=False, translucent=False,
        ...
        while self.exists(last_pos, rgb=rgb, translucent=translucent, ocrPlus=ocrPlus, ocr_match_type=ocr_match_type):
            log.info(f"{last_pos}没有消失正在尝试点击")
            if last_click_pos_2 is None:
                last_click_pos_2 = self.click(last_click_pos, rgb=rgb, translucent=translucent, ocrPlus=ocrPlus, ocr_match_type=ocr_match_type)
            else:
