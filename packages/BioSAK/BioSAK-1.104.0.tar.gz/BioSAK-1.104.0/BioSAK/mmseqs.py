...
def mmseqs(args):
    ...
    cmd_2 = 'mmseqs taxonomy %s/%s.DB %s %s/taxonomyResult %s/tmp'                                  % (op_dir, seq_file_base, db_dir, op_dir, op_dir)
    cmd_3 = 'mmseqs createtsv %s/%s.DB %s/taxonomyResult %s/taxonomyResult.tsv'                     % (op_dir, seq_file_base, op_dir, op_dir)
    # cmd_4 = 'BioSAK parse_mmseqs_tsv -i %s/taxonomyResult.tsv -o %s/taxonomyResult_reformatted.tsv' % (op_dir, op_dir)
