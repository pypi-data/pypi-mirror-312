...
class CMD(SendableMixin):
    ...
    def __add__(self, other) -> str:
        """
        Proxy for string __add__

        .. warning::
            This will "clobber" the CMD, returning a str
        """
        return self._cmd.__add__(other)
    ...
    def replace(self, oldvalue: str, newvalue: str) -> str:
        """
        Allows replacement on the init command

        .. warning::
            This will "clobber" the CMD, returning a str

        Args:
            oldvalue: source to replace with
            target: replacement
        """
        return self.sent.replace(oldvalue, newvalue)
    ...
    def communicate(
        ...
        if self._cached and use_cache:
            ...
            std, err = self._file_communicate(verbose=verbose)
    ...
    def _communicate(self, verbose: Union[None, int, bool, Verbosity]) -> (str, str):
        """
