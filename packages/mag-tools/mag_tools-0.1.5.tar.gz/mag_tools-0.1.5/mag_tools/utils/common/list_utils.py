    ...
    def split_segment(lines, keyword_map):
        ...
        for segment_name, (begin_keyword, end_keyword) in keyword_map.items():
            ...
            for match in matches:
                ...
                segments[segment_name].append(block_lines)

        # 将第一个有效段落之前的内容放在 FIRST 段落
        if first_segment_start is not None:
            first_content = text[:first_segment_start].strip().split("\n")
            segments["FIRST"].append(first_content)

        # 将最后一个有效段落之后的内容放在 END 段落
        if last_segment_end is not None:
            end_content = text[last_segment_end:].strip().split("\n")
            segments["END"].append(end_content)
...
"""
    ...
    for segment_name, segments in _segments.items():
