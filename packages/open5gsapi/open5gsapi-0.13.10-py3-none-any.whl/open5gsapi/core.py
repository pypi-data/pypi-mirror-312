...
#---------------------------------------------------------
    ...
    def run_background_nodes(self):
        try:
            ...
            # Start UPF first
            logger.info("Starting UPF container...")
            upf_command = 'docker exec upf bash -c "cd src/upf && sh init_script.sh && python3 upf-api.py"'
            subprocess.Popen(upf_command, shell=True)
            ...
            # Get configured number of UEs
            num_ues = self.get_num_ues()
            logger.info(f"Initializing {num_ues} UE(s)...")
            
            # Start UEs in batches
            batch_size = 5
            for i in range(0, num_ues, batch_size):
                batch_end = min(i + batch_size, num_ues)
                for j in range(i, batch_end):
                    ue_command = f'docker exec ue bash -c "UE_INDEX={j} sh init_script.sh && python3 auto-ue-api.py"'
                    subprocess.Popen(ue_command, shell=True)
                time.sleep(2)  # Give time for each batch to initialize
    ...
    def _wait_for_upf(self, timeout=30):
        start_time = time.time()
        while time.time() - start_time < timeout:
            upf_ready, _ = self._check_upf_api()
            if upf_ready:
                return True
            time.sleep(1)
        raise ConfigurationError("Timeout waiting for UPF to start")
    ...
    def _check_ue_interfaces(self) -> Tuple[bool, Optional[str]]:
        try:
            ...
            try:
                ...
                if num_tun_interfaces == 0:
                    return False, "No TUN interfaces found"
                elif num_tun_interfaces < num_ues:
                    return False, f"Waiting for UE interfaces ({num_tun_interfaces}/{num_ues})"
                
                # Verify interfaces are responding
                for i in range(num_tun_interfaces):
                    url = f"http://10.10.0.132:{8080 + i}/get_ue_ip"
                    try:
                        response = requests.get(url, timeout=1)
                        if response.status_code != 200:
                            return False, "UE API endpoints not responding correctly"
                    except requests.RequestException:
                        return False, "UE API endpoints not accessible"
                ...
                return False, "Error reading interface count"
