...
class Open5GS:
    ...
    def _monitor_background_processes(self, timeout: int = 120):
...
#---------------------------------------------------------
    ...
    def run_background_nodes(self):
        ...
        try:
            ...
            # Start UE container and verify interface creation
            ue_success = self._verify_ue_initialization()
            if not ue_success:
                error_msg = "UE initialization failed - Cell selection failure detected"
                self._background_process_status['error_message'] = error_msg
                logger.error(error_msg)
                return

            # Only start UE API if interfaces are properly created
            ue_command = 'docker exec ue bash -c "python3 auto-ue-api.py"'
            subprocess.Popen(ue_command, shell=True)
    ...
    def _verify_ue_initialization(self) -> bool:
        """Verify UE initialization by checking docker logs"""
        try:
            num_ues = self.get_num_ues()
            logger.info(f"Verifying initialization for {num_ues} UE(s)...")

            # Run init script first
            subprocess.run('docker exec ue bash -c "sh init_script.sh"', shell=True, check=True)
            ...
            def check_logs():
                ...
                success_pattern = r"Connection setup for PDU session.*is successful, TUN interface"
                failure_pattern = r"Cell selection failure"
                ...
                while True:
                    ...
                    if re.search(success_pattern, line):
                        logger.info("Detected successful UE interface creation")
                        process.terminate()
                        return True
                        
                    if re.search(failure_pattern, line):
                        logger.error("Detected cell selection failure")
                        process.terminate()
                        return False
            ...
            if result is True:
                # Verify actual interface creation
                result = subprocess.run(
                    "docker exec ue bash -c 'ip link show | grep uesimtun'",
                    shell=True,
                    capture_output=True,
                    text=True
                )
                interfaces = result.stdout.strip().split('\n')
                if len(interfaces) >= num_ues:
                    logger.info(f"Verified {len(interfaces)} UE interfaces created")
                    return True
                else:
                    logger.error(f"Interface count mismatch. Expected {num_ues}, found {len(interfaces)}")
                    return False
            
            return False

        except subprocess.CalledProcessError as e:
            logger.error(f"Error verifying UE initialization: {e}")
            return False
