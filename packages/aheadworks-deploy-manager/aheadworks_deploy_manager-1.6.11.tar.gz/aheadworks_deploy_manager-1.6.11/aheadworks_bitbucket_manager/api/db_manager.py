...
class DbManager:
    ...
    def create_dump(self, path_to_sh, host, user):
        command = 'test -f ' + path_to_sh + ' && echo True || echo False'
        file_exists = self.ssh_manager.run_ssh_command(command, host, user)
        file_exists = eval(file_exists[0].strip()) if file_exists else False
        if file_exists:
            result = 'Db dump created'
            self.ssh_manager.run_ssh_command(path_to_sh, host, user)
        else:
