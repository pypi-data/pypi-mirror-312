...
def init_result():
    def get_init_collection[T](coll_type: T):
        return {
            strf_time(i): defaultdict(coll_type)
            for i in range(int(TaskResultSet.start_time), int(TaskResultSet.end_time) + 1)
        }

    result: dict[str, dict] = {}
    rps: dict[str, dict[str, int]] = get_init_collection(int)
    qps: dict[str, dict[str, int]] = get_init_collection(int)
    response: dict[str, dict[str, list]] = get_init_collection(list)
    ...
    for value in result.values():
        ...
        table_data.append({
            ...
            "吞吐量（RPS）": f'{value["SUCCESS"] / (TaskResultSet.end_time - TaskResultSet.start_time):.2f}/s'
        })
