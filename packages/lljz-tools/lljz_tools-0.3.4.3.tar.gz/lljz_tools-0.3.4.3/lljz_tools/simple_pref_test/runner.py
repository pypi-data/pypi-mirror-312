...
from lljz_tools.simple_pref_test._task import find_tasks, TaskGroup
...
class PrefRunner:
    ...
    def __init__(
            self,
            __name: str = '性能测试', /, *,
            virtual_users: int | None = 1, user_add_interval: float | None = 0.1,
            pre_task: float | None = None,
            run_seconds: int | float | None = 10,
        ...
        self.pool = ThreadPoolExecutor(max_workers=max(10, virtual_users, pre_task or 0))
        ...
        def run():
            ...
            while True:
                ...
                for task in task_set.tasks:
                    ...
                    task_set.setup(task.name)
                    task()
                    task_set.teardown(task.name)
