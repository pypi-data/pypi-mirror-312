...
from moapy.enum_pre import enum_to_list, en_H_AISC10_SI, enSteelMaterial_ASTM, enBoltMaterialASTM, enUnitStress, enUnitForce, enUnitMoment, enUnitSystem, enUnitLength
...
@auto_schema(
    ...
    description=(
        """
        The AISC-LRFD16M standard outlines the requirements for designing base plates that connect steel columns to foundations, emphasizing both safety and efficiency. Key considerations include:
...
def report_aisc16m_baseplate(fck: Stress = Stress(value=24.0, unit=enUnitStress.MPa),
                             baseplate: BasePlate = BasePlate.create_default(unit_system=enUnitSystem.SI),
...
def calc_aisc16m_baseplate(fck: Stress = Stress(value=24.0, unit=enUnitStress.MPa),
                           baseplate: BasePlate = BasePlate.create_default(unit_system=enUnitSystem.SI),
                           sect: SteelSection = SteelSection.create_default(name="HP410X151", enum_list=enum_to_list(en_H_AISC10_SI)),
                           force: SectionForce = SectionForce.create_default(unit_system=enUnitSystem.SI),
...
# if __name__ == "__main__":
    # res = report_aisc16m_baseplate(InputAISC16MBasePlate())
    # print(res)