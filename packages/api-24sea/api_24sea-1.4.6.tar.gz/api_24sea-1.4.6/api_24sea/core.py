        ...
        if outer_join_on_timestamp:
            for i, df in enumerate(data_frames):
                ...
                try:
                    ...
                    logging.warning("Failed to parse timestamps with "
                        f"pd.to_datetime: {str(e1)}")
                    try:
                        # Try parsing with different format
                        data_frames[i].index = pd.to_datetime(
                            data_frames[i].index, format="mixed"
                        )
                    except Exception as e2:
                        logging.error("All timestamp parsing attempts "
                                      f"failed: {str(e2)}")
                        raise ValueError(
                            "Could not parse timestamp data"
                        ) from e2
                # fmt: on
...
def to_category_value(
    ...
    try:
        ...
        logging.warning("Failed to parse timestamps with pd.to_datetime: "
                       f"{str(e1)}")
        try:
            # Try parsing with different format
            data["timestamp"] = pd.to_datetime(data["timestamp"] \
            ...
            logging.error(f"All timestamp parsing attempts failed: {str(e2)}")
            raise ValueError("Could not parse timestamp data") from e2
