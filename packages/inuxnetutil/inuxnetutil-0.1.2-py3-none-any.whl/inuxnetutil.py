...
def is_empty_string(arg: str, arg_name: str = None, nullable: bool = False) -> str:
    """
...
def validate_password(password: str, display_help: bool = True) -> bool:
    """
    Validates password
...
def send_mail(to_address: str, subject: str, body: str, from_address: str, relay_host: str, relay_port=25,
    ...
    try:
        ...
        if attachment_path:
            if os.path.isfile(attachment_path):
                ...
                message.add_attachment(file_data, maintype='application', subtype='octet-stream',
                                       filename=file_name)
            else:
                print(f"ERROR: Invalid Path or Permissions '{attachment_path}'.", file=sys.stderr)
                return

        # Connect to the mail relay and send the email
...
def validate_coding_arguments(parser: argparse.ArgumentParser, args: argparse.Namespace):
    ...
    if not args.passphrase:
        if args.operation == "ENCRYPT":
            args.passphrase = get_valid_password(validate=not args.novalidate, complexity=not args.nocomplexity)
        else:
            args.passphrase = get_valid_password(False, False)
    else:
        if args.operation == "ENCRYPT":
            if not args.nocomplexity:
...
def main_encryptfile():
  ...
  encryptfile -k ./id_rsa -f /tmp/id_rsa.enc""", formatter_class=argparse.RawTextHelpFormatter)
    parser.add_argument("--nocomplexity", action="store_true",
                        help="(Optional) Do not require complexity for passphrase, defaults to True")
    parser.add_argument("--novalidate", action="store_true",
                        help="(Optional) Do not validate password input, defaults to true")
...
def main_sendmail():
    """
    Main Entry point for send_mail
    """
  ...
  send_email --to sarah@example.org --from bob@example.org --subject "Hello" --body "How are you?" \\
    ...
    --attachment_path ./README.md""", formatter_class=argparse.RawTextHelpFormatter)
    parser.add_argument("--to", type=str, help="Recipient Address", required=True)
    parser.add_argument("--from_address", type=str, help="Sender Address", required=True)
    parser.add_argument("--subject", type=str, help="Subject", required=True)
    parser.add_argument("--body", type=str, help="Body", required=True)
    parser.add_argument("--relay_host", type=str, help="SMTP Server", required=True)
    parser.add_argument("--relay_port", type=int, help="SMTP Port, Defaults to 25")
    parser.add_argument("--html_body", type=str, help="(Optional) HTML Body")
    parser.add_argument("--attachment_path", type=str, help="(Optional) Attachment path")
    args = parser.parse_args()
    if not args.relay_port:
        args.relay_port = 25

    send_mail(args.to, args.subject, args.body, args.from_address, args.relay_host, args.relay_port, args.html_body,
