...
def KR_predict(X, Y, x):
  '''
    KernelRidge regression with support of complex dependent variables Y
  '''
...
class PredictSeqDM(PredictDMByAtoms):
  ...
  class PredictionSequence:
    ...
    def predict_err(self, predicted_dm, descr):
      ...
      #X = np.hstack([np.array(self.preds).reshape((k, -1)).view(dtype=np.float64), np.array(self.descrs).reshape((k, -1))])
      #x = np.hstack([predicted_dm.ravel().view(dtype=np.float64), descr.ravel()]).reshape((1, -1))
      #Y = np.array(self.errs).reshape((k, -1)).view(dtype=np.float64)
      ...
      #------------------
      y += Y_mean
      #P = y.view(dtype=predicted_dm.dtype).reshape(predicted_dm.shape)
  ...
  def __call__(self, atoms, iK:int=1, iS:int=1):
    ...
    if (iK, iS) not in self.predseq:
      self.predseq[(iK, iS)] = PredictSeqDM.PredictionSequence()
  ...
  def register_DM_init(self, asi):
    ...
    asi.dm_calc_cnt = {}
    asi.dm_storage = {}
    asi.register_dm_callback(PredictSeqDM.dm_callback, self)

  def dm_callback(self, iK, iS, descr, data, matrix_descr_ptr):
    self = cast(self, py_object).value
