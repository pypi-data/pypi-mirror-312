...
from silicondiff_npu.utils import get_soc_info, SocVersion
from ..utils import replace_pattern_with_filters

aten = torch.ops.aten


def fx_pass_fuse_attention(
    ...
    def pattern(
        ...
        out = aten.scaled_dot_product_attention.default(
            transpose_query, transpose_key, transpose_value
        )
    ...
    def replacement(
        ...
        return torch_npu.npu_prompt_flash_attention(
            ...
            value,
            num_heads=shape_0[-2],
            scale_value=(1 / math.sqrt(shape_0[-1])),
            input_layout="BSH",
        )
    ...
    def filter(
        ...
        if get_soc_info().soc_version > SocVersion.Ascend910B1 and shape_0[-1] <= 256:
            return True
        return False

    gm = replace_pattern_with_filters(
        gm, pattern, replacement, match_filters=[filter], name="fuse_attention"
    )
    return gm
