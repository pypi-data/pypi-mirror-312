...
class _Uploader:
    def __init__(
        ...
        self._uploaded_file_ids: list[int] = []
    ...
    def _delete_all_files(self) -> None:
        return self._delete_files(tuple(self._uploaded_file_ids))

    def _delete_files(self, file_ids: tuple[int, ...]) -> None:
        self._event_loop.run_until_complete(self._delete_files_async(file_ids))

    async def _delete_files_async(self, file_ids: tuple[int, ...]) -> None:
        await asyncio.gather(*[self._delete_file_async(file_id) for file_id in file_ids])

    async def _delete_file_async(self, file_id: int) -> None:
        response: Response = await delete_uploaded_file.asyncio_detailed(
            file_id=file_id,
            client=self._authentication_manager._get_authenticated_client(),
        )
    ...
    def _upload_folder(self, folder_path: str, folder_description: str = "") -> dict[str, int]:
    ...
    def _upload_files(self, files_and_descriptions: list[Union[str, tuple[str, str]]]) -> dict[str, int]:
        return self._event_loop.run_until_complete(self._upload_files_async(files_and_descriptions))

    async def _upload_files_async(self, files_and_descriptions: list[Union[str, tuple[str, str]]]) -> dict[str, int]:
        ...
        return {file_name: file_id for file_name, file_id in upload_tuples}
    ...
    async def _upload_file_async(
        ...
        create_upload_response = await create_uploaded_file.asyncio_detailed(
            client=self._authentication_manager._get_authenticated_client(),
            body=CreateUploadedFile(name=file_name, extension=file_extension, description=description),
        )
        file_id: int = self._validate_response(create_upload_response, UploadedFileResponse, UploadedFile).id
        ...
        if isinstance(start_upload_response_result.sas_link, str) and isinstance(
            ...
            complete_upload_response = await upload_progress.asyncio_detailed(
                file_id=file_id,
