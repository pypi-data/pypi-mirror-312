...
@overload
def _find_tag_with_default(parent: Element, path: str, default_value: None = None) -> str | None: ...


@overload
def _find_tag_with_default(parent: Element, path: str, default_value: str) -> str: ...


def _find_tag_with_default(parent: Element, path: str, default_value: str | None = None) -> str | None:
    child = parent.find(path)
    if child is None:
...
def parse_list_buckets(result, body):
    ...
    if root.find("Owner") is not None:
        result.owner = Owner(
            _find_tag_with_default(root, "Owner/DisplayName", ""), _find_tag_with_default(root, "Owner/ID", "")
        )
...
def parse_routing_rule_condition(condition_node):
    ...
    if condition_node.find("HttpErrorCodeReturnedEquals") is not None:
        ...
        http_err_code_return_equals = None
...
def parse_routing_rule_redirect(redirect_node):
    ...
    redirect = Redirect(
        ...
        mirror_url_slave=mirror_url_slave,
        mirror_url_probe=mirror_url_probe,
        mirror_pass_query_string=mirror_pass_query_string,
        mirror_follow_redirect=mirror_follow_redirect,
        mirror_check_md5=mirror_check_md5,
...
def parse_get_live_channel(result, body):
    ...
    target.frag_duration = int(_find_tag(root, "Target/FragDuration"))
    target.frag_count = int(_find_tag(root, "Target/FragCount"))
...
def parse_lifecycle_abort_multipart_upload(abort_multipart_upload_node):
    ...
    if abort_multipart_upload_node.find("Days") is not None:
        ...
        abort_multipart_upload.created_before_date = iso8601_to_date(
            _find_tag(abort_multipart_upload_node, "CreatedBeforeDate")
        )
...
def parse_lifecycle_storage_transitions(storage_transition_nodes):
    ...
    for storage_transition_node in storage_transition_nodes:
        ...
        if storage_transition_node.find("Days") is not None:
            ...
            storage_transition.created_before_date = iso8601_to_date(
                _find_tag(storage_transition_node, "CreatedBeforeDate")
            )
...
def parse_get_bucket_cors(result, body):
    ...
    for rule_node in root.findall("CORSRule"):
        ...
        if max_age_node is not None and max_age_node.text is not None:
...
def to_put_bucket_website(bucket_website):
    ...
    for rule in bucket_website.rules:
        ...
        if rule.condition.http_err_code_return_equals is not None:
            _add_text_child(
                condition_node, "HttpErrorCodeReturnedEquals", str(rule.condition.http_err_code_return_equals)
            )
        ...
        if rule.redirect is not None:
            ...
            # Mirror
                ...
                if rule.redirect.mirror_pass_query_string is not None:
                    _add_text_child(
                        redirect_node, "MirrorPassQueryString", str(rule.redirect.mirror_pass_query_string)
                    )
...
def to_put_bucket_lifecycle(bucket_lifecycle):
    ...
    for rule in bucket_lifecycle.rules:
        ...
        if storage_transitions:
            for storage_transition in storage_transitions:
                ...
                if storage_transition.is_access_time is not None:
                    _add_text_child(
                        storage_transition_node, "IsAccessTime", str(storage_transition.is_access_time).lower()
                    )
                if storage_transition.return_to_std_when_visit is not None:
                    _add_text_child(
                        storage_transition_node,
                        "ReturnToStdWhenVisit",
                        str(storage_transition.return_to_std_when_visit).lower(),
                    )
                if storage_transition.allow_small_file is not None:
                    _add_text_child(
                        storage_transition_node, "AllowSmallFile", str(storage_transition.allow_small_file).lower()
                    )
        ...
        if noncurrent_version_expiration is not None:
            ...
            _add_text_child(
                version_expiration_node, "NoncurrentDays", str(noncurrent_version_expiration.noncurrent_days)
            )
        ...
        if noncurrent_version_sotrage_transitions is not None:
            for noncurrent_version_sotrage_transition in noncurrent_version_sotrage_transitions:
                ...
                _add_text_child(
                    version_transition_node,
                    "NoncurrentDays",
                    str(noncurrent_version_sotrage_transition.noncurrent_days),
                )
                _add_text_child(
                    version_transition_node, "StorageClass", str(noncurrent_version_sotrage_transition.storage_class)
                )
                if noncurrent_version_sotrage_transition.is_access_time is not None:
                    _add_text_child(
                        version_transition_node,
                        "IsAccessTime",
                        str(noncurrent_version_sotrage_transition.is_access_time).lower(),
                    )
                if noncurrent_version_sotrage_transition.return_to_std_when_visit is not None:
                    _add_text_child(
                        version_transition_node,
                        "ReturnToStdWhenVisit",
                        str(noncurrent_version_sotrage_transition.return_to_std_when_visit).lower(),
                    )
                if noncurrent_version_sotrage_transition.allow_small_file is not None:
                    _add_text_child(
                        version_transition_node,
                        "AllowSmallFile",
                        str(noncurrent_version_sotrage_transition.allow_small_file).lower(),
                    )
...
def to_put_inventory_configuration(inventory_config):
    ...
    if inventory_config.inventory_filter is not None:
        ...
        if inventory_config.inventory_filter.last_modify_begin_time_stamp is not None:
            _add_text_child(
                ...
                str(inventory_config.inventory_filter.last_modify_begin_time_stamp),
            )
        if inventory_config.inventory_filter.last_modify_end_time_stamp is not None:
            _add_text_child(
                filter_node,
                "LastModifyEndTimeStamp",
                str(inventory_config.inventory_filter.last_modify_end_time_stamp),
            )
    ...
    if (
        inventory_config.inventory_destination is not None
        and inventory_config.inventory_destination.bucket_destination is not None
    ):
...
def get_Inventory_configuration_from_element(elem):
    ...
    if root.find("Filter") is not None:
        ...
        result.inventory_filter = InventoryFilter(
            prefix=prefix,
            last_modify_begin_time_stamp=last_modify_begin_time_stamp,
            last_modify_end_time_stamp=last_modify_end_time_stamp,
...
def parse_get_bucket_replication_result(result, body):
    ...
    for rule_node in root.findall("Rule"):
        ...
        rule.sse_kms_encrypted_objects_status = _find_tag_with_default(
            rule_node, "SourceSelectionCriteria/SseKmsEncryptedObjects/Status", None
        )
...
def parse_get_bucket_replication_location_result(result, body):
    ...
    if root.find("LocationTransferTypeConstraint") is not None:
        ...
        for transfer_type_node in constraint_node.findall("LocationTransferType"):
            ...
            location_transfer_type.transfer_type = _find_tag_with_default(
                transfer_type_node, "TransferTypes/Type", None
            )
...
def parse_do_bucket_meta_query_result(result, body):
    ...
    for file in root.findall("Files/File"):
        ...
        oss_tagging = file.find("OSSTagging")
        if oss_tagging is not None:
            for tagging in oss_tagging.findall("Tagging"):
                tmp_tagging = OSSTaggingInfo(_find_tag(tagging, "Key"), _find_tag(tagging, "Value"))
                tmp.oss_tagging.append(tmp_tagging)

        oss_user_meta = file.find("OSSUserMeta")
        if oss_user_meta is not None:
            for meta in oss_user_meta.findall("UserMeta"):
    ...
    for aggregation in root.findall("Aggregations/Aggregation"):
        ...
        for group in aggregation.findall("Groups/Group"):
            tmp_groups = AggregationGroupInfo(
                _find_tag(group, "Value"), int(_find_tag_with_default(group, "Count", "0"))
            )
...
def parse_lifecycle_filter(filter_node):
    ...
    if filter_node is not None:
        if filter_node.find("ObjectSizeGreaterThan") is not None:
            lifecycle_filter.object_size_greater_than = int(
                _find_tag_with_default(filter_node, "ObjectSizeGreaterThan", "0")
            )
        if filter_node.find("ObjectSizeLessThan") is not None:
            lifecycle_filter.object_size_less_than = int(
                _find_tag_with_default(filter_node, "ObjectSizeLessThan", "0")
            )
