...
def initialize_server(api_urls):
    """
    Выбирает сервер с наименьшей нагрузкой из списка.
    
    :param api_urls: Список URL API серверов.
    :return: URL сервера с наименьшей нагрузкой или None, если все недоступны.
    """
    server_loads = {url: get_server_load(url.replace("/api/check_spam", "/api/status")) for url in api_urls}
...
def is_spam(message, model_name='spamNS_v6', user_token=None): 
    """
    Проверяет, является ли сообщение спамом, используя предварительно выбранный сервер.
    ...
    if selected_server is None:
        selected_server = initialize_server(api_urls)
