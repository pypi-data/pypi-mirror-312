...
from grading_tools.common.gradable_spec import GradableSpec, load_spec, GradableNode
from grading_tools.common.utils import read_generic_table
from grading_tools.table_gen.gen_grading_table import save_grading_table, \
...
def merge_grading(merge_pattern: str = None, files: list[str] = None, spec_file: str = None,
    ...
    question_columns = [mk_node_name(n) for n in spec.tree.leaves_iter() if not n.get_info('skip')]
...
class Merging(CommandModule):
    module_name = 'merge'
    commands = [('grading', register_parser, merge_grading)]
...
if __name__ == '__main__':
    Merging().as_program('merge').parse_and_run()
