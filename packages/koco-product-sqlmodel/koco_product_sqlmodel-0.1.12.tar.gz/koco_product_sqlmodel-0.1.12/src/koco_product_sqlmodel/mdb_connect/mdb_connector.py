...
def delete_catalog_by_id(
    ...
    if cat:
        delete_catalog(
            catalog=cat, delete_connected_items=delete_connected_items, user_id=user_id
        )
...
def delete_catalog(
    ...
    if delete_connected_items:
        _delete_product_groups_from_catalog(
            catalog=catalog, delete_connected_items=True, user_id=user_id
        )
    with Session(mdb_engine) as session:
        mdb_change.log_results_to_db(
...
def delete_product_group(
    ...
    if delete_connected_items:
        _delete_families_from_product_group(
            product_group=product_group, delete_connected_items=True, user_id=user_id
        )
    with Session(mdb_engine) as session:
        mdb_change.log_results_to_db(
...
def _delete_families_from_product_group(
    ...
    for fam in families:
        delete_family(
            family=fam, delete_connected_items=delete_connected_items, user_id=user_id
        )
...
def delete_family(
    ...
    if delete_connected_items:
        _delete_urls_from_family(family, user_id=user_id)
        _delete_options_from_family(family, user_id=user_id)
        _delete_applications_from_family(family, user_id=user_id)
        _delete_spectables_from_family(
            family=family, delete_connected_items=True, user_id=user_id
        )
...
def _delete_articles_from_family(
    ...
    for art in articles:
        delete_article(
            article=art, delete_connected_items=delete_connected_items, user_id=user_id
        )
...
def _delete_spectables_from_family(
    ...
    for st in res:
        delete_spectable(
            spectable=st, delete_connected_items=delete_connected_items, user_id=user_id
        )


def _delete_urls_from_family(family: sqm.CFamily, user_id: int | None = None):
    with Session(mdb_engine) as session:
        statement = (
            select(sqm.CUrl)
        ...
        res = session.exec(statement).all()
    for url in res:
        _delete_url(url=url, user_id=user_id)


def _delete_options_from_family(family: sqm.CFamily, user_id: int | None = None):
    with Session(mdb_engine) as session:
        statement = select(sqm.COption).where(sqm.COption.family_id == family.id)
        res = session.exec(statement).all()
    for option in res:
        _delete_option(option=option, user_id=user_id)


def _delete_applications_from_family(family: sqm.CFamily, user_id: int | None = None):
    with Session(mdb_engine) as session:
    ...
    for application in res:
        _delete_application(application=application, user_id=user_id)
...
def delete_spectable(
    ...
    if delete_connected_items:
        _delete_spectableitems_from_spectable(spectable=spectable, user_id=user_id)
    with Session(mdb_engine) as session:
        if user_id != None:
            mdb_change.log_results_to_db(
...
def delete_article(
    ...
    if delete_connected_items:
        _delete_urls_from_article(article, user_id=user_id)
        _delete_spectables_from_article(
            article=article, delete_connected_items=True, user_id=user_id
        )
    with Session(mdb_engine) as session:
...
def _delete_urls_from_article(article: sqm.CArticle, user_id: int | None = None):
    with Session(mdb_engine) as session:
        statement = (
            select(sqm.CUrl)
...
def _delete_url(url: sqm.CUrl, user_id: int | None = None):
    with Session(mdb_engine) as session:
        if user_id != None:
            mdb_change.log_results_to_db(
                entity_type="curl",
...
def _delete_option(option: sqm.COption, user_id: int | None = None):
    with Session(mdb_engine) as session:
        if user_id != None:
            mdb_change.log_results_to_db(
                entity_type="coption",
...
def _delete_application(application: sqm.CApplication, user_id: int | None = None):
    with Session(mdb_engine) as session:
        if user_id != None:
            mdb_change.log_results_to_db(
